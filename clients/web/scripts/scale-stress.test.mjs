...
import {
  assertCliMutationBudgets,
  assertCliReadBudgets,
  assertReindexBudgets,
  assertWeb100kAcceptance,
  commitFixtureTier,
  parseArguments,
  parseScaleCounts,
  syntheticTicket,
} from './scale-stress.mjs';
...
describe('scale stress harness', () => {
  ...
  it('accepts repeatable-run switches', () => {
    expect(
      parseArguments([
        ...
        '--skip-web',
        '--assert-cli-budgets',
        '--assert-cli-mutation-budgets',
        '--assert-reindex-budgets',
        '--assert-web-100k',
        '--keep',
        '--timeout-ms=4000',
        '--output',
      ...
      keep: true,
      skipWeb: true,
      assertCliBudgets: true,
      assertCliMutationBudgets: true,
      assertReindexBudgets: true,
      assertWeb100k: true,
      timeoutMs: 4_000,
  ...
  it('enforces opt-in bounded CLI mutation budgets at the 10K and 100K tiers', () => {
    ...
    expect(() => assertCliMutationBudgets(10_000, { ...within, create_ticket: { wall_ms: 5_001 } })).toThrow(
      'create_ticket',
    );
    expect(() =>
      assertCliMutationBudgets(10_000, { ...within, create_ticket: { skipped: 'fixture commit did not complete' } }),
    ).toThrow('create_ticket');
    expect(() => assertCliMutationBudgets(100_000, { ...within, modify_ticket: { timed_out: true } })).toThrow(
      'modify_ticket',
    );
  });
  ...
  it('enforces opt-in bounded CLI read budgets at the 10K and 100K tiers', () => {
    const within = Object.fromEntries(
      ['list_first_100', 'full_text_query', 'show_ticket'].map((name) => [name, { wall_ms: 1_999 }]),
    );
    ...
    expect(() => assertCliReadBudgets(100_000, { ...within, full_text_query: { error: 'timeout' } })).toThrow(
      'full_text_query',
    );
  ...
  it('enforces the opt-in 100K browser and bounded-page acceptance thresholds', () => {
    ...
    const web = {
      scenarios: {
        initial_load: { wall_ms: 119_999 },
        switch_backlog: { wall_ms: 1_999 },
        switch_archive: { wall_ms: 1_999 },
        switch_queue: { wall_ms: 1_999 },
        view_switch_refresh_requests: 0,
        modify_ticket: { wall_ms: 29_999 },
        browser_heap_mb: 191.9,
      },
    };
    ...
    expect(() =>
      assertWeb100kAcceptance(
        100_000,
        { scenarios: { ...server.scenarios, list_compact: { ...page, response_bytes: 1_000_001 } } },
        web,
      ),
    ).toThrow('bounded-page');
    expect(() =>
      assertWeb100kAcceptance(100_000, server, { scenarios: { ...web.scenarios, browser_heap_mb: 193 } }),
    ).toThrow('initial load/heap');
    expect(() =>
      assertWeb100kAcceptance(100_000, server, { scenarios: { ...web.scenarios, modify_ticket: { wall_ms: 30_001 } } }),
    ).toThrow('modify_ticket interaction');
    expect(() =>
      assertWeb100kAcceptance(100_000, server, { scenarios: { ...web.scenarios, view_switch_refresh_requests: 1 } }),
    ).toThrow('redundant ticket refreshes');
  ...
  it('generates unique canonical ticket fixtures across every shipped view', () => {
    ...
    expect(tickets.map((ticket) => ticket.body.match(/status: ([a-z_]+)/)?.[1])).toEqual([
      'archive',
      'started',
      'not_started',
      'backlog',
  ...
  it('records a fixture-stage timeout and does not attempt an unsafe commit', async () => {
    ...
    await expect(
      commitFixtureTier({ store: '/tmp/scale.hs2', count: 1_000_000, env: {}, timeoutMs: 300_000, run }),
    ).resolves.toEqual({
  ...
  it('records a fixture-commit failure and marks later mutation probes unsafe', async () => {
    const run = vi
      .fn()
      .mockResolvedValueOnce({ wall_ms: 10, peak_rss_kb: 1024, exit_code: 0, timed_out: false })
      .mockResolvedValueOnce({ wall_ms: 20, peak_rss_kb: 1024, exit_code: 1, timed_out: false });
    await expect(
      commitFixtureTier({ store: '/tmp/scale.hs2', count: 10_000, env: {}, timeoutMs: 1_000, run }),
    ).resolves.toEqual({
