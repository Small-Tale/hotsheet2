...
async function mockBulkProject(page: Page, { conflictFirst = false }: { conflictFirst?: boolean } = {}) {
  ...
  let pollCursor = 0,
    emitTicketChange = false,
    holdTicketRefresh = false,
    staleRefreshStarted = false,
    staleRows = rows.map((row) => ({ ...row })),
    releaseStaleRefresh!: () => void;
  const staleRefreshResponse = new Promise<void>((resolve) => {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (path === '/__hotsheet/projects/open')
      return route.fulfill({
        status: 201,
        json: request.postDataJSON().root === otherProject.root ? otherProject : project,
      });
    ...
    if (path.endsWith('/tickets') && request.method() === 'GET') {
      if (holdTicketRefresh && path.includes('/demo-checkout/')) {
        holdTicketRefresh = false;
        staleRefreshStarted = true;
        await staleRefreshResponse;
        return route.fulfill({ json: staleRows });
      }
      return route.fulfill({
        json: path.includes('/other-checkout/')
          ? [
              {
                ...base,
                native_id: 'other',
    ...
    if (path.endsWith('/batch') && request.method() === 'POST') {
      ...
      if (conflictFirst && batches.length === 1)
        return route.fulfill({ status: 409, json: { error: 'Ticket modified concurrently by another client.' } });
      const stale = updates.find(
        (update) => rows.find((row) => row.id === update.id)?.updated_at !== update.expected_token,
      );
      ...
      rows = rows.map((row) => {
        ...
        return update ? { ...row, status: update.status, updated_at: token } : row;
      });
      const changed = updates.map((update) => ({
        ...rows.find((row) => row.id === update.id)!,
        store: 'git-local',
        details: '',
        blocked_reason: null,
        notes: [],
    ...
    if (path.includes('/tickets/') && request.method() === 'GET') {
      const id = path.split('/').pop(),
        ticket = rows.find((row) => row.id === id);
      return route.fulfill({
        json: {
          store: 'git-local',
          ...ticket,
          ...
          notes: [],
          attachments: [],
          concurrency_token: ticket?.updated_at,
        },
      });
    }
    if (
    ...
    if (path.endsWith('/ws/poll')) {
      if (emitTicketChange) {
        ...
        return route.fulfill({
          json: {
            cursor: pollCursor,
            events: [{ store: 'git-local', kind: 'updated', id: 'external', slug: 'HS2-EXTERNAL' }],
            overflow: false,
          },
        });
      }
  ...
  return {
    ...
    beginStaleRefresh: () => {
      staleRows = rows.map((row) => ({ ...row }));
      holdTicketRefresh = true;
      emitTicketChange = true;
    },
    staleRefreshStarted: () => staleRefreshStarted,
    releaseStaleRefresh: () => {
      releaseStaleRefresh();
    },
  };
...
test('serializes rapid Verified then Archive batches onto fresh concurrency tokens (HS2-0K2ZP3)', async ({ page }) => {
  ...
  const menu = page.getByRole('menu', { name: 'Ticket actions' }),
    completed = page.locator('[data-column-id="completed"]'),
    first = completed.locator('[data-ticket-slug="HS2-FAST01"]'),
    second = completed.locator('[data-ticket-slug="HS2-FAST02"]');
  await first.click();
  await second.click({ modifiers: ['Meta'] });
  await first.click({ button: 'right' });
  await menu.locator('[data-context-action="Verify ticket"]').click();
  await expect.poll(() => mock.batches.length).toBe(1);
  const verified = page.locator('[data-column-id="verified"]'),
    verifiedFirst = verified.locator('[data-ticket-slug="HS2-FAST01"]');
  ...
  expect(mock.batches[0].map((update) => update.expected_token)).toEqual([
    '2026-09-11T01:00:00Z',
    '2026-09-11T01:00:00Z',
  ]);
  mock.releaseVerified();
  await expect.poll(() => mock.batches.length).toBe(2);
  expect(mock.batches[1].map((update) => update.expected_token)).toEqual([
...
test('finishes queued bulk changes for their owning project after a project switch', async ({ page }) => {
  ...
  await expect(page.getByRole('tab', { name: 'Other project' })).toHaveAttribute('aria-selected', 'true');
  await page.getByRole('tab', { name: 'Bulk status demo' }).click();
  await page.getByRole('button', { name: 'Columns view' }).click();
  const menu = page.getByRole('menu', { name: 'Ticket actions' }),
    completed = page.locator('[data-column-id="completed"]'),
    first = completed.locator('[data-ticket-slug="HS2-FAST01"]'),
  ...
  await menu.locator('[data-context-action="Verify ticket"]').click();
  await expect.poll(() => mock.batches.length).toBe(1);
  const verifiedFirst = page.locator('[data-column-id="verified"] [data-ticket-slug="HS2-FAST01"]');
  await verifiedFirst.click({ button: 'right' });
  await menu.locator('[data-context-action="Archive ticket"]').click();
  await page.getByRole('tab', { name: 'Other project' }).click();
  await expect(page.locator('[data-ticket-slug="HS2-OTHER1"]')).toBeVisible();
  mock.releaseVerified();
  await expect.poll(() => mock.batches.length).toBe(2);
  await expect.poll(() => mock.getRows().map((row) => row.status)).toEqual(['archive', 'archive']);
  await expect(page.getByRole('alert').filter({ hasText: 'selected project changed' })).toHaveCount(0);
  await page.getByRole('tab', { name: 'Bulk status demo' }).click();
  await page.locator('[data-action="select-view"][data-item-id="archive"]').click();
  await expect(page.locator('[data-ticket-slug="HS2-FAST01"]')).toBeVisible();
  await expect(page.locator('[data-ticket-slug="HS2-FAST02"]')).toBeVisible();
...
test('rolls a genuine external atomic conflict back to the prior status', async ({ page }) => {
  ...
  await openBulkProject(page);
  const first = page.locator('[data-column-id="completed"] [data-ticket-slug="HS2-FAST01"]'),
    second = page.locator('[data-column-id="completed"] [data-ticket-slug="HS2-FAST02"]'),
...
test('keeps archived tickets hidden when a refresh was already in flight (HS2-913HFN)', async ({ page }) => {
  ...
  const verified = page.locator('[data-column-id="verified"]'),
    verifiedFirst = verified.locator('[data-ticket-slug="HS2-FAST01"]'),
    verifiedSecond = verified.locator('[data-ticket-slug="HS2-FAST02"]');
  await expect(verifiedFirst).toBeVisible();
  await expect(verifiedSecond).toBeVisible();
