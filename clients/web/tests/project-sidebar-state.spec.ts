...
test('never renders one project sidebar with another project statistics', async ({ page }) => {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (/\/tickets\/HS2-DEMO1$/.test(path))
      return route.fulfill({
        json: {
          ticket: {
            ...ticket('HS2-DEMO1', 'started'),
            category: 'bug',
            priority: 'default',
            details: '',
            notes: [],
    ...
    if (path.endsWith('/tickets')) {
      ...
      const counts = other
        ? {
            ...
            started: 1,
            completed_today: 1,
            completion_trend: [0, 0, 0, 0, 1, 0, 1],
          }
        : {
            total: 5,
            queued: 5,
            ...
            started: 4,
            completed_today: 2,
            completion_trend: [0, 1, 0, 2, 0, 1, 2],
          };
      return route.fulfill({
        json: {
          items: other
            ? [ticket('HS2-OTHER1', 'started'), ticket('HS2-OTHER2', 'not_started')]
            : [ticket('HS2-DEMO1', 'started')],
          counts,
        },
      });
  ...
  releaseOther();
  await expect(summary).toHaveAccessibleName('Open project statistics: 1 completed today, 1 in progress');
  ...
  await expect(summary).toHaveAccessibleName('Open project statistics: 2 completed today, 4 in progress');
  await expect(chart).toHaveAttribute('aria-label', 'Tickets completed over the last 7 days: 0, 1, 0, 2, 0, 1, 2');
  await expect(page.locator('[data-ticket-slug="HS2-DEMO1"]')).toBeVisible();
  await expect(page.locator('[data-ticket-slug^="HS2-OTHER"]')).toHaveCount(0);
  await page.locator('[data-ticket-slug="HS2-DEMO1"]').click();
  await expect(page.locator('[data-component="ticket-reader"][aria-hidden="true"]')).toBeHidden();
  await expect(summary).toHaveAccessibleName('Open project statistics: 2 completed today, 4 in progress');
  await page.getByRole('button', { name: 'New ticket…' }).click();
  await page.getByRole('dialog', { name: 'Create ticket' }).getByRole('button', { name: 'Cancel' }).click();
...
test('an obsolete background count snapshot cannot overwrite a project after an activation round trip', async ({
  ...
  let demoPoll: import('@playwright/test').Route | undefined,
    staleTickets: import('@playwright/test').Route | undefined,
    finalTickets: import('@playwright/test').Route | undefined;
  let demoTicketRequests = 0;
  const counts = (started: number, completed: number, trend: number[]) => ({
    total: started + 1,
    queued: started + 1,
    backlog: 0,
    archive: 0,
  ...
  await page.route('**/*', async (route) => {
    ...
    if (path.endsWith('/tickets')) {
      ...
      if (demoTicketRequests === 2) {
        staleTickets = route;
        return;
      }
      if (demoTicketRequests === 4) {
        finalTickets = route;
        return;
      }
  ...
  await demoPoll!.fulfill({
    json: {
      cursor: 2,
      events: [{ store: 'git-local', kind: 'changed', id: 'HS2-DEMO', slug: 'HS2-DEMO' }],
      overflow: false,
    },
  });
  ...
  await expect(summary).toHaveAccessibleName('Open project statistics: 3 completed today, 7 in progress');
  await expect(summary.getByRole('img')).toHaveAttribute(
    'aria-label',
...
test('keeps the authoritative completion trend after a local delete drops the counts snapshot (HS2-BRDMBB)', async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 800 });
  ...
  let rows = [
    ticket('HS2-S1', 'started'),
    ticket('HS2-S2', 'started'),
    ticket('HS2-S3', 'started'),
    ticket('HS2-S4', 'started'),
    ticket('HS2-DEL', 'not_started'),
  ];
  const counts = {
    total: 8,
    queued: 5,
  ...
  await page.route('**/*', async (route) => {
    ...
    if (path.endsWith('/batch') && request.method() === 'POST') {
      const updates = request.postDataJSON().updates as Array<{ id: string } & Record<string, unknown>>;
      rows = rows.map((item) => {
        const u = updates.find((v) => v.id === item.id);
        return u ? { ...item, ...u } : item;
      });
      const changed = updates.map((u) => {
        const t = rows.find((item) => item.id === u.id)!;
        return {
          store: 'git-local',
          ...t,
          details: '',
          ...
          notes: [],
          attachments: [],
          concurrency_token: `next-${u.id}`,
        };
      });
      return route.fulfill({ json: changed });
    }
    if (path.endsWith('/tickets') && request.method() === 'GET')
      return route.fulfill({ json: { items: rows.filter((item) => item.status !== 'deleted'), counts } });
    if (path.endsWith('/repository/status'))
      return route.fulfill({
        json: { branch: 'main', ahead: 0, behind: 0, staged: 0, unstaged: 0, untracked: 0, conflicted: 0, clean: true },
      });
  ...
  await expect(summary).toHaveAccessibleName('Open project statistics: 2 completed today, 4 in progress');
  await expect(chart).toHaveAttribute('aria-label', 'Tickets completed over the last 7 days: 0, 1, 0, 2, 0, 1, 2');
  const del = page.locator('[data-component="ticket-list-row"][data-ticket-slug="HS2-DEL"]');
  await del.click();
  await del.click({ button: 'right' });
