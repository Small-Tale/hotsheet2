...
const counts={total:1,queued:1,backlog:0,archive:0,open:1,up_next:0,active:0,started:1,completed_today:0,completion_trend:[0,0,0,0,0,0,0]};
...
async function installFixture(page:import('@playwright/test').Page){
  await page.route('**/*',route=>{
    ...
    if(path.endsWith('/tickets'))return route.fulfill({json:{items:[ticket(other?'HS2-OTHER1':'HS2-DEMO1','started')],counts}});
