...
const project = {
  ...
  root: '/work/no-git',
  name: 'No Git project',
  stores: ['/work/no-git.hs2'],
  apiPath: '/__hotsheet/project-api/no-git',
};

test('recovers a project folder without Git and optionally connects origin', async ({ page }) => {
  let initialized = false,
    initBody: string | null | undefined,
    remoteBody: unknown;
  await page.route('**/*', (route) => {
    ...
    if (path.endsWith('/repository/remote') && request.method() === 'POST') {
      remoteBody = request.postDataJSON();
      return route.fulfill({
        json: {
          initialized: true,
          branch: 'master',
    ...
    if (path.endsWith('/repository/files'))
      return route.fulfill({
        json: {
          items: [{ path: 'existing.txt', unstaged: 'untracked', untracked: true, conflicted: false }],
          next_cursor: null,
        },
      });
  ...
  const initialize = dialog.locator('[data-action="initialize-repository"]');
  await expect
    .poll(() =>
      initialize.evaluate((node) => {
        const button = node.getBoundingClientRect(),
          footer = node.closest('footer')!.getBoundingClientRect();
        return Math.max(
          Math.abs(button.x + button.width / 2 - footer.x - footer.width / 2),
          Math.abs(button.y + button.height / 2 - footer.y - footer.height / 2),
        );
      }),
    )
    .toBeLessThan(1);
