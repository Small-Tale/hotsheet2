...
test('doubles reader description and note text with a remembered global toggle',async({page})=>{
  await page.setViewportSize({width:1280,height:900});await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();await page.locator('[data-ticket-slug="HS2-DEMO01"]').click();await page.getByRole('button',{name:'Open ticket reader'}).click();
  let reader=page.getByRole('dialog',{name:'Read and edit HS2-DEMO01'});const details=reader.locator('.ticket-inspector__details-surface .markdown-preview').first(),note=reader.locator('article[data-note-id="N3"] .note-card__body'),activity=reader.locator('article[data-note-id="N1"] .note-card__body'),toggle=reader.locator('[data-action="toggle-reader-text-size"]'),ordinary={details:parseFloat(await details.evaluate(element=>getComputedStyle(element).fontSize)),note:parseFloat(await note.evaluate(element=>getComputedStyle(element).fontSize)),activity:parseFloat(await activity.evaluate(element=>getComputedStyle(element).fontSize))};await expect(toggle).toHaveAttribute('aria-label','Use large reader text size');await toggle.click();await expect(reader).toHaveAttribute('data-large-text','true');await expect(toggle).toHaveAttribute('aria-pressed','true');await expect(toggle).toHaveAttribute('aria-label','Use standard reader text size');await expect.poll(async()=>parseFloat(await details.evaluate(element=>getComputedStyle(element).fontSize))).toBeCloseTo(ordinary.details*2,1);await expect.poll(async()=>parseFloat(await note.evaluate(element=>getComputedStyle(element).fontSize))).toBeCloseTo(ordinary.note*2,1);await expect.poll(async()=>parseFloat(await activity.evaluate(element=>getComputedStyle(element).fontSize))).toBeCloseTo(ordinary.activity*2,1);await expect(page.evaluate(()=>localStorage.getItem('hotsheet.reader.large-text'))).resolves.toBe('true');await page.screenshot({path:'/private/tmp/hs2-28frr0-reader-large-text.png',fullPage:true});await reader.getByRole('button',{name:'Close ticket reader'}).click();await page.getByRole('button',{name:'Open ticket reader'}).click();reader=page.getByRole('dialog',{name:'Read and edit HS2-DEMO01'});await expect(reader).toHaveAttribute('data-large-text','true');await reader.getByRole('button',{name:'Use standard reader text size'}).click();await expect(page.evaluate(()=>localStorage.getItem('hotsheet.reader.large-text'))).resolves.toBe('false');
});
