...
test('edits non-empty details on double click and empty details on one click',async({page})=>{
  ...
  const reader=page.getByRole('dialog',{name:/Read and edit HS2-DEMO01/});await expect(reader).toBeVisible();const readerPreview=reader.getByRole('button',{name:'Edit Ticket details'});await expect(readerPreview).toContainText('Carried into the larger editor');await readerPreview.dblclick();await expect(reader.getByRole('textbox',{name:'Ticket details'})).toHaveValue('Carried into the larger editor');await expect(reader.getByRole('textbox',{name:'Feedback response'})).toBeVisible();await reader.getByRole('button',{name:'Close ticket reader'}).click();
