...
test('browses repository files and commits with host-native actions',async({page})=>{
  ...
  await expect(popover).toHaveAttribute('data-view','conflicted');await expect(popover.locator('[data-repository-file="src/conflict.ts"]')).toBeVisible();
  const master=popover.locator('aside');expect(await master.evaluate(element=>element.scrollHeight===element.clientHeight)).toBe(true);await popover.getByRole('button',{name:/Staged 2/}).click();await expect(popover.locator('[data-repository-file]')).toHaveCount(2);const renamed=popover.locator('[data-change="renamed"]');await expect(renamed.locator('.repository-status-popover__file-status')).toHaveText('R');await expect(renamed.locator('.repository-status-popover__file-status')).toHaveAttribute('aria-label','Renamed');await expect(renamed.locator('.repository-status-popover__file-state')).toHaveCount(0);await expect(popover.getByRole('button',{name:/Untracked 1/}).locator('[data-lucide="square-pen"]')).toBeVisible();expect(await renamed.locator('.repository-status-popover__path > span').first().evaluate(element=>element.scrollWidth>element.clientWidth)).toBe(true);
  const staged=popover.locator('[data-repository-file="src/staged.ts"]');await staged.dblclick();await expect.poll(()=>actions.some(action=>action.action==='open'&&action.path==='src/staged.ts')).toBe(true);await staged.click({button:'right'});await expect(popover.getByRole('menuitem',{name:'Show in Finder'})).toBeVisible();await popover.getByRole('menuitem',{name:'Show in Finder'}).click();await expect.poll(()=>actions.some(action=>action.action==='reveal'&&action.path==='src/staged.ts')).toBe(true);
