...
test('browses repository files and commits with host-native actions',async({page})=>{
  await mockProject(page);const actions:Array<Record<string,unknown>>=[];let failed=false,response={branch:'feature/repository-dialog',upstream:'origin/main',ahead:2,behind:1,staged:2,unstaged:1,untracked:1,conflicted:1,clean:false,root:'/work/demo',platform:'macos',commit_count:24,difftool:'Glassbox',truncated:false,files:[{path:'src/staged.ts',staged:'added',untracked:false,conflicted:false},{path:'src/renamed.ts',original_path:'src/old.ts',staged:'renamed',untracked:false,conflicted:false},{path:'src/changed.ts',unstaged:'modified',untracked:false,conflicted:false},{path:'notes/new.md',untracked:true,conflicted:false},{path:'src/conflict.ts',staged:'unmerged',unstaged:'unmerged',untracked:false,conflicted:true}],ranges:[{from:'aaa1111',to:'bbb2222',count:2}],commits:[{sha:'bbb2222',short_sha:'bbb2222',subject:'Finish repository status dialog',committed_at:'2026-09-04T10:00:00Z'},{sha:'aaa1111',short_sha:'aaa1111',subject:'Start repository status dialog',committed_at:'2026-09-04T09:00:00Z'}]};
  ...
  await popover.getByRole('button',{name:/Staged 2/}).click();await expect(popover.locator('[data-repository-file]')).toHaveCount(2);await expect(popover.locator('[data-change="renamed"]')).toContainText('from src/old.ts');
  ...
  await popover.getByRole('button',{name:/Commits 24/}).click();await expect(popover).toContainText('Finish repository status dialog');await popover.getByRole('button',{name:/Open 2 commit bundle/}).click();await expect.poll(()=>actions.some(action=>action.mode==='range'&&action.from==='aaa1111'&&action.to==='bbb2222')).toBe(true);
  const detail=popover.locator('.repository-status-popover__detail');await expect(detail).toHaveCSS('overflow-y','auto');await popover.screenshot({path:'/private/tmp/hs2-323xhg-repository-status-wide.png'});await page.setViewportSize({width:760,height:640});await popover.screenshot({path:'/private/tmp/hs2-323xhg-repository-status-narrow.png'});
