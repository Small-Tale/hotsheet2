...
test('matches HS1 multi-selection semantics and selected outlines in list and column views',async({page})=>{
  await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();
  const first=page.locator('[data-component="ticket-list-row"][data-ticket-slug="HS2-DEMO01"]'),second=page.locator('[data-component="ticket-list-row"]').nth(1);await first.click();await second.click({modifiers:['Meta']});await expect(first).toHaveAttribute('data-selected','true');await expect(second).toHaveAttribute('data-selected','true');await expect(first).toHaveCSS('border-color','rgb(96, 165, 250)');const selectedBoxes=await Promise.all([first,second].map(locator=>locator.boundingBox()));expect(Math.abs(selectedBoxes[0]!.y+selectedBoxes[0]!.height-selectedBoxes[1]!.y)).toBe(1);
...
test('offers completed verification actions only for one completed ticket',async({page})=>{
  const patches=await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();const completed=page.locator('[data-ticket-slug="HS2-DONE01"]'),verified=page.locator('[data-ticket-slug="HS2-VERIFY01"]'),menu=page.getByRole('menu',{name:'Ticket actions'});
  await completed.click({button:'right'});await expect(menu.getByRole('menuitem',{name:'Verified',exact:true})).toBeVisible();await expect(menu.getByRole('menuitem',{name:'Not Working…',exact:true})).toBeVisible();await expect(menu.getByText('Toggle Up Next')).toHaveCount(0);await menu.getByRole('menuitem',{name:'Verified',exact:true}).click();await expect.poll(()=>patches.some(patch=>patch.status==='verified')).toBe(true);
...
test('ships TicketRow context-menu behavior through real list and board compositions',async({page})=>{
  ...
  await choose('category','bug');await choose('priority','low');await choose('status','verified');await first.click({button:'right'});await menu.getByText('Open ticket').click();await expect(page.getByRole('dialog',{name:/Read and edit HS2-DEMO01/})).toBeVisible();await page.getByRole('button',{name:'Close ticket reader'}).click();
