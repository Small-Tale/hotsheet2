...
test('renders attachment identity from a selected real ticket',async({page})=>{
  ...
  const thumbnail=page.getByRole('button',{name:'Open proof.png in image gallery'});await expect(thumbnail).toBeVisible();const thumbnailBox=await thumbnail.boundingBox();expect(thumbnailBox?.width).toBe(thumbnailBox?.height);await thumbnail.click();const gallery=page.getByRole('dialog',{name:/Image 1 of 1: proof.png/});await expect(gallery).toBeVisible();await gallery.screenshot({path:'/private/tmp/hs2-64651d-gallery-wide.png'});await page.keyboard.press('Escape');await expect(gallery).toHaveCount(0);
