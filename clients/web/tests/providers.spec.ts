...
test('animates ticket moves, arrivals, and departures in sequence',async({page})=>{
  await page.emulateMedia({reducedMotion:'no-preference'});await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();await page.getByLabel('Columns view').click();
  await page.evaluate(()=>{
    // eslint-disable-next-line @typescript-eslint/unbound-method
    const original=Element.prototype.animate;
    const records:Array<{slug:string;frames:Keyframe[];delay:number;duration:number}>=[];(window as typeof window&{__ticketAnimations?:typeof records}).__ticketAnimations=records;Element.prototype.animate=function(keyframes:Keyframe[]|PropertyIndexedKeyframes|null,options?:number|KeyframeAnimationOptions){const animation=original.call(this,keyframes,options),effect=animation.effect as KeyframeEffect|null;if(this.matches('[data-component="ticket-list-row"][data-ticket-slug]')&&effect){const timing=effect.getTiming();records.push({slug:(this as HTMLElement).dataset.ticketSlug??'',frames:effect.getKeyframes(),delay:Number(timing.delay),duration:Number(timing.duration)})}return animation};
  });
  const records=()=>page.evaluate(()=>(window as typeof window&{__ticketAnimations?:Array<{slug:string;frames:Keyframe[];delay:number;duration:number}>}).__ticketAnimations??[]),clear=()=>page.evaluate(()=>{const records=(window as typeof window&{__ticketAnimations?:unknown[]}).__ticketAnimations;if(records)records.length=0});
  const moving=page.locator('[data-column-id="started"] [data-ticket-slug="HS2-DEMO01"]');await moving.click();const status=page.locator('wa-select[name="inspector-status"]');await status.click();await status.locator('wa-option[value="completed"]').click();await expect(page.locator('[data-column-id="completed"] [data-ticket-slug="HS2-DEMO01"]')).toBeVisible();await expect.poll(async()=>{const calls=await records();return calls.some(call=>call.slug==='HS2-DEMO01'&&call.frames.some(frame=>frame.zIndex==='30'))&&calls.some(call=>call.slug==='HS2-START02'&&call.frames.some(frame=>String(frame.transform).startsWith('translate(')))}).toBe(true);
  await clear();await page.locator('[data-column-id="completed"] [data-ticket-slug="HS2-DEMO01"]').click({button:'right'});await page.getByRole('menu',{name:'Ticket actions'}).locator('[data-context-action="Archive ticket"]').click();await expect.poll(async()=>{const calls=await records();return calls.some(call=>call.slug==='HS2-DEMO01'&&call.duration===150&&call.frames.some(frame=>Number(frame.opacity)===0))&&calls.some(call=>call.delay===130)}).toBe(true);await expect(page.locator('[data-ticket-slug="HS2-DEMO01"]')).toHaveCount(0);
  await clear();await page.getByRole('button',{name:'New ticket…'}).click();await page.getByRole('textbox',{name:'Ticket title'}).fill('Animated incoming ticket');await page.getByRole('button',{name:'Create ticket'}).click();await expect(page.locator('[data-column-id="not-started"] [data-ticket-slug="HS2-NEW001"]')).toBeVisible();await expect.poll(async()=>(await records()).some(call=>call.slug==='HS2-NEW001'&&call.frames.some(frame=>Number(frame.opacity)===0)&&call.frames.some(frame=>Number(frame.opacity)===1))).toBe(true);await page.screenshot({path:'/private/tmp/hs2-ticket-motion-board.png',fullPage:true});
});
