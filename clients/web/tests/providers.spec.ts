...
test('browses repository files and commits with host-native actions',async({page})=>{
  ...
  await page.evaluate(()=>getSelection()?.removeAllRanges());await popover.screenshot({path:'/private/tmp/hs2-stva92-repository-files-wide.png'});await page.setViewportSize({width:760,height:640});await popover.screenshot({path:'/private/tmp/hs2-stva92-repository-files-narrow.png'});await page.setViewportSize({width:1280,height:900});await popover.getByRole('button',{name:/Commits 24/}).click();await expect(popover).toContainText('Finish repository status dialog');await popover.getByRole('button',{name:/Open 2 commit bundle/}).click();await expect.poll(()=>actions.some(action=>action.mode==='range'&&action.from==='aaa1111'&&action.to==='bbb2222')).toBe(true);
