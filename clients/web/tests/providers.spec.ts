...
async function mockProject(page: import('@playwright/test').Page, canUpdate = true, primaryFeedbackNeeded = false, ticketLoadDelay = 0, batchResponseDelay = 0) {
...
test('switches settings categories from the project sidebar',async({page})=>{
  await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();
  await page.getByLabel('Settings view').click();const navigation=page.getByRole('complementary',{name:'Settings categories'});await expect(navigation).toBeVisible();await expect(page.getByRole('heading',{name:'Ticket sources'})).toBeVisible();await expect(page.locator('.project-settings > h2')).toHaveCount(0);await expect(page.locator('[data-component="project-sidebar"]')).toHaveCount(0);await expect(page.getByRole('complementary',{name:'Ticket inspector'})).toBeVisible();
  ...
  await page.screenshot({path:'/private/tmp/hs2-wsmx7c-settings-sidebar-wide.png',fullPage:true});await page.setViewportSize({width:1024,height:600});await expect(page.getByRole('heading',{name:'Column view'})).toBeVisible();await page.screenshot({path:'/private/tmp/hs2-wsmx7c-settings-sidebar-floor.png',fullPage:true});
});
