...
test('browses repository files and commits with host-native actions',async({page})=>{
  ...
  await popover.getByRole('button',{name:'Compare two commits'}).click();await expect(popover.locator('.ticket-code-review__compare-banner')).toContainText('Select the A side');await newest.locator('.ticket-code-review__commit-summary').click();await expect(newest.locator('.ticket-code-review__compare-label')).toHaveText('A');await expect(popover.locator('.ticket-code-review__compare-banner')).toContainText('Select the B side');const oldest=popover.locator('li[data-commit-sha="aaa1111"]');await oldest.locator('.ticket-code-review__commit-summary').click();await expect(oldest.locator('.ticket-code-review__compare-label')).toHaveText('B');const openCompare=popover.getByRole('button',{name:'Open comparison in Glassbox'});await expect(openCompare).toBeEnabled();await openCompare.click();await expect.poll(()=>actions.some(action=>action.mode==='compare'&&action.from==='bbb2222'&&action.to==='aaa1111')).toBe(true);
