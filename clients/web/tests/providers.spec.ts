...
test('streams ANSI terminal output, input, viewport leases, driver state, and reconnects without exposing credentials',async({page})=>{
  ...
  const viewport=page.locator('[data-component="terminal-viewport"][data-terminal-id="codex-main"]').first();await expect(viewport).toHaveAttribute('data-connection','connected');await expect(viewport).toHaveAttribute('data-renderer','dom');await expect(viewport.locator('.xterm-rows')).toContainText('Live terminal ready');await expect(viewport).toHaveAttribute('data-pty-size','100x30');await expect(viewport).toHaveAttribute('data-driving','true');
...
test('opens, navigates, resizes, zooms, creates, hides, and restores the project terminal drawer',async({page})=>{
  ...
  await page.getByRole('button',{name:'Terminal dashboard'}).click();const globalTile=page.locator('[data-component="terminal-tile"][data-terminal-key="demo-checkout:codex-main"]');await globalTile.hover();await globalTile.getByRole('button',{name:'Open codex-main in demo'}).click();const drawer=page.locator('[data-component="terminal-drawer"]');await expect(drawer).toBeVisible();await expect(drawer).toHaveAttribute('data-mode','dedicated');const selectedTerminalTab=drawer.locator('[data-component="terminal-tab"][data-selected="true"]');await expect(selectedTerminalTab.getByRole('tab')).toHaveAttribute('aria-selected','true');const selectedSurfaces=await selectedTerminalTab.evaluate(element=>({root:getComputedStyle(element).backgroundColor,select:getComputedStyle(element.querySelector('.app-tab__select')!).backgroundColor}));expect(selectedSurfaces.root).not.toBe('rgba(0, 0, 0, 0)');expect(selectedSurfaces.select).toBe('rgba(0, 0, 0, 0)');
...
test('resizes the terminal drawer to the page-header boundary',async({page})=>{
  await page.setViewportSize({width:1440,height:900});await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();await page.getByRole('button',{name:'Show terminal drawer'}).click();const main=page.locator('.app-shell__main'),header=page.locator('[data-component="page-header"]'),region=page.locator('section[data-region-id="app-terminal-drawer"]'),handle=page.getByRole('separator',{name:'Resize Terminal drawer'});const geometry=await main.evaluate((element,pageHeader)=>({bottom:element.getBoundingClientRect().bottom,top:(pageHeader as HTMLElement).getBoundingClientRect().bottom}),await header.elementHandle());const expected=Math.floor(geometry.bottom-geometry.top);await expect(handle).toHaveAttribute('aria-valuemax',String(expected));const grip=(await handle.boundingBox())!;await page.mouse.move(grip.x+grip.width/2,grip.y+grip.height/2);await page.mouse.down();await page.mouse.move(grip.x+grip.width/2,geometry.top-20);await page.mouse.up();await expect(handle).toHaveAttribute('aria-valuenow',String(expected));expect(Math.abs((await region.boundingBox())!.y-geometry.top)).toBeLessThanOrEqual(1);expect(await page.evaluate(({x,y})=>Boolean(document.elementFromPoint(x,y)?.closest('[data-component="terminal-drawer"]')),{x:(await region.boundingBox())!.x+20,y:geometry.top+20})).toBe(true);await expect(page.evaluate(()=>localStorage.getItem('hotsheet.layout.app-terminal-drawer.size'))).resolves.toBe(String(expected));await page.screenshot({path:'/private/tmp/hs2-4fzgm7-drawer-max.png',fullPage:true});
});

test('double-clicks the drawer rail to maximize and restore the last manual height',async({page})=>{
  await page.setViewportSize({width:1440,height:900});await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();await page.getByRole('button',{name:'Show terminal drawer'}).click();const drawer=page.locator('[data-component="terminal-drawer"]'),rail=drawer.locator('.terminal-drawer__rail'),handle=page.getByRole('separator',{name:'Resize Terminal drawer'});await handle.focus();await page.keyboard.press('ArrowUp');await expect(handle).toHaveAttribute('aria-valuenow','336');await rail.dispatchEvent('dblclick');const maximum=await handle.getAttribute('aria-valuemax');await expect(drawer).toHaveAttribute('data-maximized','true');await expect(handle).toHaveAttribute('aria-valuenow',maximum!);await expect(page.evaluate(()=>localStorage.getItem('hotsheet.layout.app-terminal-drawer.size'))).resolves.toBe('336');await drawer.getByRole('tab',{name:'Terminal grid'}).dispatchEvent('dblclick');await expect(drawer).toHaveAttribute('data-maximized','true');await page.screenshot({path:'/private/tmp/hs2-tkx9q7-drawer-maximized.png',fullPage:true});await rail.dispatchEvent('dblclick');await expect(drawer).toHaveAttribute('data-maximized','false');await expect(handle).toHaveAttribute('aria-valuenow','336');await page.screenshot({path:'/private/tmp/hs2-tkx9q7-drawer-restored.png',fullPage:true});
});
...
test('preserves list and every board-column scroll position across ticket mutations (HS2-CEBNAJ)',async({page})=>{
  ...
  await page.setViewportSize({width:1400,height:760});await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();const workspace=page.locator('.app-shell__workspace');const listTarget=page.locator('[data-component="ticket-list-row"][data-status="completed"]').nth(8),listSlug=await listTarget.getAttribute('data-ticket-slug');await listTarget.scrollIntoViewIfNeeded();const listBefore=await workspace.evaluate(node=>node.scrollTop);await listTarget.click({button:'right'});await page.getByRole('menu',{name:'Ticket actions'}).locator('[data-context-action="Verify ticket"]').click();await expect(page.locator(`[data-component="ticket-list-row"][data-ticket-slug="${listSlug}"]`)).toHaveAttribute('data-status','verified');expect(await workspace.evaluate(node=>node.scrollTop)).toBe(listBefore);
  await page.getByRole('button',{name:'Columns view'}).click();const columns=page.locator('.ticket-board-column__tickets');await expect(columns).toHaveCount(4);const before=await columns.evaluateAll(nodes=>Object.fromEntries(nodes.map(node=>{const element=node as HTMLElement;element.scrollTop=260;return[element.closest('[data-column-id]')!.getAttribute('data-column-id'),element.scrollTop]})) as Record<string,number>);const boardTarget=page.locator('[data-column-id="completed"] [data-component="ticket-list-row"]').nth(5),boardSlug=await boardTarget.getAttribute('data-ticket-slug');await boardTarget.click({button:'right'});await page.getByRole('menu',{name:'Ticket actions'}).locator('[data-context-action="Verify ticket"]').click();await expect(page.locator(`[data-column-id="verified"] [data-ticket-slug="${boardSlug}"]`)).toBeVisible();const after=await columns.evaluateAll(nodes=>Object.fromEntries(nodes.map(node=>[node.closest('[data-column-id]')!.getAttribute('data-column-id'),(node as HTMLElement).scrollTop])) as Record<string,number>);for(const [id,scrollTop] of Object.entries(before))expect(Math.abs(after[id]-scrollTop)).toBeLessThanOrEqual(1);await page.screenshot({path:'/private/tmp/hs2-cebnaj-board-scroll-retained.png',fullPage:true});
});
