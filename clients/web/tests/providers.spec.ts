...
test('streams ANSI terminal output, input, viewport leases, driver state, and reconnects without exposing credentials',async({page})=>{
  ...
  await page.evaluate(()=>{const sockets=(window as unknown as {__terminalSockets:Array<{sent:unknown[];emitSize(cols:number,rows:number,driver:string):void}>}).__terminalSockets.filter(socket=>socket.sent.some(value=>typeof value==='string'&&value.includes('viewer_id'))),driver=JSON.parse(sockets[0].sent.find(value=>typeof value==='string'&&value.includes('viewer_id')) as string).resize.viewer_id;sockets.at(-1)!.emitSize(300,120,driver)});await expect(magnified).toHaveAttribute('data-driving','false');await expect(magnified).toHaveAttribute('data-pty-size','300x120');await expect(magnified).toHaveAttribute('data-viewing-label','Viewing at 300×120 · focus to resize');expect(Number(await magnified.getAttribute('data-scale'))).toBeLessThan(.7);const contained=await magnified.evaluate(element=>{const viewport=element.getBoundingClientRect(),style=getComputedStyle(element),screen=element.querySelector<HTMLElement>('.xterm-screen')!.getBoundingClientRect();return screen.right<=viewport.right-parseFloat(style.paddingRight)+1&&screen.bottom<=viewport.bottom-parseFloat(style.paddingBottom)+1});expect(contained).toBe(true);await page.screenshot({path:'/private/tmp/hs2-pd4mz9-live-terminal.png',fullPage:true});
