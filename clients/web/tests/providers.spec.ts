...
test('uses independent width and height terminal dashboard zoom scales',async({page})=>{
  ...
  const liveTile=dashboard.locator('[data-terminal-key="demo-checkout:tests"]');await liveTile.dblclick();await expect(page.getByRole('button',{name:'Terminal dashboard'})).toHaveAttribute('aria-pressed','false');const drawer=page.locator('[data-component="terminal-drawer"]');await expect(drawer).toHaveAttribute('data-maximized','true');await expect(drawer).toHaveAttribute('data-mode','dedicated');await expect(drawer.locator('.xterm-helper-textarea')).toBeFocused();await expect.poll(()=>latestClaim('tests')).toMatchObject({cols:expect.any(Number),rows:expect.any(Number),focus:true});await expect(drawer.locator('[data-display-mode="interactive"]')).toHaveAttribute('data-driving','true');
...
test('opens, navigates, resizes, zooms, creates, hides, and restores the project terminal drawer',async({page})=>{
  ...
  await page.getByRole('button',{name:'Terminal dashboard'}).click();const globalTile=page.locator('[data-component="terminal-tile"][data-terminal-key="demo-checkout:codex-main"]');await globalTile.click({button:'right'});await page.getByRole('menu').getByText('Open').click();const drawer=page.locator('[data-component="terminal-drawer"]');await expect(drawer).toBeVisible();await expect(drawer).toHaveAttribute('data-mode','dedicated');await expect(drawer).toHaveAttribute('data-maximized','true');const selectedTerminalTab=drawer.locator('[data-component="terminal-tab"][data-selected="true"]');await expect(selectedTerminalTab.getByRole('tab')).toHaveAttribute('aria-selected','true');await expect(selectedTerminalTab).toContainText('Codex Main');const selectedSurfaces=await selectedTerminalTab.evaluate(element=>({root:getComputedStyle(element).backgroundColor,select:getComputedStyle(element.querySelector('.app-tab__select')!).backgroundColor}));expect(selectedSurfaces.root).not.toBe('rgba(0, 0, 0, 0)');expect(selectedSurfaces.select).toBe('rgba(0, 0, 0, 0)');
  const handle=page.getByRole('separator',{name:'Resize Terminal drawer'});await selectedTerminalTab.dispatchEvent('dblclick');await expect(drawer).toHaveAttribute('data-maximized','false');await expect(handle).toHaveAttribute('aria-valuenow','320');await handle.focus();await page.keyboard.press('ArrowUp');await expect(handle).toHaveAttribute('aria-valuenow','336');await expect(page.evaluate(()=>localStorage.getItem('hotsheet.layout.app-terminal-drawer.size'))).resolves.toBe('336');
