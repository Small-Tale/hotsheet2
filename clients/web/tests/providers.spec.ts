...
test('uses independent width and height terminal dashboard zoom scales',async({page})=>{
  ...
  const grouping=page.locator('wa-select[name="terminal-grouping"]');await grouping.click();await grouping.locator('wa-option[value="flow"]').click();await expect(grouping).toHaveJSProperty('value','flow');await expect(dashboard.locator('.terminal-dashboard__project')).toHaveCount(0);
