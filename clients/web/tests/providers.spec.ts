...
test('opens, navigates, resizes, zooms, creates, hides, and restores the project terminal drawer',async({page})=>{
  ...
  const projectTab=page.locator('[data-component="project-tab"]');await projectTab.click({button:'right',modifiers:['Alt']});const projectMenu=page.getByRole('menu',{name:'Project tab actions'});await expect(projectMenu.getByText('Close Tabs to the Left')).toBeVisible();await page.keyboard.press('Escape');
