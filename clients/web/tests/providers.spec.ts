...
test('browses repository files and commits with host-native actions',async({page})=>{
  ...
  await page.route('**/repository/**',route=>{const request=route.request();if(request.url().endsWith('/repository/status'))return failed?route.fulfill({status:500,body:'git unavailable'}):route.fulfill({json:response});actions.push({path:new URL(request.url()).pathname,...request.postDataJSON()});return route.fulfill({status:204})});
