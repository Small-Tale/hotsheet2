...
async function mockProject(page: import('@playwright/test').Page, canUpdate = true, primaryFeedbackNeeded = false, ticketLoadDelay = 0, batchResponseDelay = 0, patchResponseDelay = 0) {
  ...
  await page.route('**/*', async route => {
    ...
    if(path.includes('/tickets/')&&request.method()==='PATCH'){const id=path.split('/').pop(),body=request.postDataJSON();patches.push(body);rows=rows.map(item=>item.id===id?{...item,...body}:item);if(id==='01'){const label=(value:string)=>value.split('_').map(part=>part.charAt(0).toUpperCase()+part.slice(1)).join(' '),statusNote=typeof body.status==='string'&&body.status!==selectedFull.status?{id:`N-status-${patches.length}`,kind:'activity' as const,created_at:'2026-09-02T02:00:00Z',edited_at:'2026-09-02T02:00:00Z',text:`Status changed from ${label(selectedFull.status)} to ${label(body.status)}`}:undefined;selectedFull={...selectedFull,...body,notes:statusNote?[...selectedFull.notes,statusNote]:selectedFull.notes};if(patchResponseDelay)await new Promise(resolve=>setTimeout(resolve,patchResponseDelay));return route.fulfill({json:{store:'git-local',...selectedFull}})}const changed=rows.find(item=>item.id===id)!;return route.fulfill({json:{store:'git-local',...changed,details:'',notes:[],attachments:evidenceByTicket.get(id!)??[]}})}
...
test('leaves reader details editing on blur and close without leaking edit mode into the sidebar',async({page})=>{
  const patches=await mockProject(page,true,false,0,0,300);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();const row=page.locator('[data-component="ticket-list-row"][data-ticket-slug="HS2-DEMO01"]');await row.dblclick();let reader=page.getByRole('dialog',{name:'Read and edit HS2-DEMO01'}),preview=reader.getByRole('button',{name:'Edit Ticket details'});await preview.dblclick();let source=reader.getByRole('textbox',{name:'Ticket details'});await expect(source).toBeFocused();await source.blur();await expect(reader.getByRole('textbox',{name:'Ticket details'})).toHaveCount(0);await expect(preview).toContainText('The real ticket body.');
...
test('ships TicketRow context-menu behavior through real list and board compositions',async({page})=>{
  const patches=await mockProject(page);await page.goto('/');await page.getByRole('button',{name:'Open project'}).click();await page.getByRole('button',{name:'Open project',exact:true}).last().click();const first=page.locator('[data-component="ticket-list-row"][data-ticket-slug="HS2-DEMO01"]'),second=page.locator('[data-component="ticket-list-row"][data-ticket-slug="HS2-START02"]'),menu=page.getByRole('menu',{name:'Ticket actions'});await first.click();await second.click({modifiers:['Meta']});await first.click({button:'right'});await expect(menu).toBeVisible();await expect(page.locator('[data-component="ticket-list-row"][data-selected="true"]')).toHaveCount(2);await menu.getByText('Toggle Up Next').click();await expect.poll(()=>patches.filter(patch=>patch.up_next===false).length).toBe(2);
...
test('shows and resolves cross-project permission notifications with badges and history',async({page})=>{
  ...
  await popup.getByRole('button',{name:'Always Allow'}).click();await expect.poll(()=>answer).toEqual({decision:'allow',scope:'always'});await expect(popup).toHaveCount(0);await expect(page.getByText('allowed this kind of request')).toBeVisible();await expect(page.locator('[data-component="notification-center"]')).toBeVisible();
});
