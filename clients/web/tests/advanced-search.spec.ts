...
test('evaluates is: filters and boolean expressions in the workspace search',async({page})=>{
  ...
  await expect(page.locator('[data-component="global-search-overlay"]')).toHaveCount(0);await page.getByRole('button',{name:'Search tickets'}).click();const search=page.getByRole('textbox',{name:'Search tickets'}),toolbar=page.locator('.app-shell__main > .toolbar'),searchGroup=toolbar.locator('.workspace-header__search-group'),geometry=()=>page.evaluate(()=>{const rect=(selector:string)=>{const value=document.querySelector<HTMLElement>(selector)!.getBoundingClientRect();return{top:value.top,bottom:value.bottom,height:value.height}},group=document.querySelector<HTMLElement>('.app-shell__main > .toolbar .workspace-header__search-group')!;return{group:rect('.app-shell__main > .toolbar .workspace-header__search-group'),mode:rect('.app-shell__main > .toolbar .workspace-header__actions > .view-mode-switcher'),sort:rect('.app-shell__main > .toolbar .workspace-header__sort-group'),toolbar:rect('.app-shell__main > .toolbar'),tabs:rect('.app-shell__main > .project-tab-bar'),radius:getComputedStyle(group).borderRadius}});await searchGroup.evaluate(async node=>{await Promise.all(node.getAnimations().map(animation=>animation.finished))});const single=await geometry();expect(Number.parseFloat(single.radius)).toBeCloseTo(24.685,2);await page.screenshot({path:'/private/tmp/hs2-z4t3j0-single-line-search-wide.png',fullPage:true});await page.screenshot({path:'/private/tmp/hs2-0094cq-inline-search-wide.png',fullPage:true});
