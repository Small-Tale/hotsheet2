...
async function drawerOrder(drawer:import('@playwright/test').Locator){return drawer.locator('[data-tab-kind="terminal"], [data-tab-kind="ai-chat"]').evaluateAll(tabs=>tabs.map(tab=>(tab as HTMLElement).dataset.tabId))}
...
test('orders terminal and AI-chat tabs as one persistent, keyboard-accessible drawer sequence',async({page})=>{
  ...
  await page.screenshot({path:'/private/tmp/hs2-41q0ha-drawer-tab-bar-wide.png',fullPage:true});await page.setViewportSize({width:1100,height:720});await page.evaluate(()=>scrollTo(0,0));await expect.poll(()=>drawerOrder(drawer)).toEqual([chatId!,'shell-two','shell-one']);await page.screenshot({path:'/private/tmp/hs2-41q0ha-drawer-tab-bar-narrow.png',fullPage:true});
...
test('right-click closes mixed terminal and AI-chat ranges from either tab kind',async({page})=>{
  ...
  const chats=drawer.locator('[data-tab-kind="ai-chat"]'),chatOneId=(await chats.nth(0).getAttribute('data-tab-id'))!,chatTwoId=(await chats.nth(1).getAttribute('data-tab-id'))!;await dragBefore(page,chatOneId,'shell-two');await expect.poll(()=>drawerOrder(drawer)).toEqual(['shell-one',chatOneId,'shell-two',chatTwoId]);
