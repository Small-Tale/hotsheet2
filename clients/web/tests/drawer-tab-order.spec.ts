...
const project = {
  id: 'drawer-order',
  root: '/work/drawer-order',
  name: 'Drawer order',
  stores: ['/work/drawer-order.hs2'],
  apiPath: '/__hotsheet/project-api/drawer-order',
};
const capabilities = {
  create: true,
  update: true,
...
async function installFixture(page: import('@playwright/test').Page) {
    ...
    class FakeSocket extends EventTarget {
      ...
      constructor(public url: string) {
        ...
        setTimeout(() => {
          ...
          this.dispatchEvent(
            new MessageEvent('message', { data: new TextEncoder().encode('drawer order ready').buffer }),
          );
        });
      }
      send(value: unknown) {
  ...
  let terminals = [
    { id: 'shell-one', alive: true, busy: false, cwd: project.root },
    { id: 'shell-two', alive: true, busy: false, cwd: project.root },
  ];
  await page.route('**/*', (route) => {
        ...
        json: [
          {
            id: 'codex',
            display_name: 'Codex',
            models: [{ id: 'gpt-6-astra', label: 'GPT-6 Astra', effort_levels: ['medium'] }],
    ...
    if (path.endsWith('/ai-settings'))
      return route.fulfill({ json: { tool: 'codex', model: 'gpt-6-astra', effort: 'medium' } });
    ...
    const terminal = path.match(/\/terminals\/([^/]+)$/);
    if (terminal && request.method() === 'DELETE') {
      terminals = terminals.filter((item) => item.id !== decodeURIComponent(terminal[1]));
...
async function drawerOrder(drawer: import('@playwright/test').Locator) {
  return drawer
    .locator('[data-tab-kind="terminal"], [data-tab-kind="ai-chat"]')
    .evaluateAll((tabs) => tabs.map((tab) => (tab as HTMLElement).dataset.tabId));
}

async function dragBefore(page: import('@playwright/test').Page, sourceId: string, targetId: string) {
  await page.evaluate(
    ({ sourceId, targetId }) => {
      const source = document.querySelector<HTMLElement>(`[data-tab-id="${sourceId}"]`)!,
        target = document.querySelector<HTMLElement>(`[data-tab-id="${targetId}"]`)!,
...
test('orders terminal and AI-chat tabs as one persistent, keyboard-accessible drawer sequence', async ({ page }) => {
  ...
  await expect.poll(() => drawerOrder(drawer)).toEqual(['shell-one', 'shell-two', chatId!]);
  await dragBefore(page, chatId!, 'shell-one');
  await expect.poll(() => drawerOrder(drawer)).toEqual([chatId!, 'shell-one', 'shell-two']);
  const shellTwo = drawer.locator('[data-tab-id="shell-two"] .kui-app-tab__select');
  await shellTwo.press('Alt+Shift+ArrowLeft');
  await expect.poll(() => drawerOrder(drawer)).toEqual([chatId!, 'shell-two', 'shell-one']);
  await expect(shellTwo).toBeFocused();
  await expect(shellTwo).toHaveAttribute(
    'aria-keyshortcuts',
    'Delete Backspace Alt+Shift+ArrowLeft Alt+Shift+ArrowRight',
  );
  ...
  expect(
    await page.evaluate(
      (projectId) => localStorage.getItem(`hotsheet.project.${projectId}.terminal-drawer-order`),
      project.id,
    ),
  ...
  await expect(shellTwo).toBeFocused();
  await page.keyboard.press('Delete');
  await expect.poll(() => drawerOrder(drawer)).toEqual(['shell-one']);
  const shellOne = drawer.locator('[data-tab-id="shell-one"] .kui-app-tab__select');
  await expect(shellOne).toHaveAttribute('aria-selected', 'true');
  await expect(shellOne).toBeFocused();
});
...
test('right-click closes mixed terminal and AI-chat ranges from either tab kind', async ({ page }) => {
  ...
  const chats = drawer.locator('[data-tab-kind="ai-chat"]'),
    ...
    chatTwoId = (await chats.nth(1).getAttribute('data-tab-id'))!;
  await dragBefore(page, chatOneId, 'shell-two');
  await expect.poll(() => drawerOrder(drawer)).toEqual(['shell-one', chatOneId, 'shell-two', chatTwoId]);
  await drawer.locator(`[data-tab-id="${chatOneId}"]`).getByRole('tab').click({ button: 'right' });
  const chatMenu = page.getByRole('menu', { name: 'AI chat tab actions' });
  await expect(chatMenu).toBeVisible();
  await expect(chatMenu.getByText('Rename…')).toHaveCount(0);
  await chatMenu.getByText('Close Tabs to the Right').click();
  await expect.poll(() => drawerOrder(drawer)).toEqual(['shell-one', chatOneId]);
  await drawer.locator('[data-tab-id="shell-one"]').getByRole('tab').click({ button: 'right' });
