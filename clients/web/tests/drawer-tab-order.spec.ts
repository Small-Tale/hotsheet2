...
test('orders terminal and AI-chat tabs as one persistent, keyboard-accessible drawer sequence',async({page})=>{
  ...
  await page.screenshot({path:'/private/tmp/hs2-qr9n46-drawer-order-wide.png',fullPage:true});await page.setViewportSize({width:900,height:650});await expect.poll(()=>drawerOrder(drawer)).toEqual([chatId!,'shell-two','shell-one']);await page.screenshot({path:'/private/tmp/hs2-qr9n46-drawer-order-narrow.png',fullPage:true});
