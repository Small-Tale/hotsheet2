...
const row=(slug:string,title:string)=>({connection_id:'git-local',native_id:slug,qualified_id:`git-local:${slug}`,id:slug,slug,title,category:'feature',priority:'default',status:'started',up_next:false,feedback_needed:false,tags:[],blocked_by:[],claim_count:0,created_at:'2026-09-01T00:00:00Z',updated_at:'2026-09-14T00:00:00Z'});
...
test('keeps a search-selected ticket selected through a background refresh (HS2-6AXG6Z)',async({page})=>{
  ...
  await page.route('**/*',async route=>{
    ...
    if(path.endsWith('/tickets')&&request.method()==='GET'){const counts={total:1,queued:1,backlog:0,archive:0,open:1,up_next:0,active:1,started:1,completed_today:0,completion_trend:[0,0,0,0,0,0,0]};return url.searchParams.has('text')?route.fulfill({json:{items:[match],counts}}):route.fulfill({json:{items:base,counts:{...counts,total:2,queued:2,open:2,started:2}}})}
    if(/\/tickets\/HS2-MATCH$/.test(path))return route.fulfill({json:{store:'git-local',...match,details:'Investigate the parser edge case.',blocked_reason:null,notes:[],attachments:[],concurrency_token:'tok-match'}});
    ...
    if(path.endsWith('/ws/poll')){if(url.searchParams.get('since')===null)return route.fulfill({json:{cursor:1,events:[],overflow:false}});polls.push(route);return}
  ...
  await polls.shift()!.fulfill({json:{cursor:2,events:[{store:'git-local',kind:'changed',id:'HS2-BASE1',slug:'HS2-BASE1'}],overflow:false}});
