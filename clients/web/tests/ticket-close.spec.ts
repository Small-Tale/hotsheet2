...
test('marks a ticket as a duplicate of an exact ticket in another project and reopens it',async({page})=>{
  ...
  const inspector=page.locator('[data-component="ticket-inspector"][data-presentation="sidebar"]'),outcome=inspector.locator('[data-close-reason="duplicate"]');await expect(outcome).toContainText('Duplicate of HS2-1S6DS9 · Hot Sheet 2');await expect(outcome).not.toContainText('git-target:canonical');const staleDialog=page.locator('[data-component="not-working-dialog"]');if(await staleDialog.isVisible())await staleDialog.getByRole('button',{name:'Cancel'}).click();await page.setViewportSize({width:1080,height:900});await page.screenshot({path:'/private/tmp/hs2-95jesx-readable-duplicate-wide.png',fullPage:true});await outcome.getByRole('button',{name:'HS2-1S6DS9 · Hot Sheet 2'}).click();const reader=page.locator('[data-component="ticket-inspector"][data-presentation="reader"]');await expect(reader).toHaveAttribute('data-ticket-slug','HS2-1S6DS9');
