...
const targetVisible={...canonical,native_id:'visible',qualified_id:'git-target:visible',id:'visible',slug:'HS2-VISIBLE',title:'Visible target project ticket'};
...
test('marks a ticket as a duplicate of an exact ticket in another project and reopens it',async({page})=>{
  let sourceRows=[source,collision],closeBody:Record<string,unknown>|undefined,canonicalOpened=false;
  await page.route('**/*',async route=>{
    ...
    const target=path.includes('/target-project/'),rows=target?(canonicalOpened?[canonical]:[targetVisible]):sourceRows,provider=target?'git-target':'git-source';
    ...
    if(path.endsWith('/duplicate-backlinks'))return route.fulfill({json:target?{backlinks:[{reference:'@source-project/git-source:source',project_id:'source-project',project_name:'Kerf',connection_id:'git-source',native_id:'source',qualified_id:'git-source:source',slug:'KF-1AAYP9',title:'Repeated stability report'},{reference:'@mirror-project/git-mirror:source',project_id:'mirror-project',project_name:'Kerf mirror',connection_id:'git-mirror',native_id:'source',qualified_id:'git-mirror:source',slug:'KF-1AAYP9',title:'Repeated stability report mirror'}],inaccessible_projects:[]}:{backlinks:[],inaccessible_projects:[]}});
    ...
    if((path.endsWith('/tickets/git-target:canonical')||path.endsWith('/tickets/canonical'))&&request.method()==='GET'){canonicalOpened=true;return route.fulfill({json:{store:'git-target',...full(canonical)}})}
    if(path.endsWith('/tickets')&&request.method()==='GET'){
      const query=url.searchParams.get('text');
      const results=query?(target?[canonical]:[collision]):rows;
      return route.fulfill({json:url.searchParams.has('page_size')?{items:results,counts:{all:results.length,queue:results.length,backlog:0,archive:0}}:results});
    }
  ...
  const search=dialog.getByRole('textbox',{name:'Existing ticket'});await search.fill('HS2-1S6DS9');const candidate=dialog.getByRole('button',{name:/HS2-1S6DS9.*Hot Sheet 2.*Canonical checkout registry repair/});await expect(dialog.getByRole('button',{name:/HS2-1S6DS9.*Kerf.*Different local ticket/})).toBeVisible();await expect(candidate).toBeVisible();await expect(dialog.locator('.ticket-close-dialog__form')).toHaveCSS('gap','16px');await expect(dialog.locator('.ticket-close-dialog__results')).toHaveCSS('padding','4px');await page.screenshot({path:'/private/tmp/hs2-sbw2xd-cross-project-duplicate-wide.png',fullPage:true});
  ...
  const inspector=page.locator('[data-component="ticket-inspector"][data-presentation="sidebar"]'),outcome=inspector.locator('[data-component="ticket-duplicate-target"]');await expect(outcome).toContainText('Duplicate of');await expect(outcome).toContainText('Hot Sheet 2 · HS2-1S6DS9');await expect(outcome).toContainText('Canonical checkout registry repair');await expect(outcome).not.toContainText('git-target:canonical');const staleDialog=page.locator('[data-component="not-working-dialog"]');if(await staleDialog.isVisible())await staleDialog.getByRole('button',{name:'Cancel'}).click();await page.setViewportSize({width:1080,height:900});await page.screenshot({path:'/private/tmp/hs2-95jesx-duplicate-of-details-wide.png',fullPage:true});await page.setViewportSize({width:1024,height:700});await expect(outcome).toBeVisible();await page.screenshot({path:'/private/tmp/hs2-95jesx-duplicate-of-details-narrow.png',fullPage:true});await outcome.getByRole('button',{name:'Open duplicate target HS2-1S6DS9 from Hot Sheet 2'}).click();const reader=page.locator('[data-component="ticket-inspector"][data-presentation="reader"]');await expect(reader).toHaveAttribute('data-ticket-slug','HS2-1S6DS9');
  await reader.getByRole('button',{name:'Close ticket reader'}).click();await page.getByRole('tab',{name:'Hot Sheet 2'}).click();await expect(page.locator('[data-ticket-slug="HS2-1S6DS9"]')).toBeVisible();await page.locator('[data-ticket-slug="HS2-1S6DS9"]').click();const targetInspector=page.locator('[data-component="ticket-inspector"][data-presentation="sidebar"]'),backlinks=targetInspector.locator('[data-component="ticket-duplicate-backlinks"]'),count=backlinks.locator('.kui-list-header__count');await expect(backlinks.locator('h2')).toContainText('Duplicates');await expect(backlinks.locator('h2')).toHaveAttribute('aria-label','Duplicates, 2 duplicates');await expect(count).toHaveText('2');await expect(count).toHaveAttribute('aria-hidden','true');expect(await count.evaluate(element=>getComputedStyle(element).backgroundColor)).not.toBe('rgba(0, 0, 0, 0)');await expect(backlinks.getByRole('button',{name:'Open duplicate KF-1AAYP9 from Kerf',exact:true})).toBeVisible();await expect(backlinks.getByRole('button',{name:'Open duplicate KF-1AAYP9 from Kerf mirror',exact:true})).toBeVisible();await expect(backlinks).not.toContainText('Could not check');await page.waitForTimeout(800);await page.setViewportSize({width:1080,height:900});await page.screenshot({path:'/private/tmp/hs2-fg18en-stale-checkouts-ignored-wide.png',fullPage:true});await page.setViewportSize({width:1024,height:700});await expect(backlinks).toBeVisible();await page.screenshot({path:'/private/tmp/hs2-fg18en-stale-checkouts-ignored-narrow.png',fullPage:true});
});
