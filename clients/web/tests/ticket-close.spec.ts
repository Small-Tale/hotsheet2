...
const sourceProject = {
  ...
  root: '/work/source',
  name: 'Kerf',
  stores: ['/work/source.hs2'],
  apiPath: '/__hotsheet/project-api/source-project',
};
const targetProject = {
  id: 'target-project',
  root: '/work/target',
  name: 'Hot Sheet 2',
  stores: ['/work/target.hs2'],
  apiPath: '/__hotsheet/project-api/target-project',
};
const source = {
  connection_id: 'git-source',
  native_id: 'source',
  qualified_id: 'git-source:source',
  id: 'source',
  slug: 'KF-1AAYP9',
  title: 'Repeated stability report',
  category: 'bug',
  priority: 'default',
  status: 'started',
  up_next: false,
...
const canonical = {
  ...source,
  connection_id: 'git-target',
  native_id: 'canonical',
  qualified_id: 'git-target:canonical',
  id: 'canonical',
  slug: 'HS2-1S6DS9',
  title: 'Canonical checkout registry repair',
  status: 'completed',
};
const targetVisible = {
  ...canonical,
  native_id: 'visible',
  qualified_id: 'git-target:visible',
  id: 'visible',
  slug: 'HS2-VISIBLE',
  title: 'Visible target project ticket',
};
const collision = {
  ...source,
  native_id: 'collision',
  qualified_id: 'git-source:collision',
  id: 'collision',
  slug: 'HS2-1S6DS9',
  title: 'Different local ticket with the same slug',
};
const full = (ticket: typeof source, extra: Record<string, unknown> = {}) => ({
  ...ticket,
...
test('marks a ticket as a duplicate of an exact ticket in another project and reopens it', async ({ page }) => {
  let sourceRows = [source, collision],
    closeBody: Record<string, unknown> | undefined,
    canonicalOpened = false;
  await page.route('**/*', async (route) => {
    ...
    const target = path.includes('/target-project/'),
      rows = target ? (canonicalOpened ? [canonical] : [targetVisible]) : sourceRows,
      provider = target ? 'git-target' : 'git-source';
        ...
        json: [
          {
            ...
            provider: 'git',
            display_name: target ? 'Hot Sheet git' : 'Kerf git',
            locator: target ? '/work/target.hs2' : '/work/source.hs2',
            default: true,
            capabilities,
          },
        ],
    ...
    if (path.endsWith('/duplicate-backlinks'))
      return route.fulfill({
        json: target
          ? {
              backlinks: [
                {
                  reference: '@source-project/git-source:source',
                ...
                {
                  reference: '@mirror-project/git-mirror:source',
                  project_id: 'mirror-project',
                  project_name: 'Kerf mirror',
                  connection_id: 'git-mirror',
                  native_id: 'source',
                  qualified_id: 'git-mirror:source',
                  slug: 'KF-1AAYP9',
                  title: 'Repeated stability report mirror',
                },
              ],
    ...
    if (path.endsWith('/tickets/git-source:source/close') && request.method() === 'POST') {
      closeBody = request.postDataJSON();
      sourceRows = sourceRows.map((item) => ({ ...item, status: 'completed' }));
      return route.fulfill({
        json: {
          store: 'git-source',
          ...full(
            ...
            {
              closed_at: '2026-09-09T02:00:00Z',
              close_reason: 'duplicate',
              duplicate_of: '@target-project/git-target:canonical',
            },
          ),
        },
      });
    }
    if ((path.endsWith('/tickets/git-source:source') || path.endsWith('/tickets/source')) && request.method() === 'GET')
      return route.fulfill({ json: { store: 'git-source', ...full(sourceRows[0]) } });
    if (
      (path.endsWith('/tickets/git-target:canonical') || path.endsWith('/tickets/canonical')) &&
      request.method() === 'GET'
    ) {
      canonicalOpened = true;
      return route.fulfill({ json: { store: 'git-target', ...full(canonical) } });
    }
  ...
  await page.setViewportSize({ width: 1080, height: 1190 });
  ...
  await page.locator('[data-ticket-slug="KF-1AAYP9"]').click({ button: 'right' });
  ...
  await reason.click();
  await reason.locator('wa-option[value="duplicate"]').click();
  const search = dialog.getByRole('textbox', { name: 'Existing ticket' });
  await search.fill('HS2-1S6DS9');
  ...
  await expect(outcome).toContainText('Canonical checkout registry repair');
  await expect(outcome).not.toContainText('git-target:canonical');
  ...
  expect(await count.evaluate((element) => getComputedStyle(element).backgroundColor)).not.toBe('rgba(0, 0, 0, 0)');
  await expect(
    backlinks.getByRole('button', { name: 'Open duplicate KF-1AAYP9 from Kerf', exact: true }),
  ).toBeVisible();
  await expect(
    backlinks.getByRole('button', { name: 'Open duplicate KF-1AAYP9 from Kerf mirror', exact: true }),
  ).toBeVisible();
