...
const aiToolCatalog=[
  {id:'codex',display_name:'Codex',models:[{id:'gpt-6-astra',label:'GPT-6 Astra',effort_levels:['low','medium','high']}],default_model:'gpt-6-astra',default_effort:'medium',actions:['change_model','change_effort']},
  {id:'claude',display_name:'Claude',models:[{id:'sonnet',label:'Sonnet',effort_levels:['low','medium','high']}],default_model:'sonnet',default_effort:'medium',actions:['change_model','change_effort']},
];
...
const projects={
  '/work/alpha':{id:'proj-alpha',root:'/work/alpha',name:'Alpha',stores:['/work/alpha.hs2'],apiPath:'/__hotsheet/project-api/proj-alpha',needsTicketSetup:false,needsHs1Migration:false,defaultAi:{tool:'codex',model:'gpt-6-astra',effort:'medium'}},
  '/work/bravo':{id:'proj-bravo',root:'/work/bravo',name:'Bravo',stores:['/work/bravo.hs2'],apiPath:'/__hotsheet/project-api/proj-bravo',needsTicketSetup:false,needsHs1Migration:false,defaultAi:{tool:'claude',model:'sonnet',effort:'medium'}},
} as const;
...
const emptyCounts={total:0,queued:0,backlog:0,archive:0,trash:0,open:0,up_next:0,active:0,started:0,completed_today:0};
...
async function mockTwoProjects(page: import('@playwright/test').Page){
  ...
  await page.route('**/*',async route=>{
    ...
    if(path==='/__hotsheet/projects/open'&&request.method()==='POST'){
      const root=(request.postDataJSON() as {root:string}).root,{defaultAi,...project}=(projects as Record<string,MockProject>)[root]??projects['/work/alpha'];
      void defaultAi;return route.fulfill({status:201,json:project});
    }
    const api=path.match(/\/__hotsheet\/project-api\/([^/]+)(\/.*)$/);
    if(api){
      const project=byId.get(api[1]),rest=api[2];
      if(project){
        if(rest.endsWith('/providers'))return route.fulfill({json:[{connection_id:'git-local',provider:'git',display_name:'Hot Sheet git',locator:'/tickets',default:true,capabilities}]});
        ...
        if(rest.endsWith('/repository/status'))return route.fulfill({json:{branch:'main',ahead:0,behind:0,staged:0,unstaged:0,untracked:0,conflicted:0,clean:true}});
...
test('the Drive control reflects each project default AI on boot restore and tab switch',async({page})=>{
  ...
  await page.goto('/');
  const bravoTab=page.getByRole('tab',{name:/Bravo/}),alphaTab=page.getByRole('tab',{name:/Alpha/});
  await expect(bravoTab).toBeVisible();
  await expect(alphaTab).toBeVisible();
  const drive=page.locator('[data-component="project-sidebar"] [data-component="drive-control"] [data-action="toggle-drive"]');
  ...
  if(process.env.HS2_BECC2T_SHOT)await page.locator('[data-component="project-sidebar"] [data-component="drive-control"]').screenshot({path:process.env.HS2_BECC2T_SHOT});
