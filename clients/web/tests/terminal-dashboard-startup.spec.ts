...
test('reveals a restored terminal workspace atomically and separates All Projects',async({page})=>{
  ...
  const shell=page.locator('[data-component="app-shell"]');await expect(shell).toBeVisible({timeout:5_000});await expect(restoring).toHaveCount(0);const drawer=page.locator('[data-component="terminal-drawer"]');await expect(drawer).toBeVisible();await expect(drawer.locator('[data-tab-id="demo-shell"]')).toHaveCount(1);await expect(drawer.locator('[data-tab-id="other-shell"]')).toHaveCount(0);await page.getByRole('tab',{name:/other/}).click();await expect(drawer.locator('[data-tab-id="demo-shell"]')).toHaveCount(0);await expect(drawer.locator('[data-tab-id="other-shell"]')).toHaveCount(1);await page.getByRole('button',{name:'Workspace grid'}).click();const sidebar=page.locator('[data-component="terminal-operations-sidebar"]'),all=sidebar.locator('.terminal-operations-sidebar__group[data-project-id="all"]'),summaries=sidebar.locator('[data-component="project-summary"]'),dashboard=page.getByRole('region',{name:'Workspace grid'});await expect(dashboard.locator('[data-component="terminal-tile"]')).toHaveCount(2);await expect(dashboard.locator('[data-component="terminal-tile"]:has([data-project-id="demo-checkout"])')).toHaveCount(1);await expect(dashboard.locator('[data-component="terminal-tile"]:has([data-project-id="other-checkout"])')).toHaveCount(1);await expect(sidebar.locator('.terminal-operations-sidebar__group')).toHaveCount(3);await expect(all).toHaveCSS('border-bottom-width','1px');for(const summary of await summaries.all())await expect(summary).toHaveAttribute('data-chart-maximum','5');await expect(all.locator('[data-zero="false"]')).toHaveAttribute('style','--bar-height:100%');await expect(sidebar.locator('[data-project-id="demo-checkout"] [data-zero="false"]')).toHaveAttribute('style','--bar-height:40%');await expect(sidebar.locator('[data-project-id="other-checkout"] [data-zero="false"]')).toHaveAttribute('style','--bar-height:60%');await page.screenshot({path:'/private/tmp/hs2-thpfv8-terminal-project-scope-wide.png',fullPage:true});await page.setViewportSize({width:1024,height:650});await expect(all).toHaveCSS('border-bottom-width','1px');await page.screenshot({path:'/private/tmp/hs2-thpfv8-terminal-project-scope-narrow.png',fullPage:true});
