...
const project={id:'demo',root:'/work/demo',name:'Demo',stores:['/work/demo.hs2'],apiPath:'/__hotsheet/project-api/demo',needsTicketSetup:false,needsHs1Migration:false,hs1ImportCompleted:false,hs1CleanupEligible:false};
const listRow={connection_id:'git-local',native_id:'01',qualified_id:'git-local:01',id:'01',slug:'HS2-EDIT',title:'Rapid edit ticket',category:'feature',priority:'default',status:'started',up_next:false,feedback_needed:false,tags:[],blocked_by:[],claim_count:0,created_at:'2026-09-01T00:00:00Z',updated_at:'2026-09-14T00:00:00Z'};

test('rapid same-field edits are sequenced without a false conflict (HS2-K9SG2R)',async({page})=>{
  await page.setViewportSize({width:1280,height:800});
  let token='T0',tokenSeq=0,priority='default',firstPatchDelayed=false,conflicts=0;
  const full=()=>({store:'git-local',...listRow,priority,details:'',blocked_reason:null,notes:[],attachments:[],concurrency_token:token});
  await page.route('**/*',async route=>{
    ...
    if(/\/tickets\/01$/.test(path)&&method==='PATCH'){
      ...
      if(body.expected_token&&body.expected_token!==token){conflicts+=1;return route.fulfill({status:409,json:{error:'ticket changed since it was read'}})}
      if(body.priority!==undefined)priority=body.priority;tokenSeq+=1;token=`T${tokenSeq}`;
      const snapshot=full();
      if(!firstPatchDelayed){firstPatchDelayed=true;await new Promise(resolve=>setTimeout(resolve,400))}
      return route.fulfill({json:snapshot});
    }
    if(/\/tickets\/01$/.test(path)&&method==='GET')return route.fulfill({json:full()});
    if(path.endsWith('/tickets')&&method==='GET')return route.fulfill({json:{items:[{...listRow,priority}],counts:{total:1,queued:1,backlog:0,archive:0,open:1,up_next:0,active:1,started:1,completed_today:0,completion_trend:[0,0,0,0,0,0,0]}}});
    ...
    if(path.endsWith('/ws/poll')){if(url.searchParams.get('since')===null)return route.fulfill({json:{cursor:1,events:[],overflow:false}});return}
  ...
  await priority_.evaluate((node:HTMLElement&{value:string})=>{
    node.value='high';node.dispatchEvent(new Event('change',{bubbles:true}));
    node.value='low';node.dispatchEvent(new Event('change',{bubbles:true}));
  });
...
test('a genuine external same-field write still surfaces a conflict (HS2-K9SG2R)',async({page})=>{
  ...
  let presented=false;
  const remote=()=>({store:'git-local',...listRow,priority:'urgent',details:'',blocked_reason:null,notes:[],attachments:[],concurrency_token:'T1'});
  await page.route('**/*',async route=>{
    const request=route.request(),url=new URL(request.url()),path=url.pathname,method=request.method();
    if(path==='/__hotsheet/projects/open')return route.fulfill({status:201,json:project});
    if(path==='/__hotsheet/folders/choose')return route.fulfill({json:{path:'/work/demo'}});
    if(path.endsWith('/providers'))return route.fulfill({json:[{connection_id:'git-local',provider:'git',display_name:'Hot Sheet git',locator:'/tickets',default:true,capabilities}]});
    if(/\/tickets\/01$/.test(path)&&method==='PATCH'){const body=request.postDataJSON() as {expected_token?:string};if(body.expected_token!=='T1')return route.fulfill({status:409,json:{error:'ticket changed since it was read'}});return route.fulfill({json:remote()})}
    // First detail GET (selection) sees T0/default; later GETs (the conflict-path remote fetch) see T1/urgent.
    if(/\/tickets\/01$/.test(path)&&method==='GET'){if(!presented){presented=true;return route.fulfill({json:{store:'git-local',...listRow,priority:'default',details:'',blocked_reason:null,notes:[],attachments:[],concurrency_token:'T0'}})}return route.fulfill({json:remote()})}
    if(path.endsWith('/tickets')&&method==='GET')return route.fulfill({json:{items:[listRow],counts:{total:1,queued:1,backlog:0,archive:0,open:1,up_next:0,active:1,started:1,completed_today:0,completion_trend:[0,0,0,0,0,0,0]}}});
    if(path.endsWith('/repository/status'))return route.fulfill({json:{branch:'main',ahead:0,behind:0,staged:0,unstaged:0,untracked:0,conflicted:0,clean:true}});
    if(path.endsWith('/ws/poll')){if(url.searchParams.get('since')===null)return route.fulfill({json:{cursor:1,events:[],overflow:false}});return}
    if(path.endsWith('/terminals')||path.endsWith('/connections')||path.endsWith('/commands')||path.endsWith('/command-runs')||path.endsWith('/views')||path.endsWith('/corrupt-tickets'))return route.fulfill({json:[]});
    return route.continue();
  ...
  const priority_=page.locator('[name="inspector-priority"]');
  await expect(priority_).toBeVisible();
  await priority_.evaluate((node:HTMLElement&{value:string})=>{node.value='high';node.dispatchEvent(new Event('change',{bubbles:true}))});
