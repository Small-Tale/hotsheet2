...
const ticket={store:'git',...row,details:'',blocked_reason:null,concurrency_token:'token',notes:[{id:'N1',kind:'regular',created_at:'2026-09-09T00:03:00Z',edited_at:'2026-09-09T00:03:00Z',text:'First image: attachment:first.svg\n\nSecond image: attachment:second.svg'}],attachments};
...
test('each image in one note opens its own gallery attachment',async({page})=>{
  await page.route('**/*',async route=>{const request=route.request(),url=new URL(request.url()),path=url.pathname;
    ...
    if(path.endsWith('/tickets')&&request.method()==='GET')return route.fulfill({json:[row]});
