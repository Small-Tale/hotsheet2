...
const attachments = [
  { id: 'A-FIRST', filename: 'first.svg', created_at: '2026-09-09T00:01:00Z' },
  { id: 'A-SECOND', filename: 'second.svg', created_at: '2026-09-09T00:02:00Z' },
];
const ticket = {
  store: 'git',
  ...row,
  details: 'Local evidence: `attachment:first.svg`\n\nRelated ticket: `attachment:[HS2-OTHER]report.pdf`',
  blocked_reason: null,
  ...
  notes: [
    {
      ...
      edited_at: '2026-09-09T00:03:00Z',
      text: 'First image: attachment:first.svg\n\nSecond image: attachment:second.svg',
    },
  ],
  attachments,
};
...
test('each image in one note opens its own gallery attachment', async ({ page }) => {
  await page.route('**/*', async (route) => {
    ...
    if (media) {
      const label = media === 'A-FIRST' ? 'FIRST' : 'SECOND',
        color = media === 'A-FIRST' ? '#2d73da' : '#8754c8';
      return route.fulfill({
        contentType: 'image/svg+xml',
        body: `<svg xmlns="http://www.w3.org/2000/svg" width="640" height="360"><rect width="640" height="360" fill="${color}"/><text x="320" y="190" fill="white" font-size="64" text-anchor="middle">${label}</text></svg>`,
      });
    }
  ...
  const visibleInspector = page.locator('.ticket-inspector:visible'),
  ...
  const note = visibleInspector.locator('article[data-note-id="N1"]'),
    first = note.getByRole('button', { name: 'Open first.svg in image gallery' }),
    second = note.getByRole('button', { name: 'Open second.svg in image gallery' });
  await expect(first).toHaveAttribute('data-gallery-attachment-id', 'A-FIRST');
