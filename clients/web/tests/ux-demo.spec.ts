...
test('uses the identical responsive TicketRow in list and board compositions', async ({ page }) => {
  ...
  await expect(board.locator('.ticket-board__column').first()).toHaveCSS('background-color', 'rgba(0, 0, 0, 0)');
  await expect(board.locator('.ticket-board__column').first()).toHaveCSS('padding', '0px');
  ...
  const scrollRegions = board.locator('.ticket-board__tickets');
  ...
  const firstHeaderTop = await board.locator('.ticket-board__column-header').first().evaluate(node => node.getBoundingClientRect().top);
  ...
  expect(await board.locator('.ticket-board__column-header').first().evaluate(node => node.getBoundingClientRect().top)).toBeCloseTo(firstHeaderTop, 0);
...
test('exercises the application-shell component slice and responsive composition', async ({ page }) => {
  ...
  const tabActionCenters = await tabBar.evaluate(node => {
    ...
    const more = node.querySelector('[aria-label="More projects"] svg')!.getBoundingClientRect();
    return { tab: tab.y + tab.height / 2, add: add.y + add.height / 2, more: more.y + more.height / 2 };
  });
  expect(tabActionCenters.add).toBeCloseTo(tabActionCenters.tab, 0);
  expect(tabActionCenters.more).toBeCloseTo(tabActionCenters.tab, 0);
