...
test('renders standalone ticket metadata and inspector-section demos', async ({ page }) => {
  ...
  await menu.getByRole('menuitem',{name:'Copy reference'}).click();await expect(menu).toHaveCount(0);await page.locator('[data-component="ticket-attachments"]').screenshot({path:'/private/tmp/hs2-s0pscw-video-poster-grid.png'});
  await page.setViewportSize({ width: 390, height: 844 });
