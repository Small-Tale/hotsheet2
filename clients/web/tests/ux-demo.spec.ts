...
test('navigates the catalog and preserves URL-addressable selection', async ({ page }) => {
  ...
  await expect(page.getByRole('complementary', { name: 'Component catalog' }).getByRole('heading', { name: 'Uses' })).toBeVisible();
  await page.getByRole('complementary', { name: 'Component catalog' }).getByRole('link', { name: /TagChip/ }).last().click();
  await expect(page).toHaveURL('/ux-demo?component=tag-chip');
  await expect(page.getByRole('complementary', { name: 'Component catalog' }).getByRole('heading', { name: 'Used by' })).toBeVisible();
  await page.getByRole('complementary', { name: 'Component catalog' }).getByRole('link', { name: /TicketRow/ }).last().click();
  await page.goBack();
  await expect(page.getByRole('heading', { name: 'TagChip', exact: true })).toBeVisible();
});
