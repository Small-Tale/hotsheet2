...
test('renders standalone ticket metadata and inspector-section demos', async ({ page }) => {
  ...
  const item=page.locator('[data-attachment-id="demo-video"]'),trigger=item.getByRole('button',{name:'More actions for choppy.mov'});await expect(item.getByRole('button')).toHaveCount(1);await expect(trigger).toHaveAttribute('title','More actions for choppy.mov');await expect(trigger.locator('[data-lucide="more-horizontal"]')).toBeVisible();await trigger.click();let menu=page.getByRole('menu',{name:'Attachment actions'});await expect(menu.locator('[data-component="menu-item"]')).toHaveCount(5);await expect(menu.getByRole('menuitem').allTextContents()).resolves.toEqual(['Open','Download','Copy reference','Show in file manager','Remove']);
  ...
  await page.setViewportSize({ width: 390, height: 844 });
  await item.scrollIntoViewIfNeeded();await item.click({button:'right'});menu=page.getByRole('menu',{name:'Attachment actions'});await expect(menu).toBeVisible();await expect(menu.locator('[data-component="menu-item"]')).toHaveCount(5);await page.screenshot({ path: '/private/tmp/hs2-vqvf5b-attachment-menu-ux-narrow.png',fullPage:true });
});
