...
test('switches and searches the connected workspace through WorkspaceHeader', async ({ page }) => {
  ...
  const prioritySort = header.locator('wa-dropdown-item[data-sort="priority"]');
  await prioritySort.click();
  await expect(page.getByText('Sorted by priority, ascending')).toBeVisible();
  await expect(prioritySort).toHaveAttribute('aria-current', 'true');
  await expect(prioritySort).toHaveAttribute('aria-label', 'Priority, ascending');
  await expect(prioritySort.locator('[data-lucide="arrow-up"]')).toBeVisible();
  ...
  await expect(prioritySort).toHaveAttribute('aria-current', 'true');
  await expect(prioritySort).toHaveAttribute('aria-label', 'Priority, descending');
  await expect(prioritySort.locator('[data-lucide="arrow-down"]')).toBeVisible();
