...
test('catalogs both FixedAspectTerminalCard variants and their dashboard relationship',async({page})=>{
  await page.setViewportSize({width:1728,height:971});await page.goto('/ux-demo?component=fixed-aspect-terminal-card');const stage=page.getByRole('region',{name:'Fixed aspect terminal card variants'}),preview=stage.locator('[data-fixed-aspect-terminal-card="preview"]'),magnified=stage.locator('[data-fixed-aspect-terminal-card="magnified"]'),previewViewport=preview.locator('[data-display-mode="scaled-preview"]'),magnifiedViewport=magnified.locator('[data-display-mode="interactive"]');await expect(preview).toBeVisible();await expect(magnified).toBeVisible();for(const viewport of [previewViewport,magnifiedViewport]){await expect(viewport).toHaveAttribute('data-connection','connected');await expect(viewport).toHaveAttribute('data-grid-size','80x24');await expect(viewport).toHaveAttribute('data-renderer','dom');await expect(viewport.locator('.xterm-rows')).toContainText('GNU nano 8.4');await expect(viewport.locator('.xterm-rows')).toContainText('^X Exit')}await expect(preview).toHaveCSS('border-width','0px');await expect(magnified).toHaveCSS('border-width','0px');const sizing=await stage.evaluate(element=>{const preview=element.querySelector<HTMLElement>('[data-fixed-aspect-terminal-card="preview"]')!.getBoundingClientRect(),magnified=element.querySelector<HTMLElement>('[data-fixed-aspect-terminal-card="magnified"]')!.getBoundingClientRect(),row=element.querySelector<HTMLElement>('[data-fixed-aspect-terminal-card="preview"] .xterm-rows > div')!;return{previewWidth:preview.width,magnifiedWidth:magnified.width,font:getComputedStyle(row).fontFamily}});expect(sizing.previewWidth).toBeLessThanOrEqual(352);expect(sizing.magnifiedWidth).toBeGreaterThan(sizing.previewWidth);expect(sizing.font).toContain('ui-monospace');const relationships=page.locator('.demo-relationships');await expect(relationships.locator('.select__group')).toHaveAttribute('aria-label','Used by');await expect(relationships.locator('wa-option',{hasText:'TerminalDashboard'})).toHaveCount(1);await page.screenshot({path:'/private/tmp/hs2-kp0hq8-vw5ew4-rmsss4-fixed-terminal-card-after.png',fullPage:true});await page.setViewportSize({width:390,height:844});await expect(preview).toBeVisible();await expect(magnified).toBeVisible();await expect.poll(()=>stage.evaluate(node=>node.scrollWidth<=node.clientWidth)).toBe(true);await page.screenshot({path:'/private/tmp/hs2-kp0hq8-vw5ew4-rmsss4-fixed-terminal-card-narrow-after.png',fullPage:true});
});
...
test('renders standalone ticket metadata and inspector-section demos', async ({ page }) => {
  ...
  const gridVideo=page.locator('.ticket-attachments__image-grid video').first();await expect(gridVideo).toBeVisible();expect(await gridVideo.evaluate(node=>(node as HTMLVideoElement).paused)).toBe(true);expect(await gridVideo.evaluate(node=>(node as HTMLVideoElement).autoplay)).toBe(false);
  await menu.getByRole('menuitem',{name:'Copy reference'}).click();await expect(menu).toHaveCount(0);
  await page.setViewportSize({ width: 390, height: 844 });
...
test('navigates and zooms the standalone attachment gallery demo',async({page})=>{
  await page.setViewportSize({width:1280,height:900});await page.goto('/ux-demo?component=attachment-gallery');let gallery=page.locator('[data-component="attachment-gallery"]');await expect(gallery).toHaveAttribute('aria-label',/Image 1 of 3/);await expect(gallery.locator('[data-component="toolbar"]')).toBeVisible();const darkGroups=gallery.locator('[data-component="toolbar-control-group"]');await expect(darkGroups).toHaveCount(4);for(const group of await darkGroups.all()){await expect(group).toHaveAttribute('data-tone','dark');await expect(group).toHaveCSS('color','rgb(255, 255, 255)');await expect(group).not.toHaveCSS('border-color','rgb(209, 209, 214)')}await expect(gallery.locator('.attachment-gallery__filename')).toHaveCSS('color','rgb(255, 255, 255)');const image=gallery.locator('[data-gallery-image="true"]'),fitWidth=await image.evaluate(node=>(node as HTMLElement).getBoundingClientRect().width);await gallery.getByRole('button',{name:'Zoom in'}).click();expect(await image.evaluate(node=>(node as HTMLElement).getBoundingClientRect().width)).toBeGreaterThan(fitWidth);await gallery.getByRole('button',{name:'Next image'}).click();await expect(gallery).toHaveAttribute('aria-label',/Image 2 of 3/);await gallery.screenshot({path:'/private/tmp/hs2-ddpkts-gallery-dark-wide.png'});await gallery.locator('[data-component="toolbar"]').screenshot({path:'/private/tmp/hs2-ddpkts-gallery-dark-toolbar.png'});await page.setViewportSize({width:760,height:640});await gallery.screenshot({path:'/private/tmp/hs2-ddpkts-gallery-dark-narrow.png'});await gallery.getByRole('button',{name:'Next image'}).click();await expect(gallery).toHaveAttribute('aria-label',/Video 3 of 3: walkthrough.mp4/);const video=gallery.locator('video');await expect(video).toHaveAttribute('controls','');expect(await video.evaluate(node=>(node as HTMLVideoElement).paused)).toBe(true);await video.evaluate(async node=>{if(!(node instanceof HTMLVideoElement))throw new Error('Expected gallery video');const canvas=document.createElement('canvas');canvas.width=640;canvas.height=360;const context=canvas.getContext('2d')!;context.fillStyle='#7857a4';context.fillRect(0,0,640,360);context.fillStyle='white';context.font='32px sans-serif';context.fillText('Hot Sheet video evidence',132,190);const recorder=new MediaRecorder(canvas.captureStream(5),{mimeType:'video/webm'}),chunks:Blob[]=[];recorder.ondataavailable=event=>chunks.push(event.data);recorder.start();await new Promise(resolve=>setTimeout(resolve,250));const stopped=new Promise(resolve=>{recorder.onstop=resolve});recorder.stop();await stopped;node.src=URL.createObjectURL(new Blob(chunks,{type:'video/webm'}));await new Promise((resolve,reject)=>{node.onloadedmetadata=resolve;node.onerror=reject})});await expect(video).toHaveJSProperty('videoWidth',640);await video.hover();await gallery.screenshot({path:'/private/tmp/hs2-s0pscw-video-gallery-narrow.png'});await gallery.getByRole('button',{name:'Close video gallery'}).click();await expect(gallery).toHaveCount(0);await page.getByRole('button',{name:'Open gallery'}).click();gallery=page.locator('[data-component="attachment-gallery"]');await expect(gallery).toBeVisible();
});

test('previews point and range annotation controls for video evidence',async({page})=>{
  await page.setViewportSize({width:1280,height:900});await page.goto('/ux-demo?component=attachment-gallery');const gallery=page.locator('[data-component="attachment-gallery"]');await gallery.getByRole('button',{name:'Next image'}).click();await gallery.getByRole('button',{name:'Next image'}).click();await expect(gallery).toHaveAccessibleName(/Video 3 of 3/);await gallery.getByRole('button',{name:'Annotate media'}).click();await expect(gallery.getByRole('slider',{name:'Video position'})).toBeVisible();await expect(gallery.getByRole('button',{name:'Set annotation start to playhead'})).toBeEnabled();await expect(gallery.getByRole('button',{name:'Set annotation end to playhead'})).toBeEnabled();await expect(gallery.getByText('Transition is abrupt')).toBeVisible();await gallery.screenshot({path:'/private/tmp/hs2-jw17a4-video-annotations.png'});
});

test('represents animated SVG evidence with the shared timed annotation controls',async({page})=>{
  await page.route('**/ux-gallery-preview.svg?variant=narrow',route=>route.fulfill({contentType:'image/svg+xml',body:'<svg xmlns="http://www.w3.org/2000/svg" width="640" height="360"><rect width="640" height="360"><animate attributeName="fill" values="#333;#777;#333" dur="4s" repeatCount="indefinite"/></rect></svg>'}));await page.goto('/ux-demo?component=attachment-gallery');const gallery=page.locator('[data-component="attachment-gallery"]');await gallery.getByRole('button',{name:'Next image'}).click();await expect(gallery).toHaveAccessibleName(/Image 2 of 3/);await expect(gallery.getByRole('slider',{name:'Video position'})).toBeVisible();await gallery.getByRole('button',{name:'Annotate media'}).click();await expect(gallery.getByRole('button',{name:'Set annotation start to playhead'})).toBeEnabled();await gallery.screenshot({path:'/private/tmp/hs2-jw17a4-animated-svg-annotations.png'});
});
...
test('previews AIConversation public states at wide and narrow sizes',async({page})=>{
  ...
  await conversationHost.getByRole('button',{name:'Close conversation'}).click();await expect(dialog).toBeHidden();await page.locator('[data-action="toggle-settings"]').click();const scenario=page.locator('[data-settings="ai-conversation"] [name="scenario"]');for(const value of ['empty','permission','completed','usage-unpriced','failed','interrupted'] as const){await scenario.evaluate((node:HTMLElement&{value:string},next)=>{node.value=next;node.dispatchEvent(new Event('change',{bubbles:true}))},value);await expect(dialog).toBeVisible();if(value==='empty')await expect(conversationHost).toContainText('Start a conversation');if(value==='permission')await expect(conversationHost.locator('[data-component="permission-request-card"]')).toBeVisible();if(value==='completed'){await expect(conversationHost).toContainText('≈$0.04');await expect(conversationHost).toContainText('AI-generated · may contain errors')}if(value==='usage-unpriced')await expect(conversationHost).toContainText('Cost unavailable');if(value==='failed')await expect(conversationHost).toContainText('The tool turn failed.');if(value==='interrupted')await expect(conversationHost).toContainText('Stopped before the suite completed.');await conversationHost.getByRole('button',{name:'Close conversation'}).click();await expect(dialog).toBeHidden()}
