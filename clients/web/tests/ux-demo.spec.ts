...
test('represents every server-build details state with shared dialog geometry',async({page})=>{
  await page.setViewportSize({width:1728,height:971});await page.goto('/ux-demo?component=connection-details-dialog');const dialog=page.locator('[data-component="connection-details-dialog"]'),header=dialog.locator('[data-component="dialog-header"]');await expect(dialog).toBeVisible();await expect(dialog).toHaveAttribute('data-embedded','true');await expect(header).toBeVisible();await expect(header).toHaveCSS('border-bottom-width','0px');await expect(dialog.locator('[data-component="value-table"]')).toBeVisible();await expect(dialog.getByRole('button',{name:'Close'})).toHaveCount(0);
...
test('composes and operates the complete ProjectSidebar demo', async ({ page }) => {
  ...
  const viewActionAlignment = await sidebar.evaluate(node => {
    ...
    const item = node.querySelector<HTMLElement>('.view-navigation .kui-menu-item')!.getBoundingClientRect();
  ...
  const alignedRows = await sidebar.evaluate(node => ['.repository-summary .kui-menu-item', '.view-navigation .kui-menu-item', '.command-navigation .kui-menu-item'].map(selector => node.querySelector(selector)!).map(item => {
    const bounds = item.getBoundingClientRect();
    const icon = item.querySelector('.kui-menu-item__icon')!.getBoundingClientRect();
    const label = item.querySelector('.kui-menu-item__label')!.getBoundingClientRect();
...
test('exercises the application-shell component slice and responsive composition', async ({ page }) => {
  ...
  const shellHierarchy = await shell.evaluate(node => {
    ...
    const pageHeader = node.querySelector('.kui-page-header')!.getBoundingClientRect();
