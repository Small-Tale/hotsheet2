...
test('imports an HS1 project, then offers cleanup only after remote backup',async({page})=>{
  let imported=false,remote=false,deleted=false;
  await page.route('**/*',async route=>{
    ...
    if(path.endsWith('/providers'))return route.fulfill({json:imported?[{connection_id:'git-import',provider:'git',display_name:'Imported Hot Sheet 1 tickets',locator:'/work/legacy.hs2',default:true,capabilities:{create:true,update:true,notes:true,attachments:true,watch:true,query_fields:[]}}]:[]});
  ...
  await dialog.getByRole('button',{name:'Import project'}).click();await expect(page.locator('[data-ticket-source-setup-dialog]')).toHaveJSProperty('open',true);await expect(page.locator('.app-toast')).toContainText('Imported 27 tickets and 4 attachments');await page.getByRole('textbox',{name:'Remote URL'}).fill('git@example.com:team/legacy.hs2.git');await page.getByRole('button',{name:'Connect & push'}).click();expect(remote).toBe(true);
