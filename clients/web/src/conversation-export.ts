...
export type ConversationExportScope = { kind: 'all' } | { kind: 'range'; startMessageId: string; endMessageId: string };
...
export function selectedConversationFileReferences(
  messages: readonly ConversationMessage[],
  scope: ConversationExportScope,
  options: ConversationExportBundleOptions,
) {
  const files = new Map<string, NonNullable<ConversationMessage['files']>[number]>();
  for (const message of selectedConversationMessages(messages, scope))
    for (const file of message.files ?? [])
      if ((file.kind === 'attachment' && options.includeAttachments) || (file.kind === 'media' && options.includeMedia))
...
export async function conversationExportAssets(
  messages: readonly ConversationMessage[],
  draft: ConversationExportDraft,
  fetchFile: typeof fetch = fetch,
): Promise<ConversationExportAsset[]> {
  return Promise.all(
    selectedConversationFileReferences(messages, draft.scope, draft.bundle).map(async (file) => {
      const response = await fetchFile(file.url);
      if (!response.ok) throw new Error(`Could not read ${file.filename} for export.`);
      return {
        id: file.id,
...
export function conversationExportScopeAfterMessagePick(
  ...
  if (start < 0 || end < 0 || start !== end || picked === start)
    return { kind: 'range', startMessageId: messageId, endMessageId: messageId };
  return {
    kind: 'range',
...
export function conversationExportValidation(
  ...
  if (selected.length === 0)
    return messages.length === 0 ? 'There are no messages to save.' : 'Choose a valid message range.';
  if (!draft.destination?.selectionToken) return 'Choose where to save the conversation.';
  if (draft.writeMode === 'create' && draft.destination.existing)
    return 'Choose re-export or overwrite for the existing bundle.';
  if (draft.writeMode !== 'create' && !draft.destination.existing)
    return 'The selected destination does not contain an export to replace.';
...
export function conversationExportBundleEntries(
  options: ConversationExportBundleOptions,
): ConversationExportBundleEntry[] {
...
export function buildConversationExportRequest(
  ...
  return {
    ...
    reopen: {
      ...
      resumesOriginalSession: Boolean(
        source.sessionId && source.resumable !== false && selected.at(-1)?.id === lastSourceMessageId,
      ),
    },
  };
}
...
export function conversationTranscriptMarkdown(
  ...
  for (const message of messages) {
    ...
    if (message.usage)
      lines.push(
        `_Usage: ${message.usage.tokensIn} input tokens, ${message.usage.tokensOut} output tokens${message.usage.costUsd === undefined ? '' : `, $${message.usage.costUsd.toFixed(4)}`}_`,
        '',
      );
  }
  if (activity.length) {
