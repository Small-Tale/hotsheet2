...
export interface TerminalVisibilityGroup {
  id: string;
  name: string;
  hiddenKeys: string[];
}
...
export function initialTerminalVisibilityState(): TerminalVisibilityState {
  return {
    groups: [
      { id: DEFAULT_TERMINAL_VISIBILITY_GROUP_ID, name: DEFAULT_TERMINAL_VISIBILITY_GROUP_NAME, hiddenKeys: [] },
    ],
    activeByScope: {},
  };
}
  ...
  try {
    ...
    for (const item of value.groups) {
      ...
      const hiddenKeys = Array.isArray(candidate.hiddenKeys)
        ? [...new Set(candidate.hiddenKeys.filter((key): key is string => typeof key === 'string' && Boolean(key)))]
        : [];
      seen.add(candidate.id);
      groups.push({ id: candidate.id, name: candidate.name.trim(), hiddenKeys });
    }
    if (!groups.some((group) => group.id === DEFAULT_TERMINAL_VISIBILITY_GROUP_ID))
      groups.unshift({
        id: DEFAULT_TERMINAL_VISIBILITY_GROUP_ID,
        name: DEFAULT_TERMINAL_VISIBILITY_GROUP_NAME,
        hiddenKeys: [],
      });
    const activeByScope: Record<string, string> = {},
      valid = new Set(groups.map((group) => group.id));
    if (value.activeByScope && typeof value.activeByScope === 'object' && !Array.isArray(value.activeByScope))
      for (const [scope, id] of Object.entries(value.activeByScope))
        if (scope && typeof id === 'string' && id !== DEFAULT_TERMINAL_VISIBILITY_GROUP_ID && valid.has(id))
          activeByScope[scope] = id;
    return { groups, activeByScope };
  } catch {
    return initialTerminalVisibilityState();
  }
}
...
export function activeTerminalVisibilityGroup(state: TerminalVisibilityState, scope: string): TerminalVisibilityGroup {
  ...
  return (
    ...
    state.groups.find((group) => group.id === DEFAULT_TERMINAL_VISIBILITY_GROUP_ID) ??
    initialTerminalVisibilityState().groups[0]
  );
}

export function selectTerminalVisibilityGroup(
...
export function addTerminalVisibilityGroup(
  state: TerminalVisibilityState,
  id: string,
  name = 'New group',
): { state: TerminalVisibilityState; group: TerminalVisibilityGroup } {
...
export function renameTerminalVisibilityGroup(
  state: TerminalVisibilityState,
  id: string,
  name: string,
): TerminalVisibilityState {
  const trimmed = name.trim();
  if (!trimmed || id === DEFAULT_TERMINAL_VISIBILITY_GROUP_ID) return state;
...
export function removeTerminalVisibilityGroup(state: TerminalVisibilityState, id: string): TerminalVisibilityState {
  ...
  return {
    ...
    activeByScope: Object.fromEntries(Object.entries(state.activeByScope).filter(([, active]) => active !== id)),
  };
}

export function setTerminalVisibleInGroup(
  state: TerminalVisibilityState,
  groupId: string,
  key: string,
  visible: boolean,
  ...
  return {
    ...
    groups: state.groups.map((group) => {
      ...
      return {
        ...group,
        hiddenKeys: visible ? group.hiddenKeys.filter((item) => item !== key) : [...group.hiddenKeys, key],
      };
    }),
  };
}

export function setAllTerminalsVisibleInGroup(
  state: TerminalVisibilityState,
  groupId: string,
...
export function hideNewTerminalInNamedGroups(state: TerminalVisibilityState, key: string): TerminalVisibilityState {
  return {
    ...state,
    groups: state.groups.map((group) =>
      group.id === DEFAULT_TERMINAL_VISIBILITY_GROUP_ID || group.hiddenKeys.includes(key)
        ? group
