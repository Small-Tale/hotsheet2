...
describe('advanced search semantics',()=>{
  it('distinguishes working search from complete view-independent search',()=>{const rows=[row('HS2-ONE'),row('HS2-HIDDEN','archive')];expect(filterAdvancedSearchResults(rows,'words','working',[])).toEqual([rows[0]]);expect(filterAdvancedSearchResults(rows,'words','all',[])).toEqual(rows);expect(filterAdvancedSearchResults(rows,'HS2-HIDDEN','working',[])).toEqual(rows)});
