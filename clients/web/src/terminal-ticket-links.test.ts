...
function buffer(rows: Array<{ text: string; wrapped?: boolean }>, columns: number): Pick<IBuffer, 'getLine' | 'length'> {
  const lines = rows.map(({ text, wrapped }) => ({
...
describe('terminal ticket links', () => {
  ...
  it('rejects invalid near-matches and keeps distinct references on one row', () => {
    ...
    expect(terminalTicketLinksForBufferLine(active, 48, 1, vi.fn())?.map(link => link.text)).toEqual(['HS2-OK', 'HS2-NEXT']);
  });
