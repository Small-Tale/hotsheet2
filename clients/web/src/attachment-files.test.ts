...
describe('attachment file ingestion', () => {
  ...
  it('gives actionable macOS guidance instead of exposing a fetch TypeError', () => {
    expect(describeUnreadableAttachments(['floating-capture.png'])).toBe(
      '“floating-capture.png” has no readable content yet. A new macOS screen capture can be dragged before it has been written to disk. Wait for it to appear on the desktop, then add it again.',
    );
