...
export interface WorkspaceSortPreference {
  sort: WorkspaceSort;
...
function validatedSortPreference(
  value: unknown,
  fallback: WorkspaceSortPreference,
  allowedSorts: readonly WorkspaceSort[] = sorts,
): WorkspaceSortPreference {
...
function validatedCollapsedCommandGroups(value: unknown): Record<string, string[]> {
  ...
  return Object.fromEntries(
    Object.entries(value).flatMap(([projectId, groups]) => {
      if (!projectId.trim() || !Array.isArray(groups)) return [];
      return [
        [
          projectId,
          [...new Set(groups.flatMap((group) => (typeof group === 'string' && group.trim() ? [group.trim()] : [])))],
        ],
      ];
    }),
  );
...
export function toggleCollapsedCommandGroup(
  ...
  const current = state[projectId] ?? [],
    collapsed = current.includes(normalized) ? current.filter((item) => item !== normalized) : [...current, normalized];
  ...
  return {
    ...
    sorts: {
      ...
      board: validatedSortPreference(
        storedSorts?.board,
        legacy.sort === 'status' ? DEFAULT_WORKSPACE_PREFERENCES.sorts.board : legacy,
        sorts.filter((sort) => sort !== 'status'),
      ),
    },
    sidebarVisible:
      typeof record.sidebarVisible === 'boolean' ? record.sidebarVisible : DEFAULT_WORKSPACE_PREFERENCES.sidebarVisible,
    inspectorVisible:
      typeof record.inspectorVisible === 'boolean'
        ? record.inspectorVisible
        : DEFAULT_WORKSPACE_PREFERENCES.inspectorVisible,
    commandGroupExpanded:
      typeof record.commandGroupExpanded === 'boolean'
        ? record.commandGroupExpanded
        : DEFAULT_WORKSPACE_PREFERENCES.commandGroupExpanded,
