...
const auxiliaryClientCss = [resolve(sourceRoot, 'dev-review/dev-review.css'), resolve(sourceRoot, 'ux-demo/style.css')];
const clientCss = [...productionCss, ...auxiliaryClientCss];
const kerfSpacingMigratedComponents = [
  'ai-content-label.css',
  'ai-tool-settings.css',
  'app-error.css',
  'app-shell.css',
  'attachment-context-menu.css',
  'attachment-gallery.css',
  'bulk-ticket-dialog.css',
  'command-navigation.css',
  'command-run-dialog.css',
  'command-settings-editor.css',
  'connection-details-dialog.css',
  'conversation-export-dialog.css',
  'corrupt-ticket-row.css',
  'drive-options-menu.css',
  'flow-back-button.css',
  'hs1-migration.css',
  'keyboard-settings.css',
  'manual-model-dialog.css',
  'markdown-preview.css',
  'note-card.css',
  'not-working-dialog.css',
  'pending-attachment-picker.css',
  'quick-ticket-composer.css',
  'project-close-dialog.css',
  'project-dialog.css',
  'project-tab-bar.css',
  'repository-setup.css',
...
describe('shared client theme', () => {
  it('loads Kerf Web Awesome theming before product-only tokens in both entry points', () => {
    for (const entry of ['main.tsx', 'ux-demo/main.tsx']) {
      ...
      const expectedImport =
        entry === 'main.tsx' ? "import './hot-sheet-tokens.css';" : "import '../hot-sheet-tokens.css';";
      expect(source).toContain(expectedImport);
      expect(source.indexOf("import '@kerfjs/ui/webawesome.css';")).toBeLessThan(source.indexOf(expectedImport));
  ...
  it('keeps every client-owned stylesheet on the shared semantic color palette', () => {
    for (const path of productionCss) {
      ...
      expect(source, `${path} contains a color literal outside hot-sheet-tokens.css`).not.toMatch(
        /#[\da-f]{3,8}\b|(?:rgb|hsl)a?\([^)]*\)/i,
      );
    }
    for (const path of auxiliaryClientCss) {
      expect(css(path), `${path} contains a color literal outside hot-sheet-tokens.css`).not.toMatch(
        /#[\da-f]{3,8}\b|(?:rgb|hsl)a?\([^)]*\)/i,
      );
    }
    for (const token of [
    ...
    const themeLiterals = [...css(tokenPath).matchAll(/#[\da-f]{3,8}\b|(?:rgb|hsl)a?\([^)]*\)/gi)].map((match) =>
      match[0].toLocaleLowerCase(),
    );
    expect(
      new Set(themeLiterals).size,
      'hot-sheet-tokens.css repeats a palette literal instead of aliasing its semantic source',
  ...
  it('mirrors the kerf --kui-space-* spacing scale so components can author against it (HS2-4Y6SM9)', () => {
    ...
    for (const path of kerfSpacingMigratedComponents) {
      expect(css(path), `${path} regressed to Web Awesome spacing instead of the Kerf semantic scale`).not.toContain(
        '--wa-space-',
      );
    }
  });
  ...
  it('uses the Web Awesome typography scale instead of one-off font sizes', () => {
    ...
    for (const path of clientCss) {
      ...
      for (const match of source.matchAll(/font-size:\s*([^;}]+)/g)) {
        ...
        expect(value, `${path} contains arbitrary font-size ${match[1].trim()}`).toMatch(allowedSize);
      }
      expect(source, `${path} contains an arbitrary size in a font shorthand`).not.toMatch(
        /font:\s*(?:\d+\s+)?\d*\.?\d+(?:px|rem|em)\b/,
      );
    }
