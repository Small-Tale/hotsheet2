...
export class Api {
  ...
  closeCheckoutTicket=(checkout:string,id:string,reason:TicketCloseReason,duplicateOf?:string)=>this.request<FullTicket&{store:string}>(`/checkouts/${encodeURIComponent(checkout)}/tickets/${encodeURIComponent(id)}/close`,{method:'POST',body:JSON.stringify({reason,...(duplicateOf?{duplicate_of:duplicateOf}:{})})}).then(ticket=>({store:ticket.store,ticket}));
