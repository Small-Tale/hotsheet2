...
export interface TicketRow {connection_id:string;native_id:string;qualified_id:string;id:string;slug:string;title:string;category?:string;priority?:string;status?:string;up_next:boolean;feedback_needed:boolean;tags:string[];blocked_by:string[];blocked_reason?:string;claimed_by?:string;claim_lease_expires_at?:string;worker_label?:string;legacy_number?:string;claim_count:number;created_at?:string;updated_at?:string;completed_at?:string;closed_at?:string;close_reason?:TicketCloseReason;duplicate_of?:string;details?:string;notes?:Note[]}
export interface CheckoutTicketQuery {text?:string;status?:string;collection?:'queue'|'archive'|'trash';open?:boolean;up_next?:boolean;blocked?:boolean;close_reason?:TicketCloseReason;compact?:boolean;tags?:string;has_attachment?:boolean;has_media_annotation?:boolean;has_commit?:boolean;attachment?:string;created_after?:string;created_before?:string;updated_after?:string;updated_before?:string;completed_after?:string;completed_before?:string;verified_after?:string;verified_before?:string}
export interface CheckoutTicketCounts {total:number;queued:number;backlog:number;archive:number;trash?:number;open:number;up_next:number;active:number;started:number;verified?:number;completed_today:number;completion_trend?:number[]}
...
export class Api {
  ...
  restoreCheckoutTicket=(checkout:string,id:string)=>this.request<FullTicket&{store:string}>(`/checkouts/${encodeURIComponent(checkout)}/tickets/${encodeURIComponent(id)}/restore`,{method:'POST'}).then(ticket=>({store:ticket.store,ticket}));
  /** Permanently remove every Trash ticket from the checkout's git-backed stores. */
  emptyCheckoutTrash=(checkout:string)=>this.request<{purged:number;tickets:string[]}>(`/checkouts/${encodeURIComponent(checkout)}/trash/empty`,{method:'POST'});
