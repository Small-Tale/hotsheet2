...
const statusChange = (note: Note) =>
  note.kind === 'activity' && /^Status changed from\b/.test(note.summary ?? note.text);
...
export function attachmentUploadBatchId(
  ...
  const changedAfterUpload = notes.some(
    (note) => statusChange(note) && time(note.created_at) > time(latest.created_at),
  );
...
export function attachmentRoundNumbers(
  attachments: readonly Attachment[],
  notes: readonly Note[],
): Map<string, number> {
  ...
  const changes = notes
    .filter(statusChange)
    .map((note) => time(note.created_at))
    .sort((left, right) => left - right);
  const rounds = new Map<string, number>();
  let round = 1,
    previous = ordered[0] ? time(ordered[0].created_at) : 0;
  for (const attachment of ordered) {
    const current = time(attachment.created_at);
    if (rounds.size > 0 && changes.some((change) => change > previous && change <= current)) round += 1;
    rounds.set(attachment.id, round);
    previous = current;
  }
  return rounds;
}
