 ...
 */
export async function submitNotWorkingReport(
  submission: NotWorkingSubmission,
  workflow: NotWorkingWorkflow,
): Promise<void> {
  const note = submission.note.trim();
  if (!note && submission.files.length === 0)
    throw new NotWorkingSubmissionError('Describe what is wrong or add an attachment.');
  try {
