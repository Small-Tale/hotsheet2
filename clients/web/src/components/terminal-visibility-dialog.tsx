...
export function TerminalVisibilityDialog({ open, state, scope, groups,contextMenu }: TerminalVisibilityDialogProps) {
  ...
  return <wa-dialog data-terminal-visibility-dialog label="Manage Workspace Visibility" aria-label="Manage Workspace Visibility" open={open}>
    <section class="terminal-visibility-dialog">
      <div class="terminal-visibility-dialog__toolbar"><div class="terminal-visibility-dialog__tabs" role="tablist" aria-label="Terminal visibility groups">{state.groups.map(group=><AppTab id={group.id} name={group.name} selected={group.id===active.id} closable={false} className="terminal-visibility-dialog__tab" selectAction="select-terminal-visibility-tab" rootAttributes={{'data-visibility-group-id':group.id}}/>)}<button type="button" class="terminal-visibility-dialog__add" data-action="add-terminal-visibility-group" aria-label="Add visibility group" title="Add visibility group"><LucideIcon icon={Plus} name="plus"/></button></div></div>
      <div class="terminal-visibility-dialog__body">{visibleGroups.length===0?<p>No terminals are available.</p>:visibleGroups.map(group=><section class="terminal-visibility-dialog__project" data-key={group.projectId}><ListHeader label={group.projectName}/><div class="terminal-visibility-dialog__rows">{group.sessions.map(session=>{const key=`${session.projectId}:${session.id}`,visible=!hidden.has(key);return <ListItem className="terminal-visibility-dialog__row" action="toggle-terminal-visibility" itemId={key} accessibleLabel={`${visible?'Hide':'Show'} ${session.title??session.id}`} state={visible?'visible':'hidden'} icon={<LucideIcon icon={visible?Eye:EyeOff} name={visible?'eye':'eye-off'}/>} label={session.title??session.id} trailing={<span class="terminal-visibility-dialog__row-state">{visible?'Visible':'Hidden'}</span>}/>})}</div></section>)}</div>
