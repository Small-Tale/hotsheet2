...
describe('TerminalDrawer',()=>{
  it('renders a horizontally scrollable terminal strip with its create action immediately after it',()=>{const markup=render();expect(markup).toContain('data-mode="grid"');expect(markup).toContain('data-maximized="false"');expect(markup).toContain('aria-label="Terminal grid"');expect(markup).toContain('terminal-drawer__tabs');expect(markup).toMatch(/terminal-drawer__tabs[\s\S]*Terminal 2[\s\S]*terminal-drawer__create/);for(const action of ['select-drawer-terminal','create-project-terminal','toggle-terminal-drawer','toggle-terminal-drawer-maximize'])expect(markup).toContain(`data-action="${action}"`)});
