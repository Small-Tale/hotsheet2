...
describe('DriveOptionsMenu',()=>{
  it('renders plugin-discovered submenus plus editable model entry with a default reset',()=>{const markup=String(DriveOptionsMenu({tools,selection:{tool:'codex',model:'legacy model',effort:'high'},defaultSelection:{tool:'claude',model:'opus'}}));expect(markup).toContain('aria-label="Drive provider, model, and effort options"');expect(markup).toContain('type="checkbox" data-action="select-drive-default"');expect(markup).toContain('data-action="select-drive-tool" data-value="claude"');expect(markup).toContain('data-action="select-drive-model" data-value="gpt-5.6"');expect(markup).toContain('name="drive-model" value="legacy model"');expect(markup.match(/aria-current="true"/g)).toHaveLength(1);});
