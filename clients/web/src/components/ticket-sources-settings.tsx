  ...
  return (
    <div class="ticket-provider-settings" data-component="ticket-sources-settings">
      <section>
        ...
        <p>
          This checkout uses {stores.length} git ticket source{stores.length === 1 ? '' : 's'} and{' '}
          {providerConnections.length} external provider{providerConnections.length === 1 ? '' : 's'}.
        </p>
        {stores.map((store) => (
          <div class="ticket-provider-settings__source">
            <strong>Hot Sheet git</strong>
            <code>{store}</code>
          </div>
        ))}
