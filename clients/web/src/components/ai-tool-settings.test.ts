...
describe('AiToolSettings',()=>{
  ...
  it('responds to the settings pane width and gives model ids the widest column',()=>{const css=readFileSync(new URL('./ai-tool-settings.css',import.meta.url),'utf8');expect(css).not.toContain('--wa-space-');expect(css).toContain('container-type:inline-size');expect(css).toContain('.ai-tool-settings { display:grid; container-type:inline-size; gap:var(--kui-space-l); }');expect(css).toContain('minmax(remify(224px),1.6fr)');expect(css).toContain('gap:var(--kui-space-m)');expect(css).toContain('gap:var(--kui-space-xs)');expect(css).toContain('@container (max-width:remify(640px))')});
});
