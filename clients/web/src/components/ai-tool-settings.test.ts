...
describe('AiToolSettings', () => {
  it('renders plugin models, an ephemeral selected custom model, Other, and compatible efforts', () => {
    const markup = String(
      AiToolSettings({
        tools: [
          {
        ...
        selection: { tool: 'codex', model: 'legacy model' },
      }),
    );
    for (const name of ['ai-default-tool', 'ai-default-model', 'ai-default-effort'])
  ...
  it('does not carry a stale model across a replaced provider catalog', () => {
    ...
    expect(markup).toContain('name="ai-default-model" label="Model" value="new-default"');
  ...
  it('responds to the settings pane width and gives model ids the widest column', () => {
    ...
    expect(css).toContain('@container (max-width:remify(640px))');
  });
});
