...
function MetadataSubmenu({ field, label, icon, iconName, choices, selected }: { field: 'category' | 'priority' | 'status'; label: string; icon: IconNode; iconName: string; choices: readonly { value: string; label: string; icon: IconNode; iconName: string; color?: string; separatorBefore?: boolean }[]; selected?: string }) {
...
export function TicketRowContextMenu({ x, y, category, priority, status, upNextEligible = true, verifyAction = false, notWorkingAction = false, selectionCount = 1 }: TicketRowContextMenuProps) {
  ...
  return <div class="ticket-context-menu" role="menu" aria-label="Ticket actions" style={`left:${x}px;top:${y}px`}>
    <wa-dropdown open placement="bottom-start" distance={0}>
      ...
      <MetadataSubmenu field="status" label="Change status" icon={CircleDot} iconName="circle-dot" choices={TICKET_STATUS_CHOICES} selected={status} />
