...
export type TicketEmptyStateKind='loading'|'view-loading'|'view-error'|'project'|'view'|'searching'|'search';
...
export function ticketEmptyStateCopy({kind,viewLabel='this view',query=''}:TicketEmptyStateProps){
  ...
  if(kind==='view-loading')return{title:`Loading ${viewLabel}`,detail:'Fetching tickets for this view…',icon:undefined,iconName:'loading'};
  if(kind==='view-error')return{title:`Couldn’t load ${viewLabel}`,detail:'Try selecting this view again.',icon:CircleAlert,iconName:'circle-alert'};
...
export function TicketEmptyState(props:TicketEmptyStateProps){const copy=ticketEmptyStateCopy(props),busy=props.kind==='loading'||props.kind==='view-loading';return <EmptyState className={`ticket-empty-state ticket-empty-state--${props.kind}`} title={copy.title} detail={copy.detail} busy={busy} icon={copy.icon?<LucideIcon icon={copy.icon} name={copy.iconName}/>:undefined}/>}
