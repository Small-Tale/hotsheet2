...
import { Bot,ChevronRight, Grid2X2, MessageSquare, PanelBottomClose, Plus, SquareTerminal } from 'lucide';
...
export interface TerminalDrawerChatTab {id:string;name:string;tool:string;content:SafeHtml}
export interface TerminalDrawerProps {projectId:string;projectName:string;sessions:TerminalDashboardSession[];chatTabs?:TerminalDrawerChatTab[];width:number;height:number;fitAcross:number;fitHigh:number;selectedId:string;magnifiedKey?:string;loading?:boolean;message?:string;maximized?:boolean;createMenuOpen?:boolean}

export function TerminalDrawer({projectId,projectName,sessions,chatTabs=[],width,height,fitAcross,fitHigh,selectedId,magnifiedKey,loading=false,message='',maximized=false,createMenuOpen=false}:TerminalDrawerProps){
  const selected=sessions.some(session=>session.id===selectedId)||chatTabs.some(chat=>chat.id===selectedId)?selectedId:'grid',selectedSession=sessions.find(session=>session.id===selected),selectedChat=chatTabs.find(chat=>chat.id===selected),gridSize=terminalGridContentSize(width,height);
  return <section class="terminal-drawer" data-component="terminal-drawer" data-project-id={projectId} data-mode={selectedChat?'ai-chat':selected==='grid'?'grid':'dedicated'} data-maximized={String(maximized)} data-terminal-drawer-measure="true" aria-label={`${projectName} terminal drawer`}>
    <header class="terminal-drawer__rail" data-action="toggle-terminal-drawer-maximize" title={`Double-click to ${maximized?'restore':'maximize'} terminal drawer`}>
      <div class="terminal-drawer__views" role="tablist" aria-label="Terminal drawer views"><button type="button" role="tab" tabindex="0" aria-selected={String(selected==='grid')} data-action="select-drawer-item" data-item-id="grid" aria-label="Terminal grid"><LucideIcon icon={Grid2X2} name="grid-2x-2"/></button><div class="terminal-drawer__tabs">{sessions.map(session=><AppTab kind="terminal" id={session.id} name={session.title??session.id} selected={selected===session.id} leading={<LucideIcon icon={SquareTerminal} name="square-terminal"/>} trailing={session.busy?<i aria-label="Busy"></i>:undefined}/>)}{chatTabs.map(chat=><AppTab kind="ai-chat" id={chat.id} name={chat.name} selected={selected===chat.id} leading={<LucideIcon icon={MessageSquare} name="message-square"/>}/>)}</div></div>
      <div class="terminal-drawer__create-wrap"><button type="button" class="terminal-drawer__create" data-action="toggle-terminal-create-menu" aria-label="New drawer item" aria-expanded={String(createMenuOpen)} title="New shell or AI chat"><LucideIcon icon={Plus} name="plus"/></button>{createMenuOpen&&<div class="terminal-drawer__create-menu" role="menu" aria-label="New drawer item"><MenuHeader label="New"/><MenuItem role="menuitem" action="create-terminal-drawer-item" itemId="default-shell" icon={<LucideIcon icon={SquareTerminal} name="square-terminal"/>} label="Default shell"/><MenuItem role="menuitem" action="create-terminal-drawer-item" itemId="ai-shell" icon={<LucideIcon icon={Bot} name="bot"/>} label="AI shell" trailing={<LucideIcon icon={ChevronRight} name="chevron-right"/>}/><MenuItem role="menuitem" action="create-terminal-drawer-item" itemId="ai-chat" icon={<LucideIcon icon={MessageSquare} name="message-square"/>} label="AI chat" trailing={<LucideIcon icon={ChevronRight} name="chevron-right"/>}/></div>}</div>
