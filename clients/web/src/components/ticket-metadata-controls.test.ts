...
describe('ticket metadata controls and inspector panels', () => {
  it('renders colored category icons and semantic priority icons', () => {
    ...
    expect(status).toMatch(/<span[^>]*slot="start" class="kui-select__custom-selected"><span class="status-badge status-badge--completed/);
  ...
  it('renders inspector sections independently of the inspector shell', () => {
    ...
    expect(info.match(/data-component="list-header"/g)).toHaveLength(5);
    expect(info).toContain('<h2 class="kui-list-header__label">Category</h2>');
    expect(info).toContain('<h2 class="kui-list-header__label">Priority</h2>');
    expect(info).toContain('<h2 class="kui-list-header__label">Details</h2>');
    expect(info).toContain('<h2 class="kui-list-header__label">Tags</h2>');
    ...
    const deleted = String(TicketInfoPanel({ status: 'deleted', priority: 'default', category: 'issue', tags: [], details: '' }));
    ...
    expect(info).toContain('<h2 class="kui-list-header__label" aria-label="Notes, 0 notes">Notes</h2><span class="kui-list-header__count" aria-hidden="true">0</span>');
    ...
    expect(blocked).toContain('<h2 class="kui-list-header__label">Blocked reason</h2>');
    ...
    expect(blocked.match(/data-component="list-header"/g)).toHaveLength(6);
