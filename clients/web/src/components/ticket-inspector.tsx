...
import { Info, ListTree, PanelRightClose, Paperclip, Star, X } from 'lucide';
...
export function TicketInspector({ slug, title, status, priority, category, tags, details, detailsMode, detailsDirty, activeTab = 'info', upNext = false, timelineEntries, attachments, notes, blockedReason, providerName, updatedLabel, presentation = 'sidebar' }: TicketInspectorProps) {
  return <aside class={presentation === 'reader' ? 'ticket-inspector ticket-inspector--reader' : 'ticket-inspector'} data-component="ticket-inspector" data-presentation={presentation} aria-label={`${slug} inspector`}>
    <header class="ticket-inspector__header">
      <Toolbar divider={false} center={<ToolbarText text={slug} size="small" />} trailing={<ToolbarControlGroup appearance="borderless" label="Ticket actions"><button type="button" class={`ticket-inspector__star${upNext ? ' ticket-inspector__star--active' : ''}`} data-action="toggle-inspector-up-next" aria-label={upNext ? 'Remove from Up Next' : 'Add to Up Next'}><LucideIcon icon={Star} name="star" /></button><button type="button" data-action={presentation === 'reader' ? 'close-ticket-reader' : 'close-ticket-inspector'} aria-label={presentation === 'reader' ? 'Close ticket reader' : 'Hide inspector'}><LucideIcon icon={presentation === 'reader' ? X : PanelRightClose} name={presentation === 'reader' ? 'x' : 'panel-right-close'} /></button></ToolbarControlGroup>} />
      <h1>{title}</h1>
    </header>
    ...
    {activeTab === 'info' && <TicketInfoPanel status={status} priority={priority} category={category} tags={tags} details={details} detailsMode={detailsMode} detailsDirty={detailsDirty} readerPresentation={presentation === 'reader'} notes={notes} blockedReason={blockedReason} providerName={providerName} updatedLabel={updatedLabel} />}
