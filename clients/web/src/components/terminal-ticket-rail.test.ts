...
describe('TerminalTicketRail',()=>{
  ...
  it('keeps the rectangular primary mode switch on its own row, puts search last, and centers the ticket header independently',()=>{const css=readFileSync(new URL('./terminal-ticket-rail.css',import.meta.url),'utf8');expect(css).toMatch(/\.view-mode-switcher \{[^}]*border-radius:var\(--wa-border-radius-m\)[^}]*grid-column:1 \/ -1/);expect(css).toMatch(/workspace-header__utility-group \{[^}]*grid-column:2/);expect(css).toMatch(/workspace-header__search-group \{[^}]*grid-column:3/);expect(css).toMatch(/workspace-header__search-group\[data-expanded="true"\] \{[^}]*width:100%/);expect(css).not.toMatch(/ticket-inspector__header > \.toolbar \{[^}]*padding-left/);expect(css).toMatch(/ticket-inspector__header > \.toolbar \{[^}]*grid-template-columns:1fr auto 1fr/);expect(css).toMatch(/terminal-ticket-rail__back \{[^}]*width:2\.25rem[^}]*color:var\(--wa-color-brand-on-quiet\)/)});
});
