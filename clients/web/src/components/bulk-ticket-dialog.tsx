  ...
  if (state.kind === 'empty-trash')
    return (
      <wa-dialog open data-component="empty-trash-dialog" label="Empty Trash?">
        <p>
          Permanently remove {state.count} ticket{state.count === 1 ? '' : 's'} from this project’s active store. Git
          history will still contain the removed files.
        </p>
        {state.error && (
          <p class="bulk-ticket-dialog__error" role="alert">
            {state.error}
          </p>
        )}
