...
export function ProjectCloseDialog({state}:{state?:ProjectCloseDialogState}){
  ...
  return <wa-dialog class="project-close-dialog" data-component="project-close-dialog" data-project-id={state.projectId} data-has-resources={String(hasResources)} label={`Close ${state.projectName}?`} aria-describedby={hasResources?'project-close-dialog-summary':undefined} open>
    ...
    {hasResources&&<p class="project-close-dialog__consequences"><strong>Keep running</strong> closes only this tab. Terminals and AI chat tabs return when reopened. Received chat history and the latest durable provider session return after an app or server restart. <strong>Stop all</strong> ends every item, then closes the tab.</p>}
