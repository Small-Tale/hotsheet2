...
describe('application shell components', () => {
  ...
  it('draws one continuous focus outline around the ticket work area', () => {
    const css = readFileSync(new URL('./app-shell.css', import.meta.url), 'utf8');
    expect(css).toContainSource('.app-shell__work-area:focus, .app-shell__work-area:focus-within');
  ...
  it('lets the composer own the workspace top rhythm without removing spacing when absent', () => {
    ...
    expect(css).toMatchSource(
      /@media \(max-width: remify\(672px\)\)[\s\S]*\.app-shell__composer \{ padding: remify\(11\.2px\); \}/,
    );
  });
  it('projects every ProjectTab state without nesting actions', () => {
    ...
    const activeOnlyMarkup = String(
      ProjectTab({ id: 'working', name: 'Working', location: 'local', activeTicketCount: 1 }),
    );
    ...
    expect(projectTabCss).not.toMatch(
      /\.project-tab__work-count\s*\{[^}]*color:\s*var\(--wa-color-warning-on-normal\)/,
    );
    ...
    const cappedRingMarkup = String(
      ProjectTab({ id: 'capped', name: 'Capped', location: 'local', upNextCount: 7, activeTicketCount: 12 }),
    );
  ...
  it('renders a project Select instead of the tab strip on mobile, keeping the mode switcher and add action (HS2-4C5RM7)', () => {
    const markup = String(
      ProjectTabBar({
        mobile: true,
        tabs: [
          { id: 'one', name: 'One', location: 'local', selected: false },
          { id: 'two', name: 'Two', location: 'local', selected: true },
        ],
      }),
    );
  ...
  it('shares context-menu actions across project and terminal tabs with Option reversing direction', () => {
    ...
    for (const label of ['Close Tab', 'Close Other Tabs', 'Close Tabs to the Right', 'Close All Tabs'])
      expect(project).toContain(label);
    expect(project).toContain('aria-label="Project tab actions"');
    expect(project).toContain('data-action="project-tab-context-action"');
    expect(project).not.toContain('Rename…');
    expect(terminal).toContain('aria-label="Terminal tab actions"');
    expect(terminal).toContain('Rename…');
    expect(terminal).toContain('data-tab-action="rename"');
    expect(terminal).toContain('Close Tabs to the Left');
    expect(terminal).toContain('data-action="terminal-tab-context-action"');
    expect(chat).toContain('aria-label="AI chat tab actions"');
    expect(chat).toContain('data-chat-id="chat"');
    expect(chat).toContain('data-action="terminal-tab-context-action"');
  ...
  it('clamps and projects accessible splitters in both axes', () => {
    ...
    const markup = String(
      ResizableRegion({ id: 'left', label: 'Sidebar', size: 240, min: 180, max: 420, children: 'content' as never }),
    );
  ...
  it('composes the terminal drawer as a center-only vertical region with collapsed recovery', () => {
    ...
    const tall = String(
      AppShell({
        ...
        terminalDrawer: 'drawer' as never,
        terminalDrawerVisible: true,
        terminalDrawerSize: 700,
        terminalDrawerMax: 760,
        terminalDrawerTransitioning: true,
      }),
    );
  ...
  it('renders all connection semantics and only valid actions', () => {
    ...
    expect(String(ConnectionStateBanner({ state: 'revision-mismatch' }))).toContain(
      'Different server build is running',
    );
  });
  ...
  it('composes all top-level regions', () => {
    ...
    expect(collapsedMarkup.indexOf('aria-label="Show project sidebar"')).toBeLessThan(
      collapsedMarkup.indexOf('data-component="tab-bar"'),
    );
  ...
  it('walks select, close, add, resize, and post-resize transitions', () => {
    ...
    expect(projectTabs.value.map((tab) => [tab.id, tab.selected])).toEqual([
      ['one', false],
      ['two', true],
    ]);
    closeProjectTab('two');
