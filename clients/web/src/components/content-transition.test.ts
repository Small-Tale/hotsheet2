...
describe('ContentTransition',()=>{
  ...
  it('slots label and footer crossfades into dialog chrome',()=>{expect(String(ContentTransition({active:'b',style:'crossfade',region:'label',a:'Old' as never,b:'New' as never}))).toContain('slot="label"');expect(String(ContentTransition({active:'b',style:'crossfade',region:'footer',a:'Old' as never,b:'New' as never}))).toContain('slot="footer"')})
  it('pairs full-width incoming and outgoing animations and honors reduced motion',async()=>{const css=await readFile(new URL('./content-transition.css',import.meta.url),'utf8');expect(css).toMatch(/data-active-side="b"[^}]+data-side="a"[^}]+push-out-start/);expect(css).toMatch(/data-active-side="b"[^}]+data-side="b"[^}]+push-in-end/);expect(css).toMatch(/data-active-side="a"[^}]+data-side="a"[^}]+push-in-start/);expect(css).toMatch(/data-active-side="a"[^}]+data-side="b"[^}]+push-out-end/);expect(css).toMatch(/push-out-start[^]*translateX\(-100%\)/);expect(css).toMatch(/push-in-start[^]*translateX\(-100%\)/);expect(css).toMatch(/prefers-reduced-motion:reduce[^}]+animation-duration:1ms/s)})
});
