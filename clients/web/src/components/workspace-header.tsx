      ...
      <ToolbarControlGroup className="workspace-header__search-group" expanded={searchOpen} single>
        {searchOpen
          ? <>
            ...
            {searchHelpOpen&&<aside class="workspace-header__search-help" role="dialog" aria-label="Search syntax"><strong>Search syntax</strong><p>Mix ordinary words with any filters below:</p><code>tag:client</code><code>tag:&quot;needs design&quot;</code><code>has:attachment</code><code>has:media-annotation</code><code>has:commit</code><code>attachment:*.png</code><code>is:up-next</code><code>is:active</code><code>is:open</code><code>is:not-started</code><code>is:started</code><code>is:completed</code><code>is:verified</code><code>is:backlog</code><code>is:archived</code><code>(client OR server) AND NOT is:archived</code><code>{`created-after:${localSearchDateExample}`}</code><code>{`completed-before:${localSearchDateTimeExample}`}</code><code>updated-after:4h ago</code><code>updated-after:2026-09-01T11:05</code><p>Boolean operators are case-insensitive. Parentheses control grouping; NOT binds before AND, and AND before OR. Dates and times use this device's locale. Relative minutes, hours, days, and weeks use the search time as their basis. ISO 8601 dates and date-times are always accepted. Date fields: created, completed, started, verified, archived, and updated. Use <code>-before</code> or <code>-after</code>.</p></aside>}
          </>
