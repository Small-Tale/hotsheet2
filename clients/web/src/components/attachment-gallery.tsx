...
export const attachmentGalleryAnnotationTolerance=(durationMs:number)=>Math.max(1000,Math.max(0,durationMs)*.01);
export function attachmentGalleryAnnotationVisible(annotation:MediaAnnotation,playheadMs:number,durationMs:number){if(annotation.start_ms===undefined)return true;const start=Math.min(annotation.start_ms,annotation.end_ms??annotation.start_ms),end=Math.max(annotation.start_ms,annotation.end_ms??annotation.start_ms),tolerance=attachmentGalleryAnnotationTolerance(durationMs);return playheadMs>=start-tolerance&&playheadMs<=end+tolerance}
export function attachmentGalleryDefaultRange(playheadMs:number,durationMs:number){const duration=Math.max(0,durationMs),playhead=Math.max(0,Math.min(duration,playheadMs)),radius=duration*.05;return{start_ms:Math.round(Math.max(0,playhead-radius)),end_ms:Math.round(Math.min(duration,playhead+radius))}}
...
export function AttachmentGallery({images,activeUrl,geometry={naturalWidth:0,naturalHeight:0,availableWidth:0,availableHeight:0},selectedScale,annotations=[],markup=false,selectedAnnotation,drawMode=false,playheadMs=0,durationMs=0,playing=false,volume=1,muted=false,volumeOpen=false,annotationEnabled=true,overlay}:{images:readonly AttachmentGalleryImage[];activeUrl:string;geometry?:AttachmentGalleryGeometry;selectedScale?:number;annotations?:readonly MediaAnnotation[];markup?:boolean;selectedAnnotation?:string;drawMode?:boolean;playheadMs?:number;durationMs?:number;playing?:boolean;volume?:number;muted?:boolean;volumeOpen?:boolean;annotationEnabled?:boolean;overlay?:SafeHtml}) {
  ...
  return <dialog class="attachment-gallery" data-component="attachment-gallery" aria-label={`${video?'Video':'Image'} ${index+1} of ${images.length}: ${image.name}`}>
