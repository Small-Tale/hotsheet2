...
import {ChevronLeft,ChevronRight,Eraser,Minus,MoreHorizontal,Pause,Pencil,Play,Plus,Scan,X} from 'lucide';
...
export function AttachmentGallery({images,activeUrl,geometry={naturalWidth:0,naturalHeight:0,availableWidth:0,availableHeight:0},selectedScale,annotations=[],markup=false,selectedAnnotation,drawMode=false,playheadMs=0,durationMs=0,playing=false,annotationEnabled=true}:{images:readonly AttachmentGalleryImage[];activeUrl:string;geometry?:AttachmentGalleryGeometry;selectedScale?:number;annotations?:readonly MediaAnnotation[];markup?:boolean;selectedAnnotation?:string;drawMode?:boolean;playheadMs?:number;durationMs?:number;playing?:boolean;annotationEnabled?:boolean}) {
  ...
  return <div class="attachment-gallery" data-component="attachment-gallery" role="dialog" aria-modal="true" aria-label={`${video?'Video':'Image'} ${index+1} of ${images.length}: ${image.name}`}>
    <Toolbar className="attachment-gallery__toolbar" divider={false} leading={<ToolbarText className="attachment-gallery__filename" text={image.name}/>} trailing={<>
      ...
      <ToolbarControlGroup label="Media actions" tone="dark"><GalleryButton action="toggle-gallery-markup" label={markup?'Finish markup':'Annotate media'} icon={Pencil} disabled={!annotationEnabled} className={markup?'attachment-gallery__pressed':''}/><GalleryButton action="open-gallery-attachment-menu" label={`More ${noun} actions`} icon={MoreHorizontal}/></ToolbarControlGroup>
    ...
    <div class="attachment-gallery__stage" data-gallery-zoom-stage="true"><div class="attachment-gallery__canvas"><div class="attachment-gallery__media-wrap" data-gallery-annotation-surface="true" data-draw-mode={String(drawMode)} style={validDimension(geometry.naturalWidth)&&validDimension(geometry.naturalHeight)?`width:${geometry.naturalWidth*zoom.scale}px;height:${geometry.naturalHeight*zoom.scale}px`:undefined}>{video?<video {...imageData} data-gallery-media="true" src={image.url} aria-label={image.name} controls playsInline preload="metadata"/>:<img {...imageData} data-gallery-media="true" data-gallery-image="true" src={image.url} alt={image.name}/>} {markup&&<div class="attachment-gallery__annotations">{annotations.filter(annotation=>annotationVisible(annotation,playheadMs)).map((annotation,annotationIndex)=><button type="button" class="attachment-gallery__annotation" data-action="select-gallery-annotation" data-annotation-id={annotation.id} data-selected={String(annotation.id===selectedAnnotation)} style={annotationStyle(annotation)} aria-label={`Annotation ${annotationIndex+1}${annotation.text?`: ${annotation.text}`:''}`}><span class="attachment-gallery__annotation-label" data-action="edit-gallery-annotation" data-annotation-id={annotation.id}>{annotation.text||String(annotationIndex+1)}</span>{annotation.id===selectedAnnotation&&['nw','n','ne','e','se','s','sw','w'].map(handle=><i data-annotation-handle={handle}/>)}</button>)}</div>}</div></div></div>
    {timed&&<div class="attachment-gallery__timeline"><button type="button" data-action="toggle-gallery-playback" aria-label={playing?'Pause':'Play'}><LucideIcon icon={playing?Pause:Play} name={playing?'pause':'play'}/></button><span>{formatTime(playheadMs)}</span><div class="attachment-gallery__timeline-track">{annotations.filter(annotation=>annotation.start_ms!==undefined).map(annotation=><i data-selected={String(annotation.id===selectedAnnotation)} style={`left:${durationMs?annotation.start_ms!*100/durationMs:0}%;width:${durationMs?Math.max(.35,(annotation.end_ms!-annotation.start_ms!)*100/durationMs):.35}%`}/>)}<input type="range" name="gallery-playhead" min="0" max={String(Math.max(1,durationMs))} value={String(playheadMs)} aria-label="Video position"/></div><span>{formatTime(durationMs)}</span></div>}
    {markup&&<ToolbarControlGroup className="attachment-gallery__markup" label="Media markup" tone="dark"><GalleryButton action="toggle-gallery-draw" label="Add rectangle" icon={Scan} className={drawMode?'attachment-gallery__pressed':''}/>{timed?<><GalleryButton action="set-gallery-range-start" label="Set annotation start to playhead" icon={ChevronLeft} disabled={!selectedAnnotation}/><GalleryButton action="set-gallery-range-end" label="Set annotation end to playhead" icon={ChevronRight} disabled={!selectedAnnotation}/></>:<></>}<GalleryButton action="delete-gallery-annotation" label="Erase selected annotation" icon={Eraser} disabled={!selectedAnnotation}/></ToolbarControlGroup>}
