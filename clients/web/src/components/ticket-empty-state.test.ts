...
describe('TicketEmptyState',()=>{
  it('distinguishes loading, a new project, an empty view, pending search, and no matches',()=>{
    ...
    expect(ticketEmptyStateCopy({kind:'view-loading',viewLabel:'Archive'})).toMatchObject({title:'Loading Archive',detail:'Fetching tickets for this view…'});
    expect(ticketEmptyStateCopy({kind:'view-error',viewLabel:'Archive'})).toMatchObject({title:'Couldn’t load Archive',detail:'Try selecting this view again.'});
