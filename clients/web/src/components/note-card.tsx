...
import { Activity, BookOpen, CircleAlert, MessageSquareText, Pencil, RefreshCw } from 'lucide';
import { LucideIcon } from './lucide-icon';
import './note-card.css';

export type NoteKind = 'regular' | 'status' | 'feedback_needed' | 'activity';
export interface NoteCardProps { id: string; kind: NoteKind; author: string; time: string; body: string; title?: string; editing?: boolean; draft?: string; readerAvailable?: boolean }
...
export function NoteCard({ id, kind, author, time, body, title, editing = false, draft = body, readerAvailable = false }: NoteCardProps) {
  ...
  return <article class={`note-card${editing ? ' note-card--editing' : ''}`} data-component="note-card" data-note-id={id} data-kind={kind} data-edit-on-double-click={editing ? undefined : 'true'} title={editing ? undefined : 'Double-click to edit'}>
    <header class="note-card__header">
      ...
      <span class="note-card__header-end"><span class="note-card__actions"><button type="button" data-action="edit-note" data-note-id={id} aria-label="Edit note"><LucideIcon icon={Pencil} name="pencil" /></button>{readerAvailable && <button type="button" data-action="open-ticket-reader" aria-label="Open ticket reader from note"><LucideIcon icon={BookOpen} name="book-open" /></button>}</span><time>{time}</time></span>
    </header>
