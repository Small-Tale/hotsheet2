...
import { Activity, CircleAlert, FilePenLine, MessageSquareText, Pencil, RefreshCw } from 'lucide';

import { LucideIcon } from './lucide-icon';

export type NoteKind = 'regular' | 'status' | 'feedback_needed' | 'feedback_draft' | 'activity';
export interface NoteCardProps { id: string; kind: NoteKind; author: string; time: string; body: string; title?: string; editing?: boolean; draft?: string; readerMode?: boolean; readerEditing?: boolean }
...
export function NoteCard({ id, kind, author, time, body, title, editing = false, draft, readerMode = false, readerEditing = false }: NoteCardProps) {
  ...
  const editable = !readerMode || readerEditing;
  ...
  return <article class={`note-card${editorOpen ? ' note-card--editing' : ''}`} data-component="note-card" data-note-id={id} data-kind={kind} data-edit-on-double-click={editable && !editorOpen ? 'true' : undefined} title={editable && !editorOpen ? 'Double-click to edit' : undefined}>
    <header class="note-card__header">
      ...
      <span class="note-card__header-end">{editable && !editorOpen && <span class="note-card__actions"><button type="button" data-action="edit-note" data-note-id={id} aria-label="Edit note"><LucideIcon icon={Pencil} name="pencil" /></button></span>}<time>{time}</time></span>
    </header>
    {kind === 'feedback_needed' && feedbackEditor && <p class="note-card__body">{body}</p>}
    {editorOpen ? <div class="note-card__editor"><textarea name="note-body" data-note-id={id} data-note-response={kind === 'feedback_needed' ? 'true' : undefined} aria-label={kind === 'feedback_needed' ? 'Feedback response' : 'Note body'}>{source}</textarea><div>{!feedbackEditor && <wa-button appearance="plain" data-action="cancel-note-edit" data-note-id={id}>Cancel</wa-button>}<wa-button appearance="accent" data-action="save-note-edit" data-note-id={id} data-note-response={kind === 'feedback_needed' ? 'true' : undefined}>{kind === 'feedback_needed' ? 'Respond' : kind === 'feedback_draft' ? 'Submit' : 'Save'}</wa-button></div></div> : <p class="note-card__body">{body}</p>}
