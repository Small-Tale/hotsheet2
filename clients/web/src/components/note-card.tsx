...
function FeedbackBlocks({ source, sourceStart, noteId, replies, attachmentContext }: { source: string; sourceStart: number; noteId: string; replies: readonly InlineFeedbackReply[]; attachmentContext?: AttachmentReferenceContext }) {
  if (!source) return null;
  const localReplies = replies.filter(reply => reply.offset >= sourceStart && reply.offset <= sourceStart + source.length).map(reply => ({ ...reply, offset: reply.offset - sourceStart }));
  return <>{splitFeedbackPrompt(source, localReplies).map(segment => <div class="note-card__feedback-section">{segment.markdown && <div class="note-card__feedback-block" data-action="add-inline-feedback-reply" data-note-id={noteId} data-segment-start={sourceStart + segment.start} data-segment-end={sourceStart + segment.end} role="button" tabIndex={0} aria-label="Add response at a character position"><MarkdownPreview source={segment.markdown} attachmentContext={attachmentContext} /></div>}{segment.reply && <div class="note-card__inline-reply-row"><textarea class="note-card__inline-reply" name="inline-feedback-response" data-note-id={noteId} data-offset={sourceStart + segment.reply.offset} aria-label={`Response at character ${sourceStart + segment.reply.offset}`}>{segment.reply.text}</textarea><button type="button" data-action="remove-inline-feedback-reply" data-note-id={noteId} data-offset={sourceStart + segment.reply.offset} aria-label={`Remove response at character ${sourceStart + segment.reply.offset}`}><LucideIcon icon={X} name="x" /></button></div>}</div>)}</>;
}
