...
export interface SelectChoice<Value extends string = string> { value: Value; label: string; icon?: IconNode; iconName?: string; color?: string; group?: string; separatorBefore?: boolean; action?:string }
...
export function Select<Value extends string>({ name, value, label, ariaLabel, choices, className = '', placeholder, disabled = false,fitMenu=false, renderSelected }: SelectProps<Value>) {
  ...
  const option = (choice: SelectChoice<Value>) => <>{choice.separatorBefore && <wa-divider></wa-divider>}<wa-option value={choice.value} data-action={choice.action}>{icon(choice)}{choice.label}</wa-option></>;
