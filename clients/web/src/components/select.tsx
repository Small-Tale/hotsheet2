...
export interface SelectProps<Value extends string = string> { name: string; value: Value; label?: string; ariaLabel?: string; choices: readonly SelectChoice<Value>[]; className?: string; placeholder?: string; disabled?: boolean; renderSelected?: (choice: SelectChoice<Value>) => SafeHtml }

export function Select<Value extends string>({ name, value, label, ariaLabel, choices, className = '', placeholder, disabled = false, renderSelected }: SelectProps<Value>) {
  ...
  return <wa-select class={`select${renderSelected ? ' select--custom-selected' : ''} ${className}`.trim()} data-component="select" name={name} label={label} aria-label={ariaLabel} value={value} placeholder={placeholder} disabled={disabled}>
