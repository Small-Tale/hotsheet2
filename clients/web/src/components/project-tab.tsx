...
export function ProjectTab({ id, name, location, selected = false, busy = false, disconnected = false, attention = false, closable = true,notificationCount=0,upNextCount=0,activeTicketCount=0 }: ProjectTabProps) {
  ...
  const workLabel=[normalizedUpNextCount>0?`${normalizedUpNextCount} Up Next ticket${normalizedUpNextCount===1?'':'s'}`:'',normalizedActiveTicketCount>0?`${normalizedActiveTicketCount} claimed ticket${normalizedActiveTicketCount===1?'':'s'}`:''].filter(Boolean).join(', ');
