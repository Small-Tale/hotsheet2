...
export function SavedViewDialog({ open, mode = 'create', name, query, queryTokens=[], busy = false, error = '' }: SavedViewDialogProps) {
  const rename=mode==='rename',title=rename?'Edit View':'Create View',queryValue=orderedSearchText(query,queryTokens,()=>true);
  return <wa-dialog class="saved-view-dialog" data-component="saved-view-dialog" data-mode={mode} label={title} aria-label={title} open={open || undefined}>
    <form data-action="save-saved-view" class="saved-view-dialog__form">
      ...
      <label class="saved-view-dialog__query"><span>Search query <sup aria-hidden="true">*</sup></span><ToolbarControlGroup className="workspace-header__search-group saved-view-dialog__query-field" expanded><TokenSearchField id="saved-view-query" label="Search query" query={query} tokens={queryTokens.map(toTokenSearchToken)} placeholder="Search tickets" disabled={busy} editAction="edit-saved-view-query-token" removeAction="remove-saved-view-query-token" clearAction="clear-saved-view-query" clearLabel="Clear search query" /></ToolbarControlGroup><input type="hidden" name="saved-view-query" value={queryValue}/><small>Use the same words, fields, operators, and filter chips as ticket search.</small></label>
