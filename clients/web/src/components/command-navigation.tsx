...
export function CommandNavigation({ label, commands, expanded }: CommandNavigationProps) {
  return <section class="command-navigation" data-component="command-navigation">
    ...
    {expanded && <div class="command-navigation__items">{commands.map(command => { const [icon, name] = icons[command.icon]; return <button type="button" class="command-navigation__command" style={`--command-color:${command.color}`} data-action="run-command" data-command-id={command.id} aria-pressed={String(Boolean(command.running))}><LucideIcon icon={icon} name={name} /><span>{command.running ? `Running ${command.label}` : command.label}</span>{command.running && <i aria-hidden="true"></i>}</button>; })}</div>}
  </section>;
}
