...
export function CommandNavigation({ label, commands, expanded,collapsedGroups=[] }: CommandNavigationProps) {
  ...
  return <section class="command-navigation" data-component="command-navigation">
    ...
    {expanded && [...groups].map(([group, items]) => {const groupExpanded=!group||!collapsedGroups.includes(group);return <div class="command-navigation__group" data-command-group={group || undefined}>{group&&<MenuHeader label={group} action="toggle-command-section" actionIcon={<LucideIcon icon={ChevronDown} name="chevron-down"/>} expanded={groupExpanded} toggle/>}{groupExpanded&&<div class="command-navigation__items">{items.map(command => { const [icon, name] = icons[command.icon],type=command.kind==='shell'?{icon:SquareTerminal,name:'square-terminal',label:'Shell command'}:command.kind==='ai'?{icon:Bot,name:'bot',label:'AI command'}:undefined; const color = resolveCustomizationColor(command.color); const textColor = customizationContrastColor(color); return <MenuItem action="run-command" itemId={command.id} rootAttributes={{'data-command-color':color}} className="command-navigation__command" style={`--command-color:${color};--command-text-color:${textColor};--kui-menu-item-color:${textColor};--kui-menu-item-background:${color};--kui-menu-item-hover-background:${color};--kui-menu-item-hover-border:transparent;--kui-menu-item-selected-color:${textColor};--kui-menu-item-selected-background:${color};--kui-menu-item-selected-border:transparent`} pressed={Boolean(command.running)} title={command.lastRun ? `Last run: ${command.lastRun}. Press and hold for output.` : 'Press and hold for command history.'} icon={<LucideIcon icon={icon} name={name} />} label={command.running ? `Running ${command.label}` : command.label} trailing={command.running ? <i aria-hidden="true"></i> : type?<span class="command-navigation__type" aria-label={type.label} title={type.label}><LucideIcon icon={type.icon} name={type.name}/></span>:undefined} />; })}</div>}</div>})}
  </section>;
}
