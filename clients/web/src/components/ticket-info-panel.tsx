...
export interface TicketInfoPanelProps { status: TicketStatus; priority: TicketPriority; category: string; tags: string[]; tagSuggestions?: readonly string[]; canUpdate?: boolean; details: string; detailsMode?: MarkdownEditorMode; detailsDirty?: boolean; readerPresentation?: boolean; readerEditing?: boolean; notes?: readonly NoteCardProps[]; editingNoteId?: string; noteDraft?: string; blockedReason?: string; blockedReasonEditing?: boolean; blockedReasonDraft?: string; providerName?: string; updatedLabel?: string }
export function TicketInfoPanel({ status, priority, category, tags, tagSuggestions, canUpdate = true, details, detailsMode = 'preview', detailsDirty = false, readerPresentation = false, readerEditing = false, notes = [], editingNoteId, noteDraft, blockedReason = '', blockedReasonEditing = false, blockedReasonDraft = blockedReason, providerName = 'Hot Sheet git', updatedLabel = 'Updated now' }: TicketInfoPanelProps) {
  return <div class="ticket-inspector__content" data-component="ticket-info-panel">
    ...
    <TicketNotes notes={notes} editingNoteId={editingNoteId} noteDraft={noteDraft} readerMode={readerPresentation} readerEditing={readerEditing} />
