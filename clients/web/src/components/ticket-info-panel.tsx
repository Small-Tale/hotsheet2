...
export interface TicketInfoPanelProps { status: TicketStatus; priority: TicketPriority; category: string; tags: string[]; details: string; detailsMode?: MarkdownEditorMode; detailsDirty?: boolean; readerPresentation?: boolean; notes?: readonly NoteCardProps[]; blockedReason?: string; providerName?: string; updatedLabel?: string }
export function TicketInfoPanel({ status, priority, category, tags, details, detailsMode = 'preview', detailsDirty = false, readerPresentation = false, notes = [], blockedReason, providerName = 'Hot Sheet git', updatedLabel = 'Updated now' }: TicketInfoPanelProps) {
  return <div class="ticket-inspector__content" data-component="ticket-info-panel">
    ...
    <section class="ticket-inspector__section"><header class="ticket-inspector__section-header"><h2>Details</h2>{!readerPresentation && <wa-button appearance="plain" data-action="open-ticket-reader" aria-label="Open ticket reader"><LucideIcon icon={BookOpen} name="book-open" /></wa-button>}</header><MarkdownEditor value={details} mode={detailsMode} dirty={detailsDirty} appearance="embedded" showExpand={false} label="Ticket details" /></section>
