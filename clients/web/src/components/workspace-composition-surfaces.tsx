...
export function SidebarSurface(props:SidebarSurfaceProps){
  ...
  if(props.kind==='notifications')return <NotificationNavigation selected={props.selected} counts={props.counts} collapseControl/>;
...
export function WorkspaceSurface(props:WorkspaceSurfaceProps){
  ...
  return <><TicketList {...props.list}/>{props.more}</>;
}

export interface TerminalRailSurfaceProps{rail:TerminalTicketRailProps}
export function TerminalRailSurface({rail}:TerminalRailSurfaceProps){return <TerminalTicketRail {...rail}/>}

export type GlobalWorkspaceSurfaceProps=
  | {kind:'terminals';dashboard:TerminalDashboardProps}
  | {kind:'stats';projectName?:string};
...
export function GlobalWorkspaceSurface(props:GlobalWorkspaceSurfaceProps){
  ...
  if(props.projectName)return <section class="app-empty" aria-label={`${props.projectName} project statistics`}><h1>{props.projectName} project statistics</h1><p>Detailed ticket-flow and usage charts are coming in a future Hot Sheet update.</p></section>;
  return <section class="app-empty"><h1>Cross-project stats</h1><p>This dashboard is still being designed.</p></section>;
}

export interface ProjectTerminalDrawerSurfaceProps{drawer?:TerminalDrawerProps}
export function ProjectTerminalDrawerSurface({drawer}:ProjectTerminalDrawerSurfaceProps){return drawer?<TerminalDrawer {...drawer}/>:<></>}

export interface TerminalOperationsSurfaceProps{projects:readonly TerminalProjectSummary[]}
export function TerminalOperationsSurface({projects}:TerminalOperationsSurfaceProps){return <TerminalOperationsSidebar projects={projects}/>}
