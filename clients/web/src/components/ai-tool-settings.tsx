...
export function AiToolSettings({tools,selection,loading=false,message=''}:{tools:readonly AiToolDescriptor[];selection:AiToolSelection;loading?:boolean;message?:string}){
  ...
  const modelChoices=[...(customModel?[{value:customModel,label:customModel,icon:Brain,iconName:'brain'}]:[]),...active.models.map(item=>({value:item.id,label:item.label,icon:Brain,iconName:'brain'})),{value:otherModelValue,label:'Other…',icon:Pencil,iconName:'pencil',separatorBefore:true}];
  return <section class="ai-tool-settings" data-component="ai-tool-settings" data-other-model-value={otherModelValue} aria-busy={String(loading)}>
