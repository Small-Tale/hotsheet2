...
describe('content components', () => {
  ...
  it('projects supported Markdown blocks without injecting source as HTML', () => {
    const markup = String(MarkdownPreview({ source: '# Title\n- item\n<script>alert(1)</script>' }));
    expect(markup).toContain('<h1');
    expect(markup).toContain('markdown-preview__list-item');
    expect(markup).toContain('&lt;script&gt;alert(1)&lt;/script&gt;');
  });

  it('derives the TicketReader note count and reuses NoteCard', () => {
    const markup = String(TicketReader({ slug: 'HS2-TEST', title: 'Reader', details: 'Details', notes: [{ id: 'one', kind: 'regular', author: 'Codex', time: 'Now', body: 'Done' }] }));
