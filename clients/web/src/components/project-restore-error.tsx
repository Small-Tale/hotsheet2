...
export function ProjectRestoreError({ root, name, error, recoveryPid, busy = false }: ProjectRestoreFailure) {
  const recovery = recoveryPid
    ? ` A registered Hot Sheet server process (${recoveryPid}) is not responding.`
    : '';
  return <EmptyState
    ...
    action={<wa-button appearance="accent" data-action="retry-project-restore" data-project-root={root} disabled={busy}>{busy ? 'Retrying…' : 'Retry project'}</wa-button>}
  />;
}
