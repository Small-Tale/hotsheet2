...
describe('CommandRunDialog', () => {
  it('uses Kerf semantic spacing around dialog regions, output, actions, and icon labels',()=>{
    ...
    expect(css).toMatch(/command-run-dialog__summary \{[^}]*margin-bottom: var\(--kui-space-m\)[^}]*gap: var\(--kui-space-xs\)/);
    expect(css).toMatch(/command-run-dialog"\] pre \{[^}]*padding: var\(--kui-space-xs\)/);
    expect(css).toMatch(/command-run-dialog"\], \[data-component="command-cancellation-dialog"\] \{[^}]*padding: var\(--kui-space-l\)/);
  });
