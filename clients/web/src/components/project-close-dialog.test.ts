...
describe('ProjectCloseDialog',()=>{
  ...
  it('provides stable resource identity, selection, summary, and responsive two-column layout',()=>{
    ...
    const css=readFileSync(resolve(import.meta.dirname,'project-close-dialog.css'),'utf8');expect(css).toContain('[data-has-resources="false"]');expect(css).toMatch(/grid-template-columns:minmax\(remify\(224px\),remify\(288px\)\) minmax\(0,1fr\)/);expect(css).toMatch(/@media \(max-width:remify\(672px\)\)[\s\S]*grid-template-columns:1fr/);expect(css).toContain('background:var(--wa-color-surface-default)');expect(css).toContain('.project-close-dialog__terminal .terminal-viewport--scaled-preview');expect(css).toContain('.project-close-dialog__chat > .ai-conversation');
  });

  it('uses canonical dialog-region spacing while retaining preview and control geometry',()=>{const css=readFileSync(resolve(import.meta.dirname,'project-close-dialog.css'),'utf8');expect(css).not.toContain('--wa-space-');expect(css).toMatch(/__intro \{[^}]*padding:var\(--kui-space-m\) var\(--kui-space-l\)[^}]*gap:var\(--kui-space-m\)/);expect(css).toMatch(/__layout > aside \{[^}]*padding:var\(--kui-space-xs\)/);expect(css).toMatch(/aside nav \{ display:grid; gap:var\(--kui-space-none\)/);expect(css).toMatch(/__terminal \.terminal-tile__viewport-frame \{[^}]*inset:var\(--kui-space-l\)/);expect(css).toMatch(/__consequences \{ padding:var\(--kui-space-m\) var\(--kui-space-l\) 0/);expect(css).toMatch(/__actions \{[^}]*gap:var\(--kui-space-xs\)/);expect(css).toMatch(/@media[\s\S]*viewport-frame \{ inset:var\(--kui-space-m\)/);expect(css).toMatch(/__layout \{[^}]*min-height:remify\(352px\)/)});
});
