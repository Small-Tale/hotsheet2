...
export interface TicketPrioritySelectProps { name: string; value: TicketPriority; label?: string; disabled?: boolean; placeholder?: boolean }
export function TicketPrioritySelect({ name, value, label = 'Priority', disabled = false, placeholder = false }: TicketPrioritySelectProps) {
  return <Select className="ticket-priority-select" name={name} value={value} label={label} choices={PRIORITIES} disabled={disabled} placeholder={placeholder} />;
}
