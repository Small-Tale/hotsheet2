  ...
  return <aside class="project-sidebar" data-component="project-sidebar" aria-label="Project sidebar">
    ...
    <footer class="project-sidebar__footer">
      ...
      <div class="project-sidebar__drive-row">
        <Select name="drive-tool" value={props.driveTool} ariaLabel="AI tool for Drive and Chat" choices={[{value:'codex',label:'Codex'},{value:'claude',label:'Claude'}]} fitMenu />
        <DriveControl running={props.driveRunning} tool={props.driveTool==='codex'?'Codex':'Claude'} disabled={props.driveDisabled} disabledReason={props.driveDisabledReason} />
        <button type="button" class="project-sidebar__conversation" data-action="open-conversation" aria-label={`Open ${props.driveTool==='codex'?'Codex':'Claude'} conversation`} title="Open chat without starting the Hot Sheet workflow" aria-pressed={props.conversationOpen?'true':'false'} disabled={props.driveDisabled}><LucideIcon icon={MessageSquare} name="message-square"/></button>
      </div>
    </footer>
  </aside>;
}
