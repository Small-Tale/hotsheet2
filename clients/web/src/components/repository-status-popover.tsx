...
export function RepositoryStatusPopover({status,error='',initialized=true,setupStep,setupBusy=false,setupError='',refreshing=false,view='unstaged',fileMenu,selectedFiles=[],embedded=false,comparison,expandedCommits,detailFiles,detailCommits,detailLoading=false,detailError='',detailHasMore=false}:{status:RepositoryStatus|null;error?:string;initialized?:boolean;setupStep?:RepositorySetupStep;setupBusy?:boolean;setupError?:string;refreshing?:boolean;view?:RepositoryStatusView;fileMenu?:RepositoryFileMenu;selectedFiles?:readonly string[];embedded?:boolean;comparison?:CodeReviewComparison;expandedCommits?:readonly string[];detailFiles?:RepositoryFile[];detailCommits?:CodeReview['commits'];detailLoading?:boolean;detailError?:string;detailHasMore?:boolean}){
  ...
  return <section popover={embedded?undefined:'auto'} id={embedded?undefined:'repository-status-popover'} class="dialog-surface repository-status-popover" data-component="repository-status-popover" data-state={state} data-view={view} data-setup-step={recoveryStep} data-embedded={embedded?'true':undefined} role="dialog" aria-labelledby="repository-status-title">
    ...
    {status&&!recoveryStep&&<div class="repository-status-popover__layout"><aside>
      ...
      <nav aria-label="Repository views"><ListHeader label="Views"/>{viewDefinitions.map(item=><ListItem action="select-repository-view" itemId={item.id} selected={view===item.id} icon={<LucideIcon icon={item.icon} name={item.id==='commits'?'git-commit-horizontal':item.id==='untracked'?'square-pen':item.id==='conflicted'?'square-x':item.id==='staged'?'square-plus':'square-minus'}/>} label={item.label} trailing={<small class="kui-list-item__count">{repositoryViewCount(status,item.id)}</small>}/>)}</nav>
...
export function ChangeEvidenceDialog({review,view='docs',embedded=false,fileMenu,selectedFiles=[],platform}:{review?:CodeReview;view?:ChangeEvidenceView;embedded?:boolean;fileMenu?:RepositoryFileMenu;selectedFiles?:readonly string[];platform?:RepositoryStatus['platform']}){
  ...
  return <section popover={embedded?undefined:'auto'} id={embedded?undefined:'change-evidence-dialog'} class="dialog-surface repository-status-popover change-evidence-dialog" data-component="change-evidence-dialog" data-view={view} data-embedded={embedded?'true':undefined} role="dialog" aria-labelledby="change-evidence-title">
    ...
    <div class="repository-status-popover__layout change-evidence-dialog__layout"><aside><nav aria-label="Change evidence views"><ListHeader label="Views"/>{evidenceViews.map(item=><ListItem action="select-change-evidence-view" itemId={item.id} selected={view===item.id} icon={<LucideIcon icon={item.icon} name={item.id}/>} label={item.label} trailing={<small class="kui-list-item__count">{files.filter(file=>file.category===item.id).length}</small>}/>)}</nav></aside>
