...
export function TicketCloseDialog({ state }: { state?: TicketCloseDialogState }) {
  ...
  const candidates = state.candidates.filter(
    (candidate) => duplicateTargetKey(candidate) !== duplicateTargetKey(state.source),
  );
  return (
    <wa-dialog
    ...
    >
      <form data-action="submit-ticket-close" class="ticket-close-dialog__form">
        ...
        <Select
          ...
          value={state.reason}
          label="Close as"
          choices={TICKET_CLOSE_REASON_CHOICES}
        />
        {duplicate && (
          <section class="ticket-close-dialog__duplicate" aria-label="Duplicate target">
            <wa-input
              name="ticket-close-target-search"
            ...
            {state.selected && (
              <div class="ticket-close-dialog__selected" role="status">
                <LucideIcon icon={CheckCircle2} name="check-circle-2" />
                <span>
                  <strong>
                    {state.selected.slug}
            ...
            {!state.selected && state.query.trim() && (
              <div
                class="ticket-close-dialog__results"
                aria-label="Matching tickets"
                aria-busy={String(Boolean(state.searching))}
              >
                {candidates.map((candidate) => (
                  <ListItem
                    action="select-ticket-close-target"
            ...
            <p class="ticket-close-dialog__hint">
              The selected ticket becomes the canonical target. This relationship is stored as structured duplicate
              metadata.
            </p>
          </section>
        )}
        <p class="ticket-close-dialog__error" role="alert">
        ...
        <div slot="footer" class="ticket-close-dialog__actions">
          ...
          <wa-button type="submit" variant="brand" disabled={Boolean(validation) || state.submitting}>
            {state.submitting ? 'Closing…' : duplicate ? 'Mark as duplicate' : 'Close ticket'}
          </wa-button>
        </div>
      </form>
    </wa-dialog>
