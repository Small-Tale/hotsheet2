...
const source = {
  ...
  projectId: 'alpha',
  projectName: 'Alpha',
  connectionId: 'git-alpha',
  nativeId: 'source',
  qualifiedId: 'git-alpha:source',
};
const target = {
  id: 'target',
  slug: 'HS2-TARGET',
  ...
  projectId: 'beta',
  projectName: 'Beta',
  connectionId: 'git-beta',
  nativeId: 'target',
  qualifiedId: 'git-beta:target',
};
const sameSlug = {
  ...target,
  id: 'collision',
  projectId: 'gamma',
  projectName: 'Gamma',
  connectionId: 'git-gamma',
  nativeId: 'collision',
  qualifiedId: 'git-gamma:collision',
};
...
describe('TicketCloseDialog', () => {
  ...
  it('shows the selected canonical ticket and enables duplicate close', () => {
    const markup = String(
      TicketCloseDialog({ state: { source, reason: 'duplicate', query: '', candidates: [], selected: target } }),
    );
  ...
  it('contains long result lists within a scrollable dialog surface', () => {
    ...
    expect(css).toMatch(
      /ticket-close-dialog__results \{[^}]*min-height: remify\(68px\);[^}]*max-height: remify\(224px\);[^}]*overflow-y: auto/,
    );
    expect(css).toContain('width: min(remify(544px), calc(100vw - 2 * var(--kui-space-m)))');
  });
});
