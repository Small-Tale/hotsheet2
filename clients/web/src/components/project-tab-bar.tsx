  ...
  if (mobile) {
    ...
    return (
      <div class="project-tab-bar project-tab-bar--mobile" data-component="project-tab-bar" data-mode={mode}>
        {modes}
        {tabs.length
          ? <Select className="project-tab-bar__select" name="mobile-project" value={active.id} ariaLabel="Project" choices={tabs.map(tab => ({ value: tab.id, label: tab.name }))} renderSelected={choice => <span>{choice.label}</span>} />
