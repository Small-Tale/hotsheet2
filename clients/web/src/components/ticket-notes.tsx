...
export function TicketNotes({ notes }: { notes: readonly NoteCardProps[] }) {
  return <section class="ticket-notes" data-component="ticket-notes">
    ...
    {notes.length > 0 ? <div class="ticket-notes__list">{notes.map(note => <NoteCard {...note} />)}</div> : <p class="ticket-notes__empty">No notes added.</p>}
