...
export function TicketNotes({ notes, editingNoteId, noteDraft }: { notes: readonly NoteCardProps[]; editingNoteId?: string; noteDraft?: string }) {
  return <section class="ticket-notes" data-component="ticket-notes">
    ...
    {notes.length > 0 ? <div class="ticket-notes__list">{notes.map(note => <NoteCard {...note} editing={note.id === editingNoteId} draft={note.id === editingNoteId ? noteDraft : undefined} />)}</div> : <p class="ticket-notes__empty">No notes added.</p>}
