...
describe('LucideIconPicker', () => {
  it('renders a search field and the curated popular icons by default, marking the resolved selection', () => {
    const markup = String(LucideIconPicker({ value: 'test', searchName: 'command-icon-search', selectAction: 'select-command-icon' }));
  ...
  it('shows a loading hint when searching before the catalog has loaded', () => {
    const markup = String(LucideIconPicker({ query: 'compass', searchName: 'command-icon-search', selectAction: 'select-command-icon' }));
  ...
  it('searches the full catalog once loaded and reports an empty result', async () => {
    ...
    const empty = String(LucideIconPicker({ query: 'definitely-not-an-icon-zzz', searchName: 'command-icon-search', selectAction: 'select-command-icon' }));
