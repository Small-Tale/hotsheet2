...
describe('AIConversation',()=>{
  ...
  it('offers an editable model with catalog suggestions and effort changes only when the plugin declares support',()=>{const markup=String(AIConversation({open:true,presentation:'embedded',selectionId:'chat-one',tool:'Codex',messages:[],draft:'',busy:false,interruptible:false,model:'legacy model',effort:'high',models:[{id:'gpt',label:'GPT'}],efforts:['medium','high'],canChangeModel:true,canChangeEffort:true}));expect(markup).toContain('name="conversation-model" value="legacy model"');expect(markup).toContain('list="conversation-model-choices-chat-one"');expect(markup).toContain('<option value="gpt">GPT</option>');expect(markup).toContain('name="conversation-effort" label="Effort"');expect(markup).not.toContain('data-action="stop-conversation"')});
