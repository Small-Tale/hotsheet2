...
export function TicketStatusMenu({ value, disabled = false, placeholder = false }: { value: TicketStatus; disabled?: boolean; placeholder?: boolean }) {
  const selected = statusPresentation(value);
  return <Select className="ticket-status-menu" name="inspector-status" ariaLabel={`${disabled ? 'Status' : 'Change status'}, ${selected.label}`} value={value} choices={TICKET_STATUS_CHOICES} disabled={disabled} placeholder={placeholder} renderSelected={choice => <StatusBadge status={choice.value} />} />;
}
