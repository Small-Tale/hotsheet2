...
describe('permission presentation components', () => {
  ...
  it('offers the three notification views with counts and current state', () => {
    ...
    expect(markup.match(/class="kui-menu-item__count"/g)).toHaveLength(3);
    expect(markup).toContain('class="kui-menu-item__count" data-attention="true">2</small>');
    expect(markup).toContain('data-item-id="day"');
    expect(markup).toContain('aria-current="page"');
    expect(String(NotificationNavigation({ selected: 'pending', counts: { pending: 0, day: 0, week: 0 } }))).toContain('class="kui-menu-item__count" data-attention="false">0</small>');
  });
});
