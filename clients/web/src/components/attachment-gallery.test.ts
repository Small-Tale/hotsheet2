...
describe('AttachmentGallery', () => {
  ...
  it('keeps full-screen media and its sizing wrapper square-cornered', () => {
    ...
    expect(css).toMatchSource(/\.attachment-gallery__media-wrap :is\(img,video\) \{[^}]*border-radius:0/);
  });
  ...
  it('shows the active image with accessible cyclic navigation controls', () => {
    ...
    expect(markup).toContain('Image 2 of 2: b.svg');
    expect(markup).toContain('data-action="previous-gallery-image"');
  ...
  it('uses durable attachment identity before an ambiguous shared alias', () => {
    const ambiguous = [
      { ...images[0], ticket: 'HS2-DEMO', attachmentId: 'A1', aliases: ['/shared.png'] },
      { ...images[1], ticket: 'HS2-DEMO', attachmentId: 'A2', aliases: ['/shared.png'] },
    ];
    expect(
      attachmentGallerySelectionUrl(ambiguous, {
        url: '/shared.png',
        attachmentId: 'A2',
        ticket: 'HS2-DEMO',
        name: 'b.svg',
    ...
    expect(attachmentGallerySelectionUrl(ambiguous, { url: '/shared.png', ticket: 'HS2-DEMO', name: 'b.svg' })).toBe(
      '/b.svg',
    );
  ...
  it('renders the decoded first video frame while paused with only the custom playback and volume controls', () => {
    const markup = String(
      AttachmentGallery({
        images: [
          { id: 'video', name: 'walkthrough.mp4', url: '/walkthrough.mp4', thumbnailUrl: '/walkthrough-poster.jpg' },
        ],
  ...
  it('keeps the click-open volume popup rendered until an outside interaction closes it', () => {
    const markup = String(
      AttachmentGallery({
        images: [{ id: 'video', name: 'walkthrough.mp4', url: '/walkthrough.mp4' }],
        activeUrl: '/walkthrough.mp4',
        volumeOpen: true,
        muted: true,
      }),
    ...
    expect(markup).not.toContain(
      'class="attachment-gallery__volume-popup" role="group" aria-label="Volume controls" hidden',
    );
  ...
  it('renders selected normalized rectangles, resize handles, and draggable video range brackets in markup mode', () => {
    const markup = String(
      AttachmentGallery({
        ...
        markup: true,
        drawMode: true,
        selectedAnnotation: 'annotation-1',
        playheadMs: 1_500,
        durationMs: 10_000,
        annotations: [
          {
  ...
  it('renders range handles only for the currently selected timed annotation', () => {
    const annotations = [
      ...
      { id: 'second', x: 200, y: 200, width: 1000, height: 1000, start_ms: 3000, end_ms: 4000, text: 'Second' },
    ];
    const selected = String(
      AttachmentGallery({
        images: [{ id: 'video', name: 'clip.mp4', url: '/clip.mp4' }],
        activeUrl: '/clip.mp4',
        markup: true,
        selectedAnnotation: 'second',
        playheadMs: 3000,
        durationMs: 10_000,
        annotations,
      }),
  ...
  it('uses a duration-scaled review tolerance around timed rectangles', () => {
    ...
    expect(attachmentGalleryAnnotationVisible({ ...range, start_ms: 15_000, end_ms: 15_000 }, 14_000, 20_000)).toBe(
      true,
    );
    expect(readFileSync(new URL('./attachment-gallery.css', import.meta.url), 'utf8')).toContainSource(
      '.attachment-gallery__annotation[hidden] { display:none; }',
    );
  });
  ...
  it('maps standard playback and frame-jogging keys with bounded repeated transitions', () => {
    ...
    for (let index = 0; index < 4; index++) {
      const action = attachmentGalleryKeyboardAction('ArrowRight', playhead, 6000);
      if (action?.kind === 'seek') playhead = action.playheadMs;
    }
    expect(playhead).toBe(6000);
  });
  it('arbitrates gallery swipes away from controls, markup, and zoom panning', () => {
    ...
    for (const disabled of [
      ...
      expect(attachmentGallerySwipeGesture({ ...start, ...disabled })).toBeUndefined();
  });
  ...
  it('uses the same point/range controls for animated SVG annotations', () => {
    const markup = String(
      AttachmentGallery({
        images: [{ id: 'svg', name: 'animated.svg', url: '/animated.svg' }],
        activeUrl: '/animated.svg',
        markup: true,
        playheadMs: 500,
        durationMs: 2000,
        annotations: [{ id: 'point', x: 100, y: 100, width: 1000, height: 1000, start_ms: 500, end_ms: 500, text: '' }],
  ...
  it('renders wireframe-like annotation ticks that seek to their times', () => {
    const markup = String(
      AttachmentGallery({
        ...
        durationMs: 10_000,
        annotations: [
          { id: 'point', x: 0, y: 0, width: 100, height: 100, start_ms: 2500, end_ms: 2500, text: 'Point' },
          { id: 'range', x: 0, y: 0, width: 100, height: 100, start_ms: 5000, end_ms: 7000, text: 'Range' },
        ],
      }),
    );
    ...
    expect(readFileSync(new URL('./attachment-gallery.css', import.meta.url), 'utf8')).toContainSource(
      '.attachment-gallery__timeline-annotation[data-has-range="true"]',
    );
  });
  ...
  it.each([
    ...
    [{ naturalWidth: 400, naturalHeight: 800, availableWidth: 600, availableHeight: 400 }, [0.5, 1, 1.5]],
  ] as const)('orders natural, fit, and cover zoom stops for every geometry', (geometry, expected) => {
    expect(attachmentGalleryZoomStops(geometry).stops).toEqual(expected);
  });
  it('deduplicates equal stops and disables zoom when every scale is natural size', () => {
    expect(
      attachmentGalleryZoomModel({ naturalWidth: 800, naturalHeight: 600, availableWidth: 800, availableHeight: 600 }),
    ).toMatchObject({ stops: [1], scale: 1, canZoomOut: false, canZoomIn: false });
  });
  it('falls back safely for unmeasured geometry and retains the nearest stop after resize', () => {
    expect(
      attachmentGalleryZoomStops({ naturalWidth: 0, naturalHeight: 0, availableWidth: 0, availableHeight: 0 }),
    ).toEqual({ stops: [1], fit: 1 });
    expect(
      attachmentGalleryZoomModel(
        { naturalWidth: 2000, naturalHeight: 1000, availableWidth: 1200, availableHeight: 900 },
        0.79,
      ).scale,
    ).toBe(0.9);
  });
