...
export function ResizableRegion({ id, label, size, min, max, axis = 'horizontal', edge = 'end', collapsed = false, children }: ResizableRegionProps) {
  ...
  return <section class="resizable-region" data-component="resizable-region" data-region-id={id} data-axis={axis} data-edge={edge} data-collapsed={String(collapsed)} style={`--resizable-region-size:${resolved}px;--resizable-region-expanded-size:${clampRegionSize(size, min, max)}px`} aria-label={label}>
