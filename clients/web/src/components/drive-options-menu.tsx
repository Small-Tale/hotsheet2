...
import { Bot, Brain, ChevronRight, Gauge, RotateCcw } from 'lucide';
...
function choice(action:string,value:string,label:string,selected:boolean){return <wa-dropdown-item slot="submenu" type="checkbox" checked={selected} data-action={action} data-value={value} value={value}>{label}</wa-dropdown-item>}
...
export function DriveOptionsMenu({tools,selection,defaultSelection}:{tools:readonly AiToolDescriptor[];selection:AiToolSelection;defaultSelection:AiToolSelection}){
  ...
  return <div class="drive-options-menu" role="menu" aria-label="Drive provider, model, and effort options">
    <wa-dropdown open placement="top-start" distance={6}>
      ...
      <wa-dropdown-item><span slot="icon"><LucideIcon icon={Bot} name="bot"/></span>Provider<span slot="details">{activeTool.display_name}</span><LucideIcon icon={ChevronRight} name="chevron-right"/>{tools.map(tool=>choice('select-drive-tool',tool.id,tool.display_name,tool.id===activeTool.id))}</wa-dropdown-item>
      <wa-dropdown-item disabled={!activeTool.models.length}><span slot="icon"><LucideIcon icon={Brain} name="brain"/></span>Model<span slot="details">{activeModel?.label}</span>{activeTool.models.map(model=>choice('select-drive-model',model.id,model.label,model.id===activeModel?.id))}</wa-dropdown-item>
      <wa-dropdown-item disabled={!efforts.length}><span slot="icon"><LucideIcon icon={Gauge} name="gauge"/></span>Effort<span slot="details">{selection.effort??defaultSelection.effort??activeTool.default_effort}</span>{efforts.map(effort=>choice('select-drive-effort',effort,effort,effort===(selection.effort??defaultSelection.effort??activeTool.default_effort)))}</wa-dropdown-item>
    </wa-dropdown>
  </div>;
}
