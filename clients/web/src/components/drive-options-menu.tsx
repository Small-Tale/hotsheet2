...
import { Bot, Brain, Gauge, type IconNode, Pencil, RotateCcw } from 'lucide';
...
export function DriveOptionsMenu({tools,selection,defaultSelection,loading=false,error}:{tools:readonly AiToolDescriptor[];selection:AiToolSelection;defaultSelection:AiToolSelection;loading?:boolean;error?:string}){
  ...
  return <div class="drive-options-menu" data-component="drive-options-menu" role="menu" aria-label="Drive provider, model, and effort options">
    <wa-dropdown open placement="top-start" distance={6}>
      ...
      <wa-dropdown-item><span slot="icon"><LucideIcon icon={Brain} name="brain"/></span>Model<span slot="details">{activeModel?.label??modelId}</span>{customModel&&choice('select-drive-model',customModel,customModel,true,Brain,'brain')}{activeTool.models.map(model=>choice('select-drive-model',model.id,model.label,model.id===activeModel?.id,Brain,'brain'))}<wa-divider slot="submenu"></wa-divider>{choice('open-drive-manual-model','other','Other…',false,Pencil,'pencil')}</wa-dropdown-item>
