...
function validRange(value: CompatibilityRange | undefined): value is CompatibilityRange {
  return Boolean(
    value && Number.isInteger(value.min) && Number.isInteger(value.max) && value.min >= 0 && value.min <= value.max,
  );
}
...
export function assessCompatibility(
  ...
  const unknown = (detail: string): CompatibilityAssessment => ({
    kind: 'unknown',
    detail,
  ...
  if (server.generation !== 'hs2')
    return unknown(`Expected an HS2 server, received ${server.generation || 'an unknown generation'}.`);
  if (!validRange(server.protocol) || !validRange(client))
    return unknown('The server or client reported an invalid protocol range.');
  const sourceStale =
  ...
  if (server.protocol.max < client.min)
    return {
      ...
      sourceStale,
      canRestartServer:
        server.capabilities?.lifecycle_restart === true && server.capabilities.lifecycle_quiescence === true,
      ...context,
    };
  if (client.max < server.protocol.min)
    return {
      kind: 'client_too_old',
