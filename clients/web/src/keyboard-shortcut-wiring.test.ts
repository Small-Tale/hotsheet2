...
describe('keyboard-shortcut dispatcher wiring (HS2-9SHYWD)', () => {
  ...
  const boundIds = [
    'toggle-left-sidebar', 'toggle-right-sidebar', 'toggle-bottom-drawer',
    'view-list', 'view-board', 'view-notifications', 'view-settings',
    'view-workspace-grid', 'view-all-stats', 'new-ticket',
    'project-tab-next', 'project-tab-previous', 'drawer-tab-next', 'drawer-tab-previous',
  ];

  it('resolves every new command shortcut through matchesShortcut in the dispatcher', () => {
    for (const id of boundIds) {
      expect(source, id).toContain(`matchesShortcut('${id}',event,overrides,appleShortcutPlatform)`);
    }
  });
  ...
  it('wires each binding to its corresponding app action', () => {
    ...
    expect(source).toContain('cycleTabId(projects.value.map(item=>item.id),selectedProjectId.value');
    expect(source).toContain("cycleTabId(['grid',...currentDrawerTabIds(drawerProject.id)],terminalDrawerSelected.value");
    ...
    expect(source).toMatch(/!editable&&matchesShortcut\('new-ticket'/);
  });
