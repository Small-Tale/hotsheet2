      ...
      setTimeout(() => {
        const painted = performance.now(),
          timing: InteractionTiming = {
            name,
            update_ms: updated - started,
            paint_ms: painted - started,
            budget_ms: INTERACTION_BUDGET_MS,
            over_budget: painted - started > INTERACTION_BUDGET_MS,
            ...(detail ? { detail } : {}),
          };
        document.dispatchEvent(new CustomEvent<InteractionTiming>('hotsheet:interaction-timing', { detail: timing }));
      }, 0),
    );
  };
}
