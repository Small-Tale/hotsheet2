...
export function loadProjectWorkspaceSession(storage:Pick<Storage,'getItem'>,projectId:string):ProjectWorkspaceSession|undefined{
  try{
    ...
    return {
      selectedView:((['all','backlog','archive','trash','errors'].includes(selectedView)||/^custom:[a-z0-9][a-z0-9-]{0,63}$/.test(selectedView))?selectedView:'all') as TicketView,
...
export function reconcileRememberedProjectRoots(requested:readonly string[],opened:readonly string[]){const available=new Set(opened);return{remembered:[...requested],failed:requested.filter(root=>!available.has(root))}}
...
export function hs1CleanupPromptDismissed(storage:Pick<Storage,'getItem'>,projectId:string,sourceIdentity:string){return Boolean(sourceIdentity)&&storage.getItem(HS1_CLEANUP_DISMISSED+projectId)===sourceIdentity}
export function dismissHs1CleanupPrompt(storage:Pick<Storage,'setItem'>,projectId:string,sourceIdentity:string){if(sourceIdentity)storage.setItem(HS1_CLEANUP_DISMISSED+projectId,sourceIdentity)}
