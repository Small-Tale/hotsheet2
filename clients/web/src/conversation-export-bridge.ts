...
function isManifest(value: unknown): value is ConversationExportManifest {
  ...
  return (
    ...
    manifest.manifestVersion === CONVERSATION_EXPORT_MANIFEST_VERSION &&
    typeof manifest.exportId === 'string' &&
    typeof manifest.revision === 'number' &&
    Number.isInteger(manifest.revision) &&
    manifest.revision > 0 &&
    Array.isArray(manifest.selectedMessageIds) &&
    Boolean(manifest.source?.conversationId) &&
    Boolean(manifest.reopen?.conversationId)
  );
}
...
function validatePayload(payload: ConversationExportPayload): void {
  ...
  if ((request as { manifestVersion?: unknown }).manifestVersion !== CONVERSATION_EXPORT_MANIFEST_VERSION)
    throw new Error('This conversation export version is not supported.');
  if (request.destination.kind !== 'directory') throw new Error('Only folder conversation bundles are supported.');
  if (!Array.isArray(messages) || messages.length === 0)
    throw new Error('The conversation export contains no messages.');
  if (
    (messages as unknown[]).some(
      (message) =>
        !message ||
    ...
    throw new Error('The conversation export contains an invalid message.');
  if (messages.map((message) => message.id).join('\0') !== request.selectedMessageIds.join('\0'))
    throw new Error('The selected message IDs do not match the exported transcript.');
  if (!Array.isArray(activity) || !Array.isArray(assets))
    throw new Error('The conversation export payload is incomplete.');
  let total = 0;
  for (const asset of assets) {
    if (
      ([asset] as unknown[]).some(
        (value) =>
          !value ||
          typeof value !== 'object' ||
          ...
          !['attachment', 'media'].includes(String((value as { kind?: unknown }).kind)) ||
          typeof (value as { dataBase64?: unknown }).dataBase64 !== 'string',
      )
    )
      throw new Error('The conversation export contains an invalid asset.');
  ...
  return {
        ...
        ...(existing
          ? {
              existing: {
                ...
                sourceConversationId: existing.source.conversationId,
              },
            }
          : {}),
      };
    ...
    async write(payload) {
      ...
      const occupied = await exists(path),
        existing = occupied ? await readManifest(path) : undefined;
      if (request.writeMode === 'create' && occupied)
        throw new Error('That export bundle already exists. Choose re-export, overwrite, or another destination.');
      if (request.writeMode !== 'create' && !existing)
        throw new Error('The selected folder is not a valid Hot Sheet conversation export.');
      if (request.replacesExportId && existing?.exportId !== request.replacesExportId)
        throw new Error('The export changed after it was selected. Choose the destination again.');
      if (
        request.writeMode === 'reexport' &&
        (existing?.source.conversationId !== request.source.conversationId ||
          existing.revision !== request.parentRevision)
      )
        throw new Error('The previous conversation export revision no longer matches. Choose it again.');
      if (!occupied) await mkdir(path);
      const exportedAssets: ConversationExportManifest['assets'] = [];
      for (const directory of ['attachments', 'media'])
        await rm(join(path, directory), { recursive: true, force: true });
      for (const asset of assets) {
        ...
        const directory = asset.kind === 'media' ? 'media' : 'attachments',
          filename = safeAssetName(asset.id, asset.filename),
      ...
      if (request.bundle.includeSummary)
        await atomicText(join(path, 'summary.md'), conversationSummaryMarkdown(request.source, messages));
    ...
    async open() {
      ...
      try {
        structured = JSON.parse(await readFile(join(path, 'conversation.json'), 'utf8'));
      } catch (reason) {
        throw new Error(`The saved conversation transcript could not be read: ${errorMessage(reason)}`, {
          cause: reason,
        });
      }
      const value = structured as {
        messages?: ConversationExportOpenResult['messages'];
        activity?: ConversationExportOpenResult['activity'];
      };
      ...
      validatePayload({
        request: {
          ...
          selectedMessageIds: manifest.selectedMessageIds,
          destination: { selectionToken: 'open', kind: 'directory' },
          writeMode: 'create',
          bundle: manifest.bundle,
          reopen: manifest.reopen,
        },
        messages,
        activity,
        assets: [],
