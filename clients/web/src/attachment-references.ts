...
export interface AttachmentReferenceContext {
  ...
  checkout: string;
  ticket: string;
  attachments?: readonly { ticket?: string; filename: string; id?: string }[];
}
...
export function attachmentReferenceUrl(context: AttachmentReferenceContext, reference: AttachmentReference): string {
  ...
  const ticket = reference.ticket || context.ticket,
    attachment = reference.id ? encodeURIComponent(reference.id) : `by-name/${encodeURIComponent(reference.filename)}`;
  return `${base}/checkouts/${encodeURIComponent(context.checkout)}/tickets/${encodeURIComponent(ticket)}/attachments/${attachment}`;
}

function resolveKnownAttachment(
  context: AttachmentReferenceContext,
...
export function expandAttachmentReferences(source: string, context?: AttachmentReferenceContext): string {
  ...
  const resolve = (raw: string, label?: string, image = false, autoPreview = true) => {
    const parsed = parseAttachmentReference(raw),
      reference = parsed && resolveKnownAttachment(context, parsed);
    if (!reference) return undefined;
    const url = attachmentReferenceUrl(context, reference),
      escapeBackticks = (value: string) => value.replaceAll('`', '\\`'),
      text = escapeBackticks(label || reference.filename),
      suffix = parsed.filename.slice(reference.filename.length),
      canonical = escapeBackticks(`attachment:${reference.ticket ? `[${reference.ticket}]` : ''}${reference.filename}`);
    const link =
      image || (autoPreview && isImageAttachment(reference.filename))
        ? `![${text}](${url} "${canonical}")`
        : `[${text}](${url} "${canonical}")`;
    return `${link}${suffix}`;
  };
  return source
    .replace(
      /(?<!`)(^|\s)(attachment:(?:\[[^\]]+\])?[A-Za-z0-9_.@+()-]+)/gm,
      (whole, prefix: string, raw: string) => `${prefix}${resolve(raw) ?? raw}`,
    )
    .replace(
      /(?<!`)(`+)( ?)(attachment:(?:\[[^\]]+\])?[^\n]*?)\2\1(?!`)/g,
      (whole, delimiter: string, padding: string, raw: string) => resolve(raw.trim()) ?? whole,
    )
    .replace(
      /(!?)\[([^\]]*)\]\((attachment:(?:\[[^\]]+\])?[^)]+)\)/g,
      (whole, bang: string, label: string, raw: string) => resolve(raw, label, bang === '!', false) ?? whole,
    );
}
...
export function attachmentReferences(source: string, context?: AttachmentReferenceContext): AttachmentReference[] {
  ...
  const add = (raw: string) => {
    const parsed = parseAttachmentReference(raw),
      resolved = parsed && context ? resolveKnownAttachment(context, parsed) : parsed;
    if (!resolved) return;
    const reference: AttachmentReference = resolved.ticket
        ? { ticket: resolved.ticket, filename: resolved.filename }
        : { filename: resolved.filename },
      key = `${reference.ticket ?? ''}\0${reference.filename}`;
    if (!seen.has(key)) {
      seen.add(key);
