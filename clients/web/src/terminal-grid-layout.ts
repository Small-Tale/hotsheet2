...
export const TERMINAL_TILE_VERTICAL_CHROME =
  2 * (TERMINAL_TILE_BORDER_WIDTH + TERMINAL_TILE_FRAME_INSET) + TERMINAL_TILE_FOOTER_HEIGHT;
...
export function terminalDrawerGridLayout(width: number, height: number, fitHigh: number): TerminalGridLayout {
  const basis: TerminalGridBasis = 'high',
    fit = clampTerminalFit(fitHigh, basis),
    max = TERMINAL_GRID_MAX_HIGH;
  if (fit === 1) {
    const tileHeight = Math.max(TERMINAL_DRAWER_MIN_TILE_HEIGHT, Math.floor(Math.max(0, height)));
    return { basis, fit, max, tileWidth: terminalTileWidth(tileHeight), tileHeight };
  }
  const available = Math.max(TERMINAL_DRAWER_MIN_LAYOUT_WIDTH, Math.max(0, width)),
    tileWidth = Math.max(1, Math.floor((available - TERMINAL_GRID_GAP * (fit - 1)) / fit));
  return { basis, fit, max, tileWidth, tileHeight: terminalTileHeight(tileWidth) };
}
