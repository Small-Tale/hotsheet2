...
export function resetTicketRowDemo(root?: ParentNode): void {
  ...
  if (root) syncSettingsControls(root, 'ticket-list-row', {
    values: { title: ticketRowSettings.title.value, status: ticketRowSettings.status.value, priority: ticketRowSettings.priority.value, category: ticketRowSettings.category.value, tags: ticketRowSettings.tags.value },
    checked: { 'up-next': ticketRowSettings.upNext.value, selected: ticketRowSettings.selected.value, busy: ticketRowSettings.busy.value },
  });
}
...
export function TicketRowDemo() {
  return <section class="component-stage component-stage--row" aria-label="TicketRow demo">
    <div class="component-stage__canvas component-stage__canvas--row" role="listbox" aria-label="Example ticket list">
      {TicketRow({ slug: 'HS2-D3M0', title: ticketRowSettings.title.value, status: ticketRowSettings.status.value, priority: ticketRowSettings.priority.value, category: ticketRowSettings.category.value, tags: ticketRowSettings.tags.value.split(','), upNext: ticketRowSettings.upNext.value, selected: ticketRowSettings.selected.value, busy: ticketRowSettings.busy.value })}
    </div>
