...
export const galleryDemoMarkup=signal(false),galleryDemoDrawMode=signal(false),galleryDemoSelectedAnnotation=signal('demo-annotation');
const imageAnnotations=[{id:'demo-annotation',x:900,y:1100,width:3000,height:2400,text:'Review this alignment'}];
const videoAnnotations=[{id:'demo-annotation',x:1200,y:1800,width:2800,height:2600,start_ms:1000,end_ms:3000,text:'Transition is abrupt'}];
const galleryDemoGeometry={naturalWidth:1600,naturalHeight:1000,availableWidth:1100,availableHeight:650};
export function setGalleryDemo(open:boolean){galleryDemoUrl.value=open?demoImages[0].url:undefined;galleryDemoScale.value=undefined;galleryDemoMarkup.value=false;galleryDemoDrawMode.value=false;galleryDemoSelectedAnnotation.value='demo-annotation'}
...
export function AttachmentGalleryDemo(){const video=galleryDemoUrl.value===demoImages[2].url,animatedSvg=galleryDemoUrl.value===demoImages[1].url,timed=video||animatedSvg;return galleryDemoUrl.value?<AttachmentGallery images={demoImages} activeUrl={galleryDemoUrl.value} geometry={galleryDemoGeometry} selectedScale={galleryDemoScale.value} annotations={timed?videoAnnotations:imageAnnotations} markup={galleryDemoMarkup.value} drawMode={galleryDemoDrawMode.value} selectedAnnotation={galleryDemoSelectedAnnotation.value} playheadMs={timed?1500:0} durationMs={video?6000:animatedSvg?4000:0}/>:<button type="button" data-action="open-gallery-demo">Open gallery</button>}
