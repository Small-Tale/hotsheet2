...
export function syncTerminalDemoViewports(root: HTMLElement, mounts: Map<HTMLElement, () => void>): void {
  ...
  for (const element of elements) {
    if (mounts.has(element)) continue;
    mounts.set(
      element,
      mountStaticTerminalViewport(element, {
        output: terminalDemoOutput(element.dataset.terminalId ?? 'shell'),
        autoFocus: element.dataset.displayMode === 'interactive',
      }),
    );
  }
}
