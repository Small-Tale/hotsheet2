...
export type AIConversationScenario='empty'|'streaming'|'permission'|'completed'|'failed'|'interrupted';
...
function scenarioState(){
  ...
  if(scenario==='completed')return{messages:completed,busy:false,interruptible:false};
...
export function AIConversationDemo(){const state=scenarioState();return <section aria-label="AIConversation demo"><wa-button appearance="accent" data-action="open-ai-conversation-demo">Open conversation</wa-button><AIConversation open={aiConversationDemoOpen.value} tool="Codex" sessionId="019-demo-session" messages={state.messages} draft={aiConversationDraft.value} busy={state.busy} progress={state.progress} interruptible={state.interruptible} permissions={state.permissions} error={state.error}/></section>}

export function AIConversationSettings(){return <form class="settings-form" data-settings="ai-conversation"><wa-select name="scenario" label="Public state" value={aiConversationScenario.value}><wa-option value="empty">Empty</wa-option><wa-option value="streaming">Streaming</wa-option><wa-option value="permission">Permission requested</wa-option><wa-option value="completed">Completed</wa-option><wa-option value="failed">Failed</wa-option><wa-option value="interrupted">Interrupted</wa-option></wa-select><wa-button type="button" data-action="open-ai-conversation-demo">Open conversation</wa-button></form>}
