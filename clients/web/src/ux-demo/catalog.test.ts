...
describe('UX demo catalog', () => {
  it('has unique routes and the implemented component set', () => {
    ...
    expect(findDemo('project-tabs')?.uses).toEqual(['project-tab']);expect(findDemo('project-tab')?.uses).toEqual(['app-tab']);expect(findDemo('terminal-drawer')?.uses).toEqual(['app-tab']);expect(findDemo('terminal-dashboard')?.uses).toEqual(['menu-item']);expect(findDemo('terminal-visibility-dialog')?.uses).toEqual(['menu-item']);
