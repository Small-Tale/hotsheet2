...
describe('UX demo catalog', () => {
  it('has unique routes and the implemented component set', () => {
    ...
    expect(findDemo('terminal-visibility-dialog')?.uses).toEqual(['list-item']);
    ...
    expect(findDemo('ticket-inspector')?.uses).toEqual([
      'toolbar',
      'toolbar-text',
      'toolbar-control-group',
      'app-tab',
    ...
    expect(findDemo('repository-status-popover')?.uses).toEqual([
      'dialog-header',
      'value-table',
      'list-item',
      'list-header',
  ...
  it('maps nested categories and relationship metadata into the flat kerf catalog contract', () => {
    const sections = kerfCatalogSections(demoCatalog, { 'ticket-row': '2026-09-21T00:00:00Z' }),
      ticketList = sections.find((section) => section.category === 'Ticket workspace · List'),
      ticketRow = ticketList?.entries.find((entry) => entry.id === 'ticket-row');
    ...
    expect(
      sections
        .find((section) => section.category === 'Ticket inspector · Notes and activity')
  ...
  it('publishes every implemented app-owned catalog surface with the Kerf consumer metadata contract', () => {
    const extension = JSON.parse(
        readFileSync(new URL('../../ai/component-catalog-extension.json', import.meta.url), 'utf8'),
      ) as {
        ...
        entries: Array<{
          ...
          publicTokens: string[];
          geometry: { margin: string; border: string; padding: string; notes?: string[] };
          documentation: string;
        }>;
      },
      schema = JSON.parse(
        readFileSync(
          new URL('../../node_modules/@kerfjs/ui/ai/component-catalog-extension.schema.json', import.meta.url),
          'utf8',
        ),
      ) as { $defs: { geometryOwner: { enum: string[] } } };
    expect(extension.schemaVersion).toBe(1);
    expect(extension.package).toBe('hotsheet-web');
    expect(extension.entries.length).toBeGreaterThan(60);
    expect(new Set(extension.entries.map((entry) => entry.id)).size).toBe(extension.entries.length);
    const owners = new Set(schema.$defs.geometryOwner.enum),
      implemented = new Map(
        flattenCatalog()
    ...
    for (const entry of extension.entries) {
      expect(implemented.get(entry.id)?.name).toBe(entry.name);
      expect(entry.purpose).toBe(implemented.get(entry.id)?.description);
      expect(entry.useWhen.length).toBeGreaterThan(0);
      expect(entry.avoidWhen.length).toBeGreaterThan(0);
      expect(Array.isArray(entry.publicClasses)).toBe(true);
      expect(Array.isArray(entry.publicTokens)).toBe(true);
      expect(entry.documentation).toBe('docs/ux-components.md');
      for (const owner of [entry.geometry.margin, entry.geometry.border, entry.geometry.padding])
        expect(owners.has(owner)).toBe(true);
      if (Object.values(entry.geometry).includes('conditional'))
        expect(entry.geometry.notes?.length).toBeGreaterThan(0);
      expect(demoKind(entry.id)).toBe(entry.kind);
    }
    expect(demoKind('app-shell')).toBe('composition');
    expect(demoKind('tag-chip')).toBe('component');
    expect(demoKind('toolbar')).toBe('component');
  });
  ...
  it('accounts for every production component in the catalog or a documented exemption', () => {
    ...
    const EXEMPT: Record<string, string> = {
      ...
      'reader-overlay-surfaces':
        'Internal typed composition boundary for individually cataloged reader, dialog, menu, gallery, and transient surfaces.',
  ...
  it('records the planned ProjectSidebar composition', () => {
    expect(findDemo('project-sidebar')?.uses).toEqual([
      'toolbar',
      'project-summary',
      'repository-summary',
      'view-navigation',
  ...
  it('resets every canonical TagChip demo setting', () => {
    ...
    expect({
      ...
      event: tagChipSettings.event.value,
    }).toEqual({
      label: 'needs-design',
      variant: 'neutral',
      appearance: 'filled',
  ...
  it('resets every canonical StatusBadge demo setting', () => {
    ...
    }).toEqual({ status: 'started', showIcon: true, appearance: 'filled', compact: false });
  });

  it('offers and resets every repository-status headline scenario', () => {
    const settings = String(RepositoryStatusPopoverSettings());
    const demo = String(RepositoryStatusPopoverDemo()),
      style = readFileSync(fileURLToPath(new URL('./style.css', import.meta.url)), 'utf8');
    expect(demo).toContain('class="repository-status-demo"');
    expect(style).toMatchSource(/\.demo-catalog-examples \{ width: 100%; min-width: 0; \}/);
    expect(style).toMatchSource(/\.repository-status-demo \{ width: 100%; min-width: 0; \}/);
    for (const scenario of ['clean', 'dirty', 'ahead', 'behind', 'diverged', 'conflicted', 'error'])
      expect(settings).toContain(`value="${scenario}"`);
    expect(
      (['clean', 'dirty', 'ahead', 'behind', 'diverged', 'conflicted'] as const).map((scenario) => [
        scenario,
        repositoryStatusForScenario(scenario),
      ]),
    ).toMatchObject([
  ...
  it('offers and resets every server compatibility detail state', () => {
    const settings = String(ConnectionDetailsDialogSettings()),
      scenarios = [
        'source-stale',
        'revision-mismatch',
        'server-too-old-safe',
        'server-too-old-manual',
        'client-too-old',
        'unknown',
      ] as const;
    for (const scenario of scenarios) expect(settings).toContain(`value="${scenario}"`);
    expect(scenarios.map((scenario) => connectionDetailsAssessment(scenario).kind)).toEqual([
      'compatible',
      'compatible',
      'server_too_old',
      'server_too_old',
    ...
    connectionDetailsScenario.value = 'unknown';
    resetConnectionDetailsDemo();
    expect(connectionDetailsScenario.value).toBe('source-stale');
  });
  ...
  it('resets every canonical TicketRow demo setting', () => {
    ...
    expect({
      ...
      title: 'Build the first client ticket list',
      status: 'started',
      priority: 'high',
      category: 'feature',
      tags: 'client, ux',
