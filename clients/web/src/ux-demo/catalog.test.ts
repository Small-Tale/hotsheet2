...
describe('UX demo catalog', () => {
  it('has unique routes and the implemented component set', () => {
    ...
    expect(entries.filter(entry => entry.implemented).map(entry => entry.id)).toEqual(['app-shell', 'project-sidebar', 'project-summary', 'repository-summary', 'repository-status-popover', 'change-evidence-dialog', 'view-navigation', 'command-navigation', 'drive-control', 'workspace-header', 'page-header', 'project-tab', 'project-tabs', 'resizable-region', 'connection-state-banner', 'connection-details-dialog', 'quick-ticket-composer', 'ticket-list', 'ticket-row', 'ticket-board', 'ticket-board-column', 'global-search', 'search-query-input', 'search-scope-picker', 'search-suggestion-list', 'search-result-row', 'active-filter-bar', 'filter-chip', 'saved-view-action', 'ticket-inspector', 'ticket-info-panel', 'ticket-timeline', 'ticket-code-review', 'ticket-attachments', 'ticket-category-select', 'ticket-priority-select', 'ticket-status-menu', 'status-badge', 'tag-chip', 'ticket-reader', 'markdown-editor', 'attachment-gallery', 'not-working-dialog', 'note-composer', 'note-card', 'ai-conversation', 'permission-request', 'notification-center', 'terminal-drawer', 'terminal-dashboard', 'terminal-operations-sidebar', 'terminal-ticket-rail', 'fixed-aspect-terminal-card', 'terminal-visibility-dialog', 'content-transition', 'app-tab', 'select', 'toolbar', 'menu-item', 'menu-header', 'toolbar-control-group', 'toolbar-text', 'dialog-header', 'value-table', 'pending-attachment-picker']);
    ...
    expect(findDemo('project-tabs')?.uses).toEqual(['project-tab']);expect(findDemo('project-tab')?.uses).toEqual(['app-tab']);expect(findDemo('terminal-drawer')?.uses).toEqual(['app-tab']);expect(findDemo('terminal-dashboard')?.uses).toEqual(['fixed-aspect-terminal-card','menu-item']);expect(findDemo('terminal-visibility-dialog')?.uses).toEqual(['menu-item']);
