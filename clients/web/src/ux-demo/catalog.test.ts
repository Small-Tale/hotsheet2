...
describe('UX demo catalog', () => {
  it('has unique routes and the implemented component set', () => {
    ...
    expect(entries.filter(entry => entry.implemented).map(entry => entry.id)).toEqual(['workspace-header', 'quick-ticket-composer', 'ticket-list', 'ticket-row', 'ticket-board', 'ticket-inspector', 'status-badge', 'tag-chip', 'toolbar-control-group']);
