...
export class TicketHistory {
  ...
  constructor(
    private readonly read: (slug: string) => TicketSnapshot | undefined,
    private readonly apply: (slug: string, patch: TicketPatch) => Promise<boolean>,
  ) {}
  ...
  async executeMany(operations: Array<{ slug: string; patch: TicketPatch }>): Promise<boolean> {
    const changes = operations.flatMap(({ slug, patch }) => {
      const current = this.read(slug);
      return current
        ? [{ slug, before: Object.fromEntries(Object.keys(patch).map((key) => [key, current[key]])), after: patch }]
        : [];
    });
  ...
  async undo(): Promise<boolean> {
    return this.move(this.undoStack, this.redoStack, true);
  }
  async redo(): Promise<boolean> {
    return this.move(this.redoStack, this.undoStack, false);
  }
  private async move(source: Transaction[], destination: Transaction[], undo: boolean): Promise<boolean> {
    ...
    if (transaction.kind === 'external') {
      if (!(await (undo ? transaction.undo() : transaction.redo()))) return false;
      source.pop();
      destination.push(transaction);
      return true;
    }
    for (const change of undo ? [...transaction.changes].reverse() : transaction.changes) {
      const current = this.read(change.slug);
...
function matchingFields(current: TicketSnapshot, expected: TicketPatch, replacement: TicketPatch): TicketPatch {
  return Object.fromEntries(
    Object.keys(expected)
      ...
      .map((key) => [key, replacement[key]]),
  );
}

export interface ClipboardTicket {
  id: string;
  slug: string;
...
export function deduplicateTitle(title: string, existing: readonly string[]): string {
  ...
  let suffix = 2;
  while (occupied.has(`${title} (Copy ${suffix})`.toLocaleLowerCase())) suffix += 1;
