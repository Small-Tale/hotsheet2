...
describe('inline advanced-search tokens', () => {
  ...
  it('only consumes complete trailing tokens and preserves ordinary text', () => {
    expect(consumeSearchToken('parser tag:client ')).toMatchObject({
      text: 'parser',
      token: { kind: 'tag', value: 'client' },
    });
    ...
    expect(consumeSearchToken('has:attachment ')).toMatchObject({
      text: '',
      token: { kind: 'has', value: 'attachment' },
    });
    ...
    expect(consumeSearchToken('is:duplicate', true)).toMatchObject({
      text: '',
      token: { kind: 'is', value: 'duplicate' },
    });
    expect(consumeSearchToken('updated-after:4h ')).toEqual({ text: 'updated-after:4h ' });
    expect(consumeSearchToken('updated-after:4h ago ')).toMatchObject({
      text: '',
      token: { kind: 'date', raw: 'updated-after:4h ago' },
    });
    expect(consumeSearchToken('updated-after:2026/09/07', true)).toMatchObject({
      text: '',
      token: { kind: 'date', raw: 'updated-after:2026/09/07' },
    });
    expect(effectiveSearch('parser has:commit', [])).toMatchObject({
      text: 'parser',
      tokens: [{ kind: 'has', value: 'commit' }],
    });
    expect(effectiveSearch('', [tokenFromRaw('is:active')!])).toMatchObject({
      text: 'is:active',
      tokens: [{ kind: 'is', value: 'active' }],
    });
    ...
    expect(consumeSearchTokens('tag:"hello world" ')).toMatchObject({
      text: '',
      tokens: [{ kind: 'tag', value: 'hello world' }],
    });
  ...
  it('keeps committed tokens ordered inside ordinary boolean text', () => {
    expect(consumeSearchTokens('NOT tag:client', true)).toMatchObject({
      text: 'NOT ',
      tokens: [{ raw: 'tag:client', offset: 4 }],
    });
  ...
  it('labels date-picker tokens in the machine locale while keeping editable ISO syntax', () => {
    expect(dateTokenFromInput('created-after', '2026-09-01', '23:05', 'en-GB')).toMatchObject({
      kind: 'date',
      raw: 'created-after:2026-09-01T23:05',
      label: 'created after 01/09/2026, 23:05',
    });
    expect(dateTokenFromInput('created-after', '2026-09-01', '', 'en-US')).toMatchObject({
  ...
  it('maps lifecycle and attachment tokens to structured server queries', () => {
    const tokens = [
      tokenFromRaw('has:attachment')!,
      tokenFromRaw('has:media-annotation')!,
      tokenFromRaw('has:commit')!,
      tokenFromRaw('attachment:*.png')!,
      tokenFromRaw('tag:"needs design"')!,
      tokenFromRaw('completed-after:2026-09-01')!,
    ];
    expect(tokenQuery(tokens)).toMatchObject({
      has_attachment: true,
      has_media_annotation: true,
      has_commit: true,
      attachment: '*.png',
      tags: 'needs design',
      completed_after: parseSearchDate('2026-09-01'),
    });
    expect(tokenQuery([tokenFromRaw('started-before:2026-09-01')!])).toMatchObject({
      status: 'started',
      updated_before: expect.any(String),
    });
  ...
  it('adapts inline tokens to kerf TokenSearchToken keyed on raw', () => {
    ...
    expect(toTokenSearchToken(tag)).toEqual({
      value: 'tag:client',
      label: 'tag:client',
      offset: 3,
      accessibleLabel: 'tag client',
    });
    const has = tokenFromRaw('has:media-annotation')!;
    expect(toTokenSearchToken(has)).toMatchObject({
      value: 'has:media-annotation',
      label: 'has media annotation',
      accessibleLabel: 'has media annotation',
    });
  });
  ...
  it('round-trips kerf TokenSearchTokens back to inline tokens, preferring current parsed values', () => {
    ...
    const read = [
      { value: 'tag:client', label: 'tag:client', offset: 5 },
      { value: current[1].raw, label: current[1].label, offset: 9 },
    ];
  ...
  it('reconstructs an unknown chip from its raw and drops an unparseable one', () => {
    const back = fromTokenSearchTokens(
      [
        { value: 'is:started', label: 'is:started', offset: 0 },
        { value: 'not-a-token', label: 'x', offset: 4 },
      ],
      [],
    );
