...
describe('terminal viewport protocol', () => {
  it('builds a credential-free same-origin attach URL', () => {
    expect(
      terminalBrowserWebSocketUrl('/__hotsheet/project-api/project%20one', 'codex/main', {
        protocol: 'https:',
        host: 'hs.test',
      }),
    ).toBe('wss://hs.test/__hotsheet/project-api/project%20one/terminals/codex%2Fmain/attach');
  });
  it('encodes leased viewport identity, focus, visibility, interaction, and bounded finite dimensions', () => {
    ...
    expect(JSON.parse(terminalResizeClaim('viewer-1', 0, 24.9, true, false))).toEqual({
      resize: { viewer_id: 'viewer-1', cols: 1, rows: 24, focus: true, visible: false, interacting: false },
    });
    expect(JSON.parse(terminalResizeClaim('viewer-2', Number.NaN, Number.POSITIVE_INFINITY, false, false))).toEqual({
      resize: { viewer_id: 'viewer-2', cols: 1, rows: 1, focus: false, visible: false, interacting: false },
    });
    // A genuine interaction (tap/keystroke) marks the claim interacting so this device takes over sizing.
    expect(JSON.parse(terminalResizeClaim('viewer-3', 100, 40, true, true, true))).toEqual({
      resize: { viewer_id: 'viewer-3', cols: 100, rows: 40, focus: true, visible: true, interacting: true },
    });
  });
  it('accepts only valid server size broadcasts', () => {
    expect(parseTerminalSizeMessage('{"pty_size":{"cols":120,"rows":40},"driven_by":"viewer-1"}')).toEqual({
  ...
  it('focuses only the exact newly created project terminal', () => {
    const request = { projectId: 'project-a', terminalId: 'new-terminal' };
    expect(terminalViewportShouldAutoFocus(request, 'project-a', 'new-terminal')).toBe(true);
    expect(terminalViewportShouldAutoFocus(request, 'project-a', 'old-terminal')).toBe(false);
    expect(terminalViewportShouldAutoFocus(request, 'project-b', 'new-terminal')).toBe(false);
    expect(terminalViewportShouldAutoFocus(undefined, 'project-a', 'new-terminal')).toBe(false);
  });
  it('letterboxes smaller PTYs and always contains larger PTYs without clipping', () => {
    expect(terminalViewportScale(120, 40, 80, 24)).toBe(1);
    expect(terminalViewportScale(80, 24, 100, 30)).toBe(0.8);
    expect(terminalViewportScale(40, 12, 120, 40)).toBe(0.3);
  });
  it('uniformly scales the fixed 80 by 24 dashboard surface without changing its aspect', () => {
    expect([TERMINAL_PREVIEW_NATURAL_WIDTH, TERMINAL_PREVIEW_NATURAL_HEIGHT]).toEqual([1280, 768]);
  ...
  it('does not duplicate deep scrollback into read-only dashboard previews', () => {
    ...
    expect(terminalScrollbackLimit('scaled-preview', true)).toBe(0);
  ...
  it('uses the DOM renderer on Apple WebKit where WebGL can paint a blank glyph layer', () => {
    ...
    expect(
      terminalShouldUseWebgl(
        'Mozilla/5.0 (iPhone; CPU iPhone OS 27_0 like Mac OS X) AppleWebKit/605.1.15 CriOS/150.0 Mobile/15E148 Safari/604.1',
      ),
    ).toBe(false);
  ...
  it('removes only a leading styled zsh partial-line marker from late fixed-grid replay', () => {
    const encode = (value: string) => new TextEncoder().encode(value),
      decode = (value: Uint8Array) => new TextDecoder().decode(value);
    expect(
      decode(stripLeadingZshPromptEolMark(encode('\u001b[1m\u001b[7m%\u001b[27m\u001b[1m\u001b[0m\r\nprompt % '))),
    ).toBe('prompt % ');
    expect(decode(stripLeadingZshPromptEolMark(encode('%\r\nprompt % ')))).toBe('%\r\nprompt % ');
    expect(decode(stripLeadingZshPromptEolMark(encode('\u001b[7m% intentional\u001b[0m')))).toBe(
      '\u001b[7m% intentional\u001b[0m',
    );
  });
  it('reserves one physical containment row in dedicated terminals', () => {
    expect(terminalDedicatedGridSize(100, 30)).toEqual({ cols: 100, rows: 29 });
    expect(terminalDedicatedGridSize(1, 1)).toEqual({ cols: 1, rows: 1 });
  });
  it('keeps dashboard grids at one immutable 80 by 24 canonical geometry', () => {
    expect([
      TERMINAL_DASHBOARD_COLS,
      TERMINAL_DASHBOARD_ROWS,
      TERMINAL_DASHBOARD_FONT_SIZE,
