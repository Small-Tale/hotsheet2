...
export function boardColumnStatuses(columnId: string, hideVerified: boolean): string[] {
  switch (columnId) {
    case 'not-started': return ['not_started'];
    case 'started': return ['started'];
    case 'completed': return hideVerified ? ['completed', 'verified'] : ['completed'];
    case 'verified': return hideVerified ? [] : ['verified'];
    default: return [];
  }
}
...
export interface BoardColumnStream { cursor?: string; exhausted?: boolean }
...
export interface BoardColumnPage { loaded: number; exhausted?: boolean; streams?: Record<string, BoardColumnStream> }
...
export function nextBoardColumnFetch(statuses: readonly string[], page: BoardColumnPage | undefined): { status: string; cursor?: string } | undefined {
...
export function applyBoardColumnFetch(statuses: readonly string[], page: BoardColumnPage | undefined, status: string, nextCursor: string | undefined, loaded: number): BoardColumnPage {
