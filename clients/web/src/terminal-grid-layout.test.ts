...
describe('terminal grid layout', () => {
  it('switches axes only above the exact 600px boundary', () => {
    ...
    expect(terminalGridLayout(1200, 601, 4, 2)).toMatchObject({
      basis: 'across',
      fit: 4,
      max: 10,
      tileWidth: 291,
    ...
    expect(terminalGridLayout(1200, 600, 4, 2)).toMatchObject({
      basis: 'high',
      fit: 2,
      max: 3,
      tileWidth: 402,
  ...
  it('removes both container insets before fitting drawer tiles', () => {
    ...
    expect(terminalGridLayout(876, 296, 2, 1)).toMatchObject({
      basis: 'high',
      fit: 1,
      tileWidth: 405,
      tileHeight: 296,
  ...
  it('fits drawer level one by height but levels two and three by width', () => {
    expect(terminalDrawerGridLayout(876, 296, 1)).toMatchObject({
      basis: 'high',
      fit: 1,
      tileWidth: 405,
      tileHeight: 296,
    });
    expect(terminalDrawerGridLayout(876, 296, 2)).toMatchObject({
      basis: 'high',
      fit: 2,
      tileWidth: 432,
      tileHeight: 312,
    });
    expect(terminalDrawerGridLayout(876, 296, 3)).toMatchObject({
      basis: 'high',
      fit: 3,
      tileWidth: 284,
      tileHeight: 223,
