...
describe('terminal grid layout', () => {
  it('switches axes only above the exact 600px boundary', () => {
    ...
    expect(terminalGridLayout(1200, 601, 4, 2)).toMatchObject({ basis:'across', fit:4, max:10, tileWidth:291, tileHeight:228 });
    expect(terminalGridLayout(1200, 600, 4, 2)).toMatchObject({ basis:'high', fit:2, max:3, tileWidth:401, tileHeight:294 });
  });
  ...
  it('removes both container insets before fitting drawer tiles',()=>{
    ...
    expect(terminalGridLayout(876,296,2,1)).toMatchObject({basis:'high',fit:1,tileWidth:404,tileHeight:296});
  });

  it('fits drawer level one by height but levels two and three by width',()=>{
    expect(terminalDrawerGridLayout(876,296,1)).toMatchObject({basis:'high',fit:1,tileWidth:404,tileHeight:296});
    expect(terminalDrawerGridLayout(876,296,2)).toMatchObject({basis:'high',fit:2,tileWidth:432,tileHeight:313});
    expect(terminalDrawerGridLayout(876,296,3)).toMatchObject({basis:'high',fit:3,tileWidth:284,tileHeight:224});
  });

  it('keeps a 160px level-one tile and treats narrower layout widths as 240px',()=>{
    expect(terminalDrawerGridLayout(180,100,1)).toMatchObject({tileWidth:177,tileHeight:160});
    expect(terminalDrawerGridLayout(180,300,2)).toMatchObject({tileWidth:114,tileHeight:122});
  });

  it('adds token-mirrored card chrome outside the five-by-three terminal viewport',()=>{
    ...
    expect(TERMINAL_TILE_VERTICAL_CHROME).toBe(69.2);
    expect(terminalTileHeight(818)).toBe(544);
    expect(terminalTileWidth(541)).toBe(812);
  });
