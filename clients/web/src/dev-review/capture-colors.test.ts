...
describe('replaceModernColorFunctions', () => {
  it('normalizes modern colors in standalone, shadow, and gradient values', () => {
    ...
    expect(replaceModernColorFunctions('linear-gradient(lab(20% 1 2), color(display-p3 1 0 0))', convert)).toBe(
      'linear-gradient(rgb(1, 2, 3), rgb(1, 2, 3))',
    );
    expect(seen).toEqual(['oklab(50% 0.1 0.2)', 'oklch(70% 0.2 40 / 50%)', 'lab(20% 1 2)', 'color(display-p3 1 0 0)']);
  });
