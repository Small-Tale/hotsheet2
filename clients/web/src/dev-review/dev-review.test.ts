...
const attachment = {
  id: 'file-1',
  filename: '../notes.txt',
  dataUrl: `data:text/plain;base64,${Buffer.from('notes').toString('base64')}`,
  mimeType: 'text/plain',
  size: 5,
};
const submission = {
  notes: 'Button overlaps heading',
  captures: [capture],
  attachments: [attachment],
  actorRole: 'human' as const,
  pageUrl: 'http://localhost/ux-demo',
...
describe('dev review tool', () => {
  it('keeps the ticket dialog header and footer visually continuous with its body', async () => {
    ...
    const sharedDialogCss = await readFile(
      resolve(import.meta.dirname, '../components/native-popover-dialog.css'),
      'utf8',
    );
    expect(sharedDialogCss).toMatchSource(/\.dialog-surface .kui-panel-header \{ border-bottom: 0; \}/);
    expect(css.match(/\.hs-dev-review__dialog .kui-panel-header \{([^}]*)\}/)?.[1]).not.toContain('border-bottom');
  ...
  it('uses canonical spacing for the ticket dialog composition', async () => {
    ...
    expect(css).toMatch(
      /\.hs-dev-review__dialog-body \{[^}]*padding: var\(--kui-space-l\)[^}]*gap: var\(--kui-space-l\)/,
    );
    expect(css).toMatch(
      /\.hs-dev-review__dialog footer \{[^}]*padding: var\(--kui-space-s\) var\(--kui-space-l\)[^}]*gap: var\(--kui-space-xs\)/,
    );
  ...
  it('normalizes, resizes, and clamps capture geometry', () => {
    ...
    expect(resizeRect({ id: 'a', x: 20, y: 10, width: 60, height: 60 }, 'se', 100, 90)).toEqual({
      id: 'a',
      x: 20,
      y: 10,
      width: 80,
    ...
    expect(resizeRect({ id: 'a', x: 20, y: 10, width: 60, height: 60 }, 'nw', 75, 65)).toEqual({
      id: 'a',
      x: 56,
      y: 46,
      width: 24,
    ...
    expect(resizeRect({ id: 'a', x: 20, y: 10, width: 60, height: 60 }, 'e', 110, 40)).toEqual({
      id: 'a',
      x: 20,
      y: 10,
      width: 90,
    ...
    expect(resizeRect({ id: 'a', x: 20, y: 10, width: 60, height: 60 }, 'n', 50, 30)).toEqual({
      id: 'a',
      x: 20,
      y: 30,
      width: 60,
    ...
    expect(clampRectToViewport({ id: 'a', x: -5, y: 90, width: 120, height: 40 }, 100, 100)).toEqual({
      id: 'a',
      x: 0,
      y: 90,
      width: 100,
    ...
    expect(intersectRectWithViewport({ id: 'a', x: -5, y: 90, width: 20, height: 20 }, 100, 100)).toEqual({
      id: 'a',
      x: 0,
      y: 90,
      width: 15,
    ...
    expect(
      translateAnchoredRect({ id: 'a', x: 20, y: 30, width: 40, height: 50 }, { x: 100, y: 200 }, { x: 85, y: 140 }),
    ).toEqual({ id: 'a', x: 5, y: -30, width: 40, height: 50 });
  });
  ...
  it('validates notes and sanitizes capture and attachment filenames', () => {
    ...
    expect(() =>
      validateDevReviewSubmission({ ...submission, captures: [{ ...capture, dataUrl: 'data:image/jpeg;base64,x' }] }),
    ).toThrow(/PNG/);
    expect(() =>
      validateDevReviewSubmission({ ...submission, attachments: [{ ...attachment, dataUrl: 'not-data' }] }),
    ).toThrow(/Attachment/);
  });
  ...
  it('keeps ticket submission development-only and delegates a validated payload', async () => {
    ...
    expect(
      (await app.request('/__hotsheet/dev-review/tickets', { method: 'POST', body: JSON.stringify(submission) }))
        .status,
    ).toBe(404);
    const response = await app.request('/__hotsheet/dev-review/tickets', {
      method: 'POST',
    ...
    expect(submit).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        captures: [expect.objectContaining({ filename: 'review.png' })],
      }),
    );
    expect(
      (
        await createDevApp(false, submit).request('/__hotsheet/dev-review/tickets', {
          method: 'POST',
          headers: { 'x-hotsheet-dev-review': '1' },
        })
      ).status,
    ).toBe(404);
  });
  ...
  it('creates through the CLI adapter and attaches every decoded PNG', async () => {
    ...
    try {
      ...
      const result = await createCliDevReviewSubmitter({
        repoRoot: temp,
        storePath: resolve(temp, 'store'),
        cliPath: cli,
        finalize,
        commandRunner,
      })({ ...submission, notes: '- Cancel is too close to the countdown.' });
      ...
      expect(calls[0].args).toEqual(
        expect.arrayContaining([
          ...
          resolve(temp, 'store'),
          'new',
          '--title=UX feedback: - Cancel is too close to the countdown.',
        ]),
      );
      expect(
        calls[0].args.some((argument) => argument.startsWith('--details=- Cancel is too close to the countdown.')),
      ).toBe(true);
  ...
  it('surfaces a shell-quoted, copy-paste-runnable command when the CLI fails', async () => {
    ...
    try {
      ...
      const finalize = vi.fn(async () => undefined);
      const run = createCliDevReviewSubmitter({
        repoRoot: temp,
        storePath: resolve(temp, 'my store.hs2'),
        cliPath: cli,
        finalize,
        commandRunner,
      });
