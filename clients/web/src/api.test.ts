...
describe('attachment filename transport', () => {
  ...
  it('encodes filename references and host actions on checkout routes', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue(new Response(JSON.stringify({ path: '/store/screen shot.svg' }), { status: 200 }));
    const api = new Api('/api');
    expect(api.checkoutAttachmentByNameUrl('folder one', 'HS2-ONE', 'screen shot.svg')).toBe(
      '/api/checkouts/folder%20one/tickets/HS2-ONE/attachments/by-name/screen%20shot.svg',
    );
    expect(api.checkoutAttachmentThumbnailUrl('folder one', 'HS2-ONE', 'clip id')).toBe(
      '/api/checkouts/folder%20one/tickets/HS2-ONE/attachments/clip%20id/thumbnail',
    );
    await expect(
      api.checkoutAttachmentByNameAction('folder one', 'HS2-ONE', 'screen shot.svg', 'reveal'),
    ).resolves.toEqual({ path: '/store/screen shot.svg' });
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/checkouts/folder%20one/tickets/HS2-ONE/attachments/by-name/screen%20shot.svg/action',
      expect.objectContaining({ method: 'POST', body: '{"action":"reveal"}' }),
    );
  ...
  it('stores normalized media annotations on the attachment route', async () => {
    ...
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(
        new Response(JSON.stringify({ store: 'git-local', ticket: { attachments: [] } }), { status: 200 }),
      );
    await new Api('/api').updateCheckoutAttachmentAnnotations('folder', 'ticket', 'attachment', [annotation]);
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/checkouts/folder/tickets/ticket/attachments/attachment',
      expect.objectContaining({ method: 'PUT', body: JSON.stringify({ annotations: [annotation] }) }),
    );
    fetchMock.mockRestore();
  });
  ...
  it('requests the checkout-scoped corrupt ticket collection', async () => {
    const fetchMock = vi
      ...
      .mockImplementation(
        async () =>
          new Response(
            JSON.stringify([{ store: 'local', store_path: '/store', path: '/store/ticket.md', error: 'bad ticket' }]),
            { status: 200 },
          ),
      );
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(
      '/api/checkouts/folder%20with%20spaces/corrupt-tickets/repair',
      expect.objectContaining({ method: 'POST', body: '{"path":"/tmp/broken.md"}' }),
    );
    fetchMock.mockRestore();
  });
});
...
describe('custom view transport', () => {
  it('loads and replaces the shared saved-view collection', async () => {
    ...
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockImplementation(async () => new Response(JSON.stringify(views), { status: 200 }));
    const api = new Api('/api');
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/views',
      expect.objectContaining({ method: 'PUT', body: JSON.stringify(views) }),
    );
    fetchMock.mockRestore();
  });
});
...
describe('repository setup transport', () => {
  it('initializes the selected checkout and adds origin through checkout-scoped routes', async () => {
    ...
    await expect(
      api.configureRepositoryRemote('folder with spaces', 'git@example.com:team/project.git'),
    ).resolves.toEqual(status);
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      '/api/checkouts/folder%20with%20spaces/repository/init',
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/checkouts/folder%20with%20spaces/repository/remote',
      expect.objectContaining({ method: 'POST', body: '{"remote":"git@example.com:team/project.git"}' }),
    );
    fetchMock.mockRestore();
  });
});
...
describe('change polling transport', () => {
  ...
  it('requests the secret-hiding project proxy with a cursor and abort signal', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(new Response(JSON.stringify({ cursor: 8, events: [], overflow: false }), { status: 200 }));
      ...
      events: [
        {
          ...
          id: 'c',
          slug: 'codex',
          turn: { connection_id: 'c', event: { type: 'output', content: 'hello', truncated: false } },
        },
        {
          cursor: 12,
          store: 's',
          ...
          id: 'c',
          slug: 'codex',
          turn: { connection_id: 'c', event: { type: 'future_server_event', value: 1 } },
        },
      ],
    };
  ...
  it('delivers each cursor-addressed turn event once across duplicate project poll streams', () => {
    const guard = new TurnStreamReplayGuard(2),
      event = {
        cursor: 11,
        store: 's',
        kind: 'turn_event',
      ...
      response = { cursor: 11, overflow: false, events: [event, event] };
...
describe('client-owned AI drive transport', () => {
  it('starts, drives, interrupts, and closes a checkout-scoped connection', async () => {
    const connection = {
      ...
      role: 'main',
      busy: false,
      actions: ['send_turn', 'interrupt', 'close'],
    };
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockImplementation(async () => new Response(JSON.stringify(connection), { status: 202 }));
    const api = new Api('/api');
    ...
    await api.createToolConnection({
      tool: 'codex',
      checkout: 'checkout-1',
      source: 'git-main',
      connection_id: 'main/1',
      model: 'gpt-5.4',
      effort: 'high',
    });
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      5,
      '/api/ai-settings',
      expect.objectContaining({ method: 'PUT', body: '{"tool":"codex","model":"gpt-5.4","effort":"high"}' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      6,
      ...
      expect.objectContaining({
        method: 'POST',
        body: '{"tool":"codex","checkout":"checkout-1","source":"git-main","connection_id":"main/1","model":"gpt-5.4","effort":"high"}',
      }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      7,
      '/api/drive/connections/main%2F1/turns',
      expect.objectContaining({
        method: 'POST',
        body: '{"content":"Continue this work","session_id":"thread/1","model":"gpt-5.4","effort":"xhigh"}',
      }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      8,
      '/api/drive/connections/main%2F1/interrupt',
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      9,
      '/api/checkouts/checkout%2F1/drive/connections/main%2F1',
...
describe('provider onboarding transport', () => {
  it('creates a non-secret connection and links it as the checkout default', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/checkouts/folder%20with%20spaces/sources/github-main',
      expect.objectContaining({
        method: 'PUT',
        body: '{"provider":"github","locator":"small-tale/hotsheet2","make_default":true}',
      }),
    );
    fetchMock.mockRestore();
  });
  it('keeps GitHub device credentials on the server while starting, waiting, and cancelling', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      '/api/github-auth/device',
      expect.objectContaining({ method: 'POST', body: '{"web_base":"https://github.test"}' }),
    );
...
describe('terminal dashboard transport', () => {
  it('lists terminals, reads a safely encoded snapshot, and creates a project terminal', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(
        new Response(JSON.stringify([{ id: 'agent/1', alive: true, busy: false }]), { status: 200 }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ id: 'agent/1', alive: true, busy: false, scrollback: 'ready' }), { status: 200 }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ id: 'terminal-2', alive: true, busy: false }), { status: 200 }),
      )
      .mockResolvedValueOnce(
    ...
    await expect(
      api.createTerminal({ cwd: '/project root', connect: 'codex', model: 'gpt-5.4', effort: 'high' }),
    ).resolves.toMatchObject({ id: 'terminal-2' });
    await expect(api.createTerminal({ cwd: '/project root', shell_command: 'npm run lint' })).resolves.toMatchObject({
      id: 'terminal-3',
    });
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        method: 'POST',
        body: '{"cwd":"/project root","connect":"codex","model":"gpt-5.4","effort":"high"}',
      }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      4,
      '/api/terminals',
      expect.objectContaining({ method: 'POST', body: '{"cwd":"/project root","shell_command":"npm run lint"}' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      5,
...
describe('terminal settings transport', () => {
  it('keeps the global-history opt-out behind a machine-local host API', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockImplementation(async () => new Response('{"inherit_global_shell_history":true}', { status: 200 }));
    const api = new Api('/api');
    await expect(api.terminalSettings()).resolves.toEqual({ inherit_global_shell_history: true });
    await api.saveTerminalSettings({ inherit_global_shell_history: false });
    expect(fetchMock).toHaveBeenNthCalledWith(1, '/api/terminal-settings', expect.any(Object));
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/terminal-settings',
      expect.objectContaining({ method: 'PUT', body: '{"inherit_global_shell_history":false}' }),
    );
    fetchMock.mockRestore();
  });
});
...
describe('ticket search transport', () => {
  it('requests a bounded checkout page with an opaque cursor', async () => {
    ...
    const requested = new URL(target, 'https://hotsheet.test');
    expect(requested.pathname).toBe('/api/checkouts/folder%20with%20spaces/tickets');
  ...
  it('sends trimmed text through the comprehensive checkout query', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/checkouts/folder%20with%20spaces/tickets?text=HS2-QQRY00',
      expect.any(Object),
    );
    fetchMock.mockRestore();
  });
  it('requests non-compact provider-indexed rows for advanced search', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/checkouts/demo/tickets?text=referenced+ticket&compact=false&up_next=true',
      expect.any(Object),
    );
    fetchMock.mockRestore();
  });
  it('encodes inline attachment and lifecycle filters as structured query fields', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('[]', { status: 200 }));
    await new Api('/api').checkoutTickets('demo', {
      tags: 'client,needs design',
      has_attachment: true,
      has_media_annotation: true,
      has_commit: true,
      attachment: '*.png',
      completed_after: '2026-09-01T03:05:00.000Z',
    });
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/checkouts/demo/tickets?tags=client%2Cneeds+design&has_attachment=true&has_media_annotation=true&has_commit=true&attachment=*.png&completed_after=2026-09-01T03%3A05%3A00.000Z',
      expect.any(Object),
    );
    fetchMock.mockRestore();
  });
...
describe('checkout Trash transport', () => {
  it('empties Trash through the checkout-scoped capability route', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/checkouts/folder%20with%20spaces/trash/empty',
      expect.objectContaining({ method: 'POST' }),
    );
    fetchMock.mockRestore();
  });
  it('reads and writes the shared Trash retention setting', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockImplementation(async () => new Response('{"trash_cleanup_days":14}', { status: 200 }));
    const api = new Api('/api');
    await expect(api.trashSettings('folder with spaces')).resolves.toEqual({ trash_cleanup_days: 14 });
    await expect(api.saveTrashSettings('folder with spaces', { trash_cleanup_days: 21 })).resolves.toEqual({
      trash_cleanup_days: 14,
    });
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      '/api/checkouts/folder%20with%20spaces/trash-settings',
      expect.any(Object),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/checkouts/folder%20with%20spaces/trash-settings',
      expect.objectContaining({ method: 'PUT', body: '{"trash_cleanup_days":21}' }),
    );
    fetchMock.mockRestore();
  });
});
...
describe('structured ticket close transport', () => {
  it('sends duplicate outcomes and canonical target identity through the checkout route', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(new Response('{"store":"git","ticket":{}}', { status: 200 }));
    await new Api('/api').closeCheckoutTicket('folder with spaces', 'source/1', 'duplicate', {
  ...
  it('reads provider-aware reverse duplicate backlinks through the exact ticket route', async () => {
    const payload = {
        ...
        inaccessible_projects: [{ project_id: 'offline', project_name: 'Offline' }],
      },
      fetchMock = vi
        .spyOn(globalThis, 'fetch')
        .mockResolvedValue(new Response(JSON.stringify(payload), { status: 200 }));
    await expect(
      new Api('/api').checkoutTicketDuplicateBacklinks('folder with spaces', 'git:target/1'),
    ).resolves.toEqual(payload);
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/checkouts/folder%20with%20spaces/tickets/git%3Atarget%2F1/duplicate-backlinks',
      expect.anything(),
    );
    fetchMock.mockRestore();
  });
...
describe('ticket code review transport', () => {
  it('reads review targets and accepts an empty successful launch response', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ commits: [], ranges: [], truncated: false }), { status: 200 }),
      )
      .mockResolvedValueOnce(new Response(null, { status: 204 }))
      .mockResolvedValueOnce(new Response(null, { status: 204 }));
    const api = new Api('/api');
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/checkouts/folder%20with%20spaces/tickets/ticket%2F1/code-review',
      expect.objectContaining({ method: 'POST', body: '{"mode":"commit","commit":"abc"}' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      3,
      '/api/checkouts/folder%20with%20spaces/tickets/ticket%2F1/code-review',
...
describe('repository browser transport', () => {
  it('sends host file and review actions through checkout routes', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({ method: 'POST', body: '{"path":"src/a b.ts","action":"reveal"}' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/checkouts/folder%20with%20spaces/repository/review',
      expect.objectContaining({ method: 'POST', body: '{"mode":"range","from":"aaa","to":"bbb"}' }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      3,
      '/api/checkouts/folder%20with%20spaces/repository/review',
...
describe('atomic Not Working transport', () => {
  it('sends note, evidence, and concurrency token in one multipart request', async () => {
    const fetchMock = vi
      .spyOn(globalThis, 'fetch')
      .mockResolvedValue(new Response(JSON.stringify({ status: 'not_started', up_next: true }), { status: 200 }));
