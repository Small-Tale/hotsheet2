...
describe('ticketBoardGroups', () => {
  ...
  it('uses one column for Backlog and keeps Archive disjoint from Verified and Trash', () => {
    ...
    expect(
      ticketBoardGroups([ticket('archive'), ticket('moved'), ticket('verified'), ticket('deleted')], 'archive', false),
    ).toMatchObject([{ id: 'archive', title: 'Archive', tickets: [{ status: 'archive' }, { status: 'moved' }] }]);
  });
  ...
  it('derives absolute built-in column totals from the checkout summary', () => {
    ...
    expect(
      ['not-started', 'started', 'completed', 'verified'].map((id) =>
        ticketBoardGroupTotal(id, 10, 'all', counts, false),
      ),
    ).toEqual([150, 120, 50, 30]);
