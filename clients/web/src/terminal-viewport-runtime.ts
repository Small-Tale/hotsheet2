...
import { parseTerminalSizeMessage,TERMINAL_DRAWER_RESIZE_END_EVENT,TERMINAL_PREVIEW_NATURAL_HEIGHT,TERMINAL_PREVIEW_NATURAL_WIDTH,TERMINAL_RESIZE_SETTLE_MS,terminalPreviewScale,terminalReconnectDelay,terminalResizeClaim,terminalShouldAdoptServerSize,terminalViewportScale } from './terminal-viewport';
...
export function mountTerminalViewportRuntime(element:HTMLElement,{url,viewerId,autoFocus=false}:{url:string;viewerId:string;autoFocus?:boolean}):()=>void {
  const scaledPreview=element.dataset.displayMode==='scaled-preview',settledResize=element.classList.contains('terminal-viewport--dedicated'),insideDrawer=Boolean(element.closest('[data-region-id="app-terminal-drawer"]'));
  ...
  const applySettledGeometry=()=>{if(disposed)return;try{fit.fit()}catch{/* layout can be transiently zero-sized */}element.dataset.gridSize=`${terminal.cols}x${terminal.rows}`;reconcileScale();claim()};
