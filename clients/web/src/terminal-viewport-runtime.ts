...
export function mountTerminalViewportRuntime(element:HTMLElement,{url,viewerId}:{url:string;viewerId:string}):()=>void {
  ...
  let socket:WebSocket|undefined,reconnect:number|undefined,heartbeat:number|undefined,attempt=0,visible=true,disposed=false,serverSize:{cols:number;rows:number}|undefined;
  ...
  const resize=new ResizeObserver(()=> { reconcileScale();claim(); });resize.observe(element);
  ...
  const focus=()=> { claim(); };element.addEventListener('focusin',focus);element.addEventListener('focusout',focus);
  const input=terminal.onData(value=>{if(socket?.readyState===WebSocket.OPEN)socket.send(value)});
  connect();
  return ()=>{disposed=true;if(reconnect!==undefined)window.clearTimeout(reconnect);if(heartbeat!==undefined)window.clearInterval(heartbeat);resize.disconnect();intersection.disconnect();input.dispose();element.removeEventListener('focusin',focus);element.removeEventListener('focusout',focus);socket?.close();terminal.dispose()};
}
