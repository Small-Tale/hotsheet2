...
export function mountTerminalViewportRuntime(element:HTMLElement,{url,viewerId,autoFocus=false}:{url:string;viewerId:string;autoFocus?:boolean}):()=>void {
  ...
  const mobile80xM=()=>fixedDashboardGrid&&!scaledPreview&&isMobileViewport(window.innerWidth);
  ...
  const applyDashboardPhysicalFill=()=>{const screen=terminal.element?.querySelector<HTMLElement>('.xterm-screen');if(!terminal.element||!screen||screen.offsetWidth<=0||screen.offsetHeight<=0)return;const targetWidth=Math.max(1,element.clientWidth-1),targetHeight=Math.max(1,element.clientHeight-1);
    if(mobile80xM()){
      ...
      mobileGridRows=rows;terminal.element.style.transformOrigin='top left';terminal.element.style.transform=`scale(${scale})`;terminal.element.style.width='';terminal.element.style.height='';element.dataset.scale=String(scale);element.dataset.physicalScale=String(scale);element.dataset.gridSize=`${TERMINAL_DASHBOARD_COLS}x${rows}`;element.dataset.geometryReady='true';if(autoFocus&&focusRequested){terminal.focus();claim()}return;
    }
    const scale=terminalPhysicalScale(screen.offsetWidth,screen.offsetHeight,targetWidth,targetHeight);if(scale<=0)return;terminal.element.style.transformOrigin='';terminal.element.style.transform=`scale(${scale})`;terminal.element.style.width='';terminal.element.style.height='';element.dataset.physicalScale=String(scale);element.dataset.geometryReady='true';if(autoFocus&&focusRequested){terminal.focus();claim()}};
  const scheduleDashboardFill=()=>{if(!fixedDashboardGrid||dashboardFrame!==undefined)return;dashboardFrame=window.requestAnimationFrame(()=>{dashboardFrame=undefined;applyDashboardPhysicalFill()})};
  const render=terminal.onRender(scheduleDashboardFill);
  const applySettledGeometry=()=>{if(disposed)return;try{if(fixedDashboardGrid){terminal.resize(TERMINAL_DASHBOARD_COLS,mobile80xM()?mobileGridRows:TERMINAL_DASHBOARD_ROWS);scheduleDashboardFill()}else{fit.fit();if(settledResize){const contained=terminalDedicatedGridSize(terminal.cols,terminal.rows);terminal.resize(contained.cols,contained.rows);element.dataset.containmentRows='1'}}}catch{/* layout can be transiently zero-sized */}element.dataset.gridSize=`${terminal.cols}x${terminal.rows}`;reconcileScale();claim()};
