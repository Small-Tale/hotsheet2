...
export function mountTerminalViewportRuntime(element:HTMLElement,{url,viewerId,autoFocus=false}:{url:string;viewerId:string;autoFocus?:boolean}):()=>void {
  ...
  const connect=()=>{
    ...
    current.addEventListener('open',()=>{if(socket!==current)return;attempt=0;element.dataset.connection='connected';fitAndClaim();heartbeat=window.setInterval(claim,5_000)});
  ...
  connect();
  if(autoFocus)window.requestAnimationFrame(()=>{window.setTimeout(()=>{if(!terminal.element)return;terminal.focus();claim()},0)});
