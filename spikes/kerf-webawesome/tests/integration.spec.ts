...
test('uses host input/change events and does not emit wa-prefixed aliases', async ({ page }) => {
  ...
  const events = await page.evaluate(
    () => (globalThis as typeof globalThis & { spike: { events: string[] } }).spike.events,
  );
...
test('kerf morph preserves the upgraded element, live value, and focus', async ({ page }) => {
  ...
  expect(
    await page.evaluate(
      () =>
        document.querySelector('wa-input') ===
        (globalThis as typeof globalThis & { originalInput?: Element }).originalInput,
    ),
  ).toBe(true);
  await expect(host).toHaveJSProperty('value', 'survives');
  await host.locator('input').focus();
  await page.locator('[data-action="rerender"]').evaluate((button: HTMLElement) => {
    button.click();
  });
...
test('theme token changes and production build makes no external requests', async ({ page }) => {
  ...
  const before = await button.evaluate((el) =>
    getComputedStyle(el).getPropertyValue('--wa-color-brand-fill-loud').trim(),
  );
