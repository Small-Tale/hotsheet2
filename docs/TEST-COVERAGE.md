...
| ux-demo-tag-chip | docs/ux-components.md | Dev-only Hono UX catalog routing, hierarchical master/detail navigation, responsive persistent demo inspector with a stable single toggle, and reusable TagChip defaults/removal contract | `clients/web/src/components/tag-chip.test.ts`; `clients/web/src/ux-demo/catalog.test.ts` | `clients/web/tests/ux-demo.spec.ts` | — | double-covered |
