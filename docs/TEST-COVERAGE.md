...
| ux-demo-status-badge | docs/ux-components.md | Readable status mapping/Lucide icon contract plus complete control→output→reset→live-control→post-reset round trip | `clients/web/src/components/status-badge.test.ts`; `clients/web/src/ux-demo/catalog.test.ts` | `clients/web/tests/ux-demo.spec.ts` | — | double-covered |
