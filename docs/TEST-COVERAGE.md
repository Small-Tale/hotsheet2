...
| ux-demo-ticket-row | docs/ux-components.md | Responsive shared flat-list/elevated-column summary, normal inline slug/title/priority flow with stable-width slug and two-line clamp, baseline-pinned updated time, fully wrapping star/status/owner/tags second line with no hidden metadata, persistent independently keyboard-operable Up Next star, HS1-precedence special-state rail, blue selection, contrast-adjusted palette-validated Lucide category icons or fixed three-letter uppercase labels, functional context menu, relationship navigation, and complete settings reset round trip | `clients/web/src/components/category-presentation.test.ts`; `clients/web/src/components/ticket-row.test.ts`; `clients/web/src/ux-demo/catalog.test.ts` | `clients/web/tests/ux-demo.spec.ts` | — | double-covered |
