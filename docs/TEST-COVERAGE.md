...
| ux-demo-ticket-row | docs/ux-components.md | Responsive shared list/column summary, two-line title clamp, HS1-precedence special-state rail, blue selection, contrast-adjusted palette-validated Lucide category presentation, HS1 directional priority icons after the title, composed StatusBadge/TagChip, Lucide Up Next star, functional context menu, relationship navigation, and complete settings reset round trip | `clients/web/src/components/category-presentation.test.ts`; `clients/web/src/components/ticket-row.test.ts`; `clients/web/src/ux-demo/catalog.test.ts` | `clients/web/tests/ux-demo.spec.ts` | — | double-covered |
