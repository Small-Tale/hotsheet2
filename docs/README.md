...
and a from-scratch rewrite of the original [Hot Sheet](https://github.com/brianwestphal/hotsheet). It is
...
| Doc | Topic | Status |
|---|---|---|
| [02-ticket-storage.md](02-ticket-storage.md) | Default git-provider format, activity history, durable attachment metadata, multi-store IDs, auto-merge, copy/move, Trash restore + 30-day retention purge | Shipped core/server/web |
...
| [18-dev-review-tool.md](18-dev-review-tool.md) | Embeddable screenshot annotation, CSS Live Edit capture, and Hot Sheet ticket submission tool | Shipped web implementation |
...
| [13-drive-transport-interface.md](13-drive-transport-interface.md) | Drive/transport capability (capability-based, all tools) | Shipped |
...
| **Close reasons** (completed / not planned / duplicate-of / obsolete) | 02 §2.6a | Shipped (field + `close` op + index columns + query filter `close_reason`/`closed` across CLI/MCP/server + reopen-clears; capability-aware web close dialog searches all open projects, persists exact project/provider/native target identity, and navigates across projects without slug ambiguity; HS2-61/HS2-FTBFS5/HS2-SBW2XD) |
...
| Direct-to-disk CLI (+ `merge-driver`) | 04 §4.4 | Shipped (`hotsheet` init/bootstrap/link/new/ls/show/edit/close/copy/move/assign/people/read/setup/plugin/settings/key/import/sync/merge-driver/doctor/reindex/worklist/metrics/serve/cert/claim/claim-next/release/renew/trigger/work/launch/permission-hook`; `bootstrap` idempotently prepares a clean or existing project/store, refreshes detected AI-tool skills/MCP configuration, and configures an optional existing remote without replacing user configuration; `launch claude` reuses the checkout's local store link and running-server instance to route an ordinary external-terminal Claude session's permissions; `key` manages OS-keychain provider credentials without argv/settings values) |
...
| Plugin-only AI-tool integration (host + registry + capability accessors + first plugins) | 05 | Built (HS2-9/PEQ6Q8/HTNN4F: plugin registry and conformance gate; Claude+Codex+Antigravity+OpenCode bundled; optional drive-owned runtime model discovery merges current ids/efforts with manifest labels/default fallback and caches by runtime version; OpenCode has merge-safe MCP setup plus a live ACP stdio session client, streaming/cancel, metrics, and drift oracle; plugin-first lint forbids core branching on a tool id) |
...
| Secure provider/API-key registry | 04 §4.7.1 | Shipped (HS2-M1XMSX, HS2-N3R18X): injected secret-store port + macOS Keychain/Linux Secret Service/Windows Credential Manager adapters, metadata-only global registry, stdin-only CLI writes, env-only fallback |
...
| Repository/dashboard APIs | 04 §4.3, 06 | Git status snapshot and its production client chip/popover are shipped (HS2-RPVFA4); current flow/throughput/cycle time and usage summary are server-testable slices whose client visualizations remain (38RJMK) |
...
| Terminal/PTY hosting for AI tools | 05 §5.4 | Shipped (`hotsheet-terminals`: PTY + scrollback/live fan-out, manager, OSC 7/8/9/133 + spinner busy inference, server CRUD/input + live WS attach, tool-terminal busy→registry feed, explicit plugin `[launch]` setup/command composition, fake-agent permission round-trip E2E, and default detached broker preserving terminal streams/sizing across server stop/restart with Ping/Pong health + conservative idle-GC/socket cleanup; destructive cleanup is explicit via `serve --stop --kill-all-terminals` — HS2-10/A6R5QV/XTTTMV/RCKEJ9/4M67VN/G0ETNQ/SSHCRM/8HHFHN/ERT00F/SV3XS8/8GQNDQ) |
| Multi-viewer PTY sizing (server-arbitrated, focus-follows, leased; remote-safe) | 06 §6.7 | Shipped server/browser floor (HS2-BD7Q74: `hotsheet-terminals::SizeArbiter` — focus-follows default + smallest/largest/pinned, leased viewport claims + heartbeat/expire + focus-hold/min-delta/min-interval guards + disconnect self-heal, pure & transition/adversarial-tested; wired into the WS attach — size claims in, `{pty_size,driven_by}` out. HS2-946EQG settled the client grid contract, HS2-2ZCN7K provides its responsive global dashboard, HS2-PD4MZ9 supplies credential-hidden interactive xterm WebSocket viewports with input, claims, scaling, and reconnect, and HS2-586BVQ composes the persisted project drawer) |
| Drive/transport interface (steer a running tool) | 13, 05 §5.5, 06 §6.3 | Shipped (`Drive`/`SpawnDrive`, persistent app-server/channel/ACP drives, async `TurnEvent`, trigger/work loops, checkout/source-bound client-owned start/resume/interrupt/session APIs, the production ProjectSidebar `$hotsheet` control, and a per-connection streamed multi-turn Markdown conversation with inline permissions, truthful per-turn/total usage, and linked-source attributed normalized activity — HS2-106/108/115/DTPX2V/1TY7GC/PEQ6Q8/CQ6B96/5DGFG2/R2FGZ5/S3J29E/BXSPMN/AB9RNJ) |
...
| Trigger / permissions bridge | 05 §5.5, §5.7; 06 §6.6 | Shipped (`hotsheet-cli trigger`/`work`; capability-aware external-terminal `launch` with Claude hook route-back and no-`-C` linked-store discovery; FIFO `SharedPermissionBridge`; machine-local durable Always Allow rules; shared 24-hour eventual safe-deny guard; server `/permissions`,`/permissions/{id}`,`/permissions/ask`; Codex app-server approvals + Claude native interactive `PermissionRequest` / marked-headless `PreToolUse` hooks; web popup, cross-project badges, Notifications history, Ignore, and visible-only auto-Allow/auto-Deny — HS2-11/HS2-9R9YZW/HS2-YMR9HE/HS2-XCTAHM/HS2-TFBV7Z/HS2-C46G58/N4R6F3) |
| Browser web UX client (1st) → Tauri host (2nd) | 06, UX catalog | Partial; the production shell and `/ux-demo` cover the main ticket workflow, including provider-indexed advanced search through the ordinary project toolbar as the sole primary search surface; the duplicate global-search overlay and its overlay-only catalog entries are removed (HS2-383D6K/0094CQ). Exact ticket-reference links now push responsive, project-qualified reader layers that unwind independently without changing the main workspace selection (HS2-T5VNJ8). Every open project's live stream keeps inactive tab counts and live-work indicators authoritative while preserving cache-first switching (HS2-QBNRZY). Remaining feature-floor work is explicitly tracked: shell/navigation (HS2-W6JHT1), persistent selection/undo (HS2-4CAN74), board states (HS2-0W67Y6), global drops (HS2-R6P8MZ), catalog tooling (HS2-89692E), people/review UX (HS2-CRW5CP), settings (HS2-BDW1BN/S4TZ31), and native host/lifecycle work |
