...
| **Human assignment + in-the-loop/review** | 10 | Partial (`assignees` + `review_requests` fields, committed `people.json` roster, semantic merge, CLI `assign`/`people`, indexed `assignee`/`review_requested`/`review_by` filters, and `me`-resolved "assigned to me"/"needs my review"/"I requested" views — HS2-20/89/T84F9F/TCDTCH/NZT80R. Live/on-sync attention delivery and GitHub roster seeding remain) |
...
| Plugin-only AI-tool integration (host + registry + capability accessors + first plugins) | 05 | Built (HS2-9: `hotsheet-plugins` loader/registry + `Manifest`/`Plugin` capability accessors — instructions/skills(absence-as-feature)/mcp/drive; Claude+Codex+Antigravity+OpenCode bundled; OpenCode adds merge-safe `opencode.json` MCP setup and the ACP drive/metrics declarations; behavioral half in `hotsheet-aitools`; **plugin-first lint** forbids core branching on a tool id; conformance-gated. Live OpenCode ACP stdio session client remains a follow-up) |
...
| Checkout identity/discovery | 04 §4.3 | Shipped core/server/CLI/MCP (HS2-NGC8AE): readable canonical-path ids, many-to-many store links, setup registration, authenticated discovery API; server bearer tokens remain separate |
