...
| One server per machine serves all projects (multi-store) | 04 §4.3.1 | Partial (`server::multistore`: StoreHost + GET/POST /stores + scoped /stores/{id}/tickets read+write + GET /resolve/{ulid} cross-store + per-store watcher + file-backed indexes + stores.json discovery + one-machine-server-per-project discovery reconciliation (topology A) — HS2-87. Remaining: per-store writer locks + a client to drive multi-project discovery) |
