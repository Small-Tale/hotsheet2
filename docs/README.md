...
| Terminal/PTY hosting for AI tools | 05 §5.4 | Partial (`hotsheet-terminals`: `Terminal` (PTY + scrollback ring + live output fan-out), `TerminalManager`, `BusyDetector`; server `/terminals`,`/terminals/{id}`,`/terminals/{id}/input` open/read/input/kill routes — HS2-10/HS2-A6R5QV; **live WS attach** `GET /terminals/{id}/attach` (replay scrollback + stream output + forward input) — HS2-XTTTMV. Remaining: detached broker, busy→registry feed, OSC 7/8/9, tool-launch) |
