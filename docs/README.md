...
| **Human assignment + in-the-loop/review** | 10 | Core/server shipped (`assignees` + review model, roster, merge, CLI/server/MCP assignment, indexed `me` views, assignment live events, recipient notifications, GitHub public-email roster seed — HS2-20/89/T84F9F/TCDTCH/NZT80R). Native/on-sync presentation remains client work |
...
| Cross-tool activity stream | 15 | Core shipped: rolling digest storage + timeline API, full activity payloads over WebSocket/long-poll, and coarse attributed server-drive start/permission/end events (HS2-KP31ZE/4C68Y8). Rich native transcript mapping deferred; Announcer UI/TTS remains post-floor |
...
| Multi-viewer PTY sizing (server-arbitrated, focus-follows, leased; remote-safe) | 06 §6.7 | Partial (HS2-BD7Q74: `hotsheet-terminals::SizeArbiter` — focus-follows default + smallest/largest/pinned, leased viewport claims + heartbeat/expire + focus-hold/min-delta/min-interval guards + disconnect self-heal, pure & transition/adversarial-tested; wired into the WS attach — size claims in, `{pty_size,driven_by}` out. Remaining: the client-side viewport UX — letterbox/scale-to-fit, "tap to resize to this screen") |
...
| Trigger / permissions bridge | 05 §5.5, §5.7; 06 §6.6 | Shipped (`hotsheet-cli trigger`/`work`; FIFO `SharedPermissionBridge`; machine-local durable Always Allow rules; shared 24-hour eventual safe-deny guard; server `/permissions`,`/permissions/{id}`,`/permissions/ask`; Codex approvals + Claude PreToolUse hook; web popup, cross-project badges, Notifications history, Ignore, and visible-only auto-Allow/auto-Deny — HS2-11/HS2-9R9YZW/HS2-YMR9HE/HS2-XCTAHM/HS2-TFBV7Z) |
| Browser web UX client (1st) → Tauri host (2nd) | 06, UX catalog | Partial; dev-only Hono `/ux-demo` catalog now includes the responsive AppShell with inline QuickTicketComposer, project tabs, resize splitters, connection states, sidebar, workspace header, ticket collections/rows, a notes/Markdown/provenance-capable inspector, a dialog-sized reader reusing that inspector, GFM Markdown editing, and shared controls with unit + browser E2E coverage; broader feature floor and Tauri host remain |
