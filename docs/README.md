...
| Browser web UX client (1st) → Tauri host (2nd) | 06, UX catalog | Partial; dev-only Hono `/ux-demo` catalog now includes the responsive AppShell, project tabs, resize splitters, connection states, sidebar, workspace header, ticket collections/rows/composer, inspector sections, NoteCard, TicketReader, MarkdownEditor, and shared controls with unit + browser E2E coverage; broader feature floor and Tauri host remain |
