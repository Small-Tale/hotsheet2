...
| Drive/transport interface (steer a running tool) | 13, 05 §5.5, 06 §6.3 | Shipped (`Drive`/`SpawnDrive`, persistent app-server/channel/ACP drives, async `TurnEvent`, trigger/work loops, client-owned start/resume/interrupt/session APIs, and the production ProjectSidebar `$hotsheet` start/stop control projected from replayable connection events — HS2-106/108/115/DTPX2V/1TY7GC/PEQ6Q8/CQ6B96/5DGFG2/R2FGZ5) |
