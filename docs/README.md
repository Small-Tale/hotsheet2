...
| Drive/transport interface (steer a running tool) | 13, 05 §5.5, 06 §6.3 | Shipped (`Drive`/`SpawnDrive`, persistent app-server/channel/ACP drives, async `TurnEvent`, trigger/work loops, client-owned start/resume/interrupt/session APIs, the production ProjectSidebar `$hotsheet` control, and a per-connection streamed multi-turn Markdown conversation with inline permissions, truthful per-turn/total usage, and attributed normalized activity — HS2-106/108/115/DTPX2V/1TY7GC/PEQ6Q8/CQ6B96/5DGFG2/R2FGZ5/S3J29E/BXSPMN) |
