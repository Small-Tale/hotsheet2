...
> owning doc; the _field list + types + tiering_ live here.
...
| `close_reason`                                                                           | `completed\|not_planned\|duplicate\|obsolete\|works_as_designed` \| null                                                    | no             | shared | **Optional**, orthogonal to `status`. HS2-24/61                                                                                                                                                            |
...
| **Coordination** ([05](05-ai-tool-plugins.md) §5.7)                                      |                                                                                                                             |                |        |                                                                                                                                                                                                            |
...
| `claim_history`                                                                          | list<{ id:ULID, kind:`claim\|renew\|release`, worker:string, at:RFC3339, lease_expires_at:RFC3339?, worker_label:string? }> | no (def `[]`)  | shared | Append-only lifecycle. Claim/renew expiry is the exact implicit end when no release event follows.                                                                                                         |
