...
- **Real local web entry point (initial implementation, HS2-0P1MDG).** `/` renders the
  ...
  metadata and never autoplay. Their poster comes from a content-addressed thumbnail
  generated by `ffmpeg` at upload time and kept in the host's local Hot Sheet cache;
  existing attachments generate it lazily on first request. `HOTSHEET_FFMPEG` can name
  a non-PATH executable. The original video response uses its native media MIME type,
  advertises byte-range support, and returns valid single-range `206` responses so
  Safari and other media engines can discover duration and seek normally. App packaging
