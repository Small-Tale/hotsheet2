...
(docs/9). _Change for HS2:_ ULID ids + all-caps slug, git-file backing (docs/02).
...
→ larger editing surface**, with the reader button reachable _while_ editing to jump
...
(docs/3, 5); paste from clipboard (docs/77); promised-file drops (docs/130). _Change:_
...
auto-generated from the index (docs/6). _Change:_ generated from SQLite index
...
category/priority filters, include-backlog/archive rows (docs/4 §4.8, 40). _Change:_
...
project-scoped client-state guard (docs/125, 126). _Change:_ a "project" now
...
_Change:_ maps directly onto the shared-vs-local data model (docs/02 §2.11); simplify
...
per-project allow-rules, commands log, busy/idle (docs/12, 14, 47, 64). _Change:_
...
be able to ask, _relatively unsupervised_, for support for another tool (OpenCode /
...
129). _Decide:_ which transports v1 supports (Claude channel is a given; pick 1–2
...
78, 21). _Change:_ proxy the Rust core, not REST-over-HTTP.
...
The Announcer _build_ is post-floor (long-tail HS2-17).
...
_Change:_ **git history IS the backup** — most of this subsystem is obsoleted
...
self-hosting, remote-client tab mounting (docs/46, 93, 94, 96, 97, 112). _Change:_
...
134). _Change:_ Tauri **launches + supervises an independent local server** that
...
memory-pressure budgeting (docs/8, 1, 87, 128, 131). _Change:_ Rust server/CLI over
...
render targets, project-scoped state guard (docs/1, 60, 61, 62, 126). _Change:_ HS2
