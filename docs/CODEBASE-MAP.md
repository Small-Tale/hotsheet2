...
hot-sheet2/                  # this repo = CODE only; tickets are a SEPARATE store (below)
  ...
  clients/web/               # Kerf + Web Awesome API-only web/Tauri UI foundation
    ...
    src/components/project-close-dialog.tsx # Shared-menu running-resource inventory, live terminal/chat preview, and explicit keep-running versus stop-all project-tab dismissal
  ...
  crates/
    ...
    hotsheet-server/         # `hotsheet-server` binary (axum HTTP + WS)
      ...
      src/client_drive.rs    #   client-owned prepared AI connections: plugin-neutral create/attach, sequential resumable turns, shared-session exclusion, durable project session catalog/home, capability-present interrupt, metrics-priced usage plus normalized activity projection, and injectable fake/native backends (HS2-5DGFG2/KH8FBA/BXSPMN)
    ...
    hotsheet-index/          # disposable SQLite + FTS5 index (cache over the store)
      src/lib.rs             #   Index (SCHEMA_VERSION 9): open_reconciled/reconcile (git-diff fast path)/rebuild/upsert/delete/query + hash_bytes; FTS covers slug/title/tags/details/notes/attachment filenames; feedback_needed column mirrors unresolved description, typed-note, or legacy-prefix feedback exchanges; reconcile/rebuild skip unparseable files (HS2-PRVPCQ); facets: tags + assignees + reviews(who/requested_by) tables; filters incl. assignee/review_requested/review_by (facet joins) + claimed + blocked/unblocked (json_each over blocked_by vs done set) + created/updated date-range + moved-tombstones-hidden-by-default; keyset page_after paging
...
| Web client UI foundation | `clients/web/src/{dev-server.ts,project-bridge.ts,compatibility.ts,active-ticket-work.ts,project-tab-refresh.ts,not-working-workflow.ts,ticket-bulk-operations.ts,ticket-link-resolution.ts,conversation-export.ts,conversation-export-bridge.ts,render-metrics.ts,theme.css,components/,ux-demo/,dev-review/}` (dev-only Hono catalog + real components, including stateful PermissionRequestCard and AIConversation settings matrices; trusted host-selected portable conversation bundle reads/writes with `components/conversation-export-dialog.*`; authenticated server compatibility negotiation including local source-staleness warnings and corrupt-file platform reveal; claim-lease-derived ticket activity with local expiry scheduling and authoritative background cache reconciliation for every open project tab; exact cross-project ticket-link resolution with `components/ticket-link-choice-dialog.*`; development render-pass/DOM-mutation budgets; shared Web Awesome-backed surface/fill/border/text theme plus Hot Sheet-only ticket-state and shell-divider aliases in `theme.css`, consumed by production, UX-demo, and Dev Review styles and enforced against repeated literals by `theme.test.ts`; content-anchored review overlay with frame-coalesced geometry writes, lazy html2canvas capture, and a single-commit local CLI bridge in `dev-review/`; application shell composition in `components/{app-shell,workspace-header,page-header,project-tab-bar,project-tab,resizable-region,connection-state-banner,loading-spinner,ai-conversation}.*`; shared project/settings navigation and local custom-command UI in `components/{menu-item,menu-header,settings-navigation,command-navigation,command-run-dialog}.*`; board and capability-aware bulk-action composition in `components/{ticket-board,ticket-board-column,ticket-row,ticket-row-context-menu,bulk-ticket-dialog}.*`; ticket content reading/editing in `components/{corrupt-ticket-row,note-card,note-composer,ticket-notes,markdown-preview,markdown-editor,ticket-info-panel,ticket-inspector,ticket-inspector-placeholder,ticket-reader,not-working-dialog,pending-attachment-picker}.*`, including safe reveal/AI-repair affordances, zero-selection separator handling, provider-gated note create/edit/delete, and compensating Not Working evidence submission), `spikes/kerf-webawesome/` (Kerf/Web Awesome compatibility proof), [06](06-clients.md), [UX component catalog](ux-components.md), [Dev Review](18-dev-review-tool.md) |
| Inline advanced-search parsing | `clients/web/src/inline-search.ts` normalizes quoted tags, attachment globs, and lifecycle date filters for the checkout query contract; `clients/web/src/components/workspace-header.*` owns chips, autocomplete, date helper, and syntax help, [03](03-indexing-and-query.md) |
...
| Portable video posters | `clients/web/src/video-posters.ts` for browser-native upload/backfill and non-fatal codec fallback; checkout attachment `thumbnail` GET/PUT handlers plus SHA-256 cache and optional-only ffmpeg acceleration in `crates/hotsheet-server/src/{lib.rs,media.rs}`, [04](04-core-server-cli.md), [06](06-clients.md) |
