...
hot-sheet2/                  # this repo = CODE only; tickets are a SEPARATE store (below)
  ...
  crates/
    ...
    hotsheet-server/         # `hotsheet-server` binary (axum HTTP + WS)
      ...
      src/terminal_broker.rs #   server↔detached-broker integration (HS2-ERT00F): TerminalBroker::ensure (discover/spawn the broker per project under ${HOTSHEET_HOME}/broker) + call (per-request BrokerClient round-trip); `serve --terminal-broker` routes /terminals ops through it so terminals survive a server restart (live WS attach through the broker is a follow-up)
