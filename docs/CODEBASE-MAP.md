...
| Repository status browser / host file actions | `crates/hotsheet-ticketing/src/repository_status.rs` for porcelain-v2 file state; `crates/hotsheet-server/src/repository_browser.rs` plus checkout `/repository/status`, `/repository/review`, and `/repository/files/action` routes; client composition in `clients/web/src/components/{repository-status-popover,ticket-code-review}.*`, [06](06-clients.md) §6.10 |
