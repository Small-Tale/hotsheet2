...
| Board (column) view & per-column pagination                | `clients/web/src/ticket-board-layout.ts` (status→column bucketing + absolute per-column totals from the checkout counts) and `clients/web/src/board-pagination.ts` (column→status map, per-column-view predicate, hasMore) drive `clients/web/src/components/ticket-board*.tsx`; the per-column cursors/loaders (`boardColumnPages`, `loadBoardColumnMore`, `fetchExpandedBoardColumnRows`) live in `clients/web/src/main.tsx` (HS2-8NBGBX), [06](06-clients.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
