...
hot-sheet2/                  # this repo = CODE only; tickets are a SEPARATE store (below)
  ...
  crates/
    ...
    hotsheet-plugins/        # AI-tool plugin loader + registry (core `plugins` module)
      src/setup.rs           #   run_setup: core-owned one-shot setup writers (monotonic versioned instructions/skill bundle; equal-version divergent project variants are preserved; MCP config and permission hook via [hooks] capability; merge-safe and idempotent, HS2-XCTAHM) + mcp_command — shared by CLI + server (HS2-91)
