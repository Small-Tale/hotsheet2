...
| Multi-source checkout ordering / pagination                | `hotsheet-ticketing/src/checkout_order.rs` comparator + `merge_rows`; value keysets via `TicketQuery::after_key` (index SQL in `hotsheet-index/src/lib.rs`) and `TicketProvider::query_after` (`provider.rs` helpers + GitHub/GitLab/Jira native overrides); server batched k-way page merge and unpaged array in `hotsheet-server/src/lib.rs` (`list_checkout_ticket_page`, `list_checkout_tickets`); serverless MCP merge in `hotsheet-mcp/src/lib.rs`, [03](03-indexing-and-query.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
