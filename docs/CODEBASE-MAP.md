...
hot-sheet2/                  # this repo = CODE only; tickets are a SEPARATE store (below)
  ...
  clients/web/               # Kerf + Web Awesome API-only web/Tauri UI foundation
    ...
    src/dev-review/          #   Framework-neutral content-anchored review overlay, batched capture UI, and single-commit local-dev CLI submission adapter
    src/components/          #   Production domain UI components, including shared MenuItem/MenuHeader, project/page headers, sidebar/tab-shell surfaces; shared HS1 customization palette, cursor semantics, and Lucide renderer/policy gate
