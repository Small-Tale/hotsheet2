...
hot-sheet2/                  # this repo = CODE only; tickets are a SEPARATE store (below)
  ...
  crates/
    ...
    hotsheet-ticketing/      # engine crate (sync API, injected ports)
      ...
      src/distwork.rs        #   distributed self-claim fan-out: select_and_claim (worklist candidate + git-ref CAS → local claim; two machines never double-claim); docs/08 §8.5, HS2-E7RXXR
    ...
    hotsheet-aitools/        # AI-tool host (behavioral half): the drive/transport interface
      ...
      src/codex.rs (perm)    #   Codex approval ServerRequests route through a PermissionPolicy (bridge allow-rules + default) via decide_approval, not blind-approve (HS2-0QGW07); live human WS round-trip pending (HS2-113)
