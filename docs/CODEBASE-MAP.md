...
hot-sheet2/                  # this repo = CODE only; tickets are a SEPARATE store (below)
  ...
  crates/
    ...
    hotsheet-ticketing/      # engine crate (sync API, injected ports)
      ...
      src/checkout_order.rs  #   shared multi-source checkout merge order, ranks, and global limit (HS2-M0YTB6)
...
| Multi-source checkout ordering / pagination                | `hotsheet-ticketing/src/checkout_order.rs` comparator + `merge_rows`; server k-way page merge and unpaged array in `hotsheet-server/src/lib.rs` (`list_checkout_ticket_page`, `list_checkout_tickets`); serverless MCP merge in `hotsheet-mcp/src/lib.rs`, [03](03-indexing-and-query.md) |
